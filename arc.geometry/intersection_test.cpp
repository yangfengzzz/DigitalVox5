...
bool IntersectionTest::TriangleTriangle3d(const Eigen::Vector3d& p0,
    ...
    const Eigen::Vector3d sigma =
            (((p0 - mu).array().square() + (p1 - mu).array().square() + (p2 - mu).array().square() +
              (q0 - mu).array().square() + (q1 - mu).array().square() + (q2 - mu).array().square()) /
             5)
                    .sqrt() +
            1e-12;
    ...
    return NoDivTriTriIsect(p0m.data(), p1m.data(), p2m.data(), q0m.data(), q1m.data(), q2m.data()) != 0;
}
...
bool IntersectionTest::TriangleAABB(const Eigen::Vector3d& box_center,
    ...
    double* tri_verts[3] = {const_cast<double*>(vert0.data()), const_cast<double*>(vert1.data()),
                            const_cast<double*>(vert2.data())};
    return triBoxOverlap(const_cast<double*>(box_center.data()), const_cast<double*>(box_half_size.data()),
                         tri_verts) != 0;
}
...
double IntersectionTest::LineSegmentsMinimumDistance(const Eigen::Vector3d& p0,
                                                     const Eigen::Vector3d& p1,
                                                     const Eigen::Vector3d& q0,
                                                     const Eigen::Vector3d& q1) {
