...
bool ReadTriangleMesh(const std::string &filename,
    ...
    if (params.print_progress) {
        auto progress_text = std::string("Reading ") + utility::ToUpper(filename_ext) + " file: " + filename;
...
bool AddTrianglesByEarClipping(geometry::TriangleMesh &mesh, std::vector<unsigned int> &indices) {
    ...
    if (n > 3) {
        for (int i = 0; i < n; i++) {
            const Eigen::Vector3d &v1 = mesh.vertices_[indices[(i + 1) % n]] - mesh.vertices_[indices[i % n]];
            const Eigen::Vector3d &v2 = mesh.vertices_[indices[(i + 2) % n]] - mesh.vertices_[indices[(i + 1) % n]];
    ...
    while (n > 3) {
        ...
        found_ear = false;
        for (int i = 1; i < n - 1; i++) {
            const Eigen::Vector3d &v1 = mesh.vertices_[indices[i]] - mesh.vertices_[indices[i - 1]];
            const Eigen::Vector3d &v2 = mesh.vertices_[indices[i + 1]] - mesh.vertices_[indices[i]];
