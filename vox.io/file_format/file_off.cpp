...
bool ReadTriangleMeshFromOFF(const std::string &filename,
    ...
    if (!file) {
        LOGW("Read OFF failed: unable to open file: {}", filename)
        return false;
    }
    ...
    if (header != "OFF" && header != "COFF" && header != "NOFF" && header != "CNOFF") {
        LOGW("Read OFF failed: header keyword '{}' not supported.", header)
        return false;
    }
    ...
    if (!(iss >> num_of_vertices >> num_of_faces >> num_of_edges)) {
        LOGW("Read OFF failed: could not read file info.")
        return false;
    }

    if (num_of_vertices == 0 || num_of_faces == 0) {
        LOGW("Read OFF failed: mesh has no vertices or faces.")
        return false;
    }
    ...
    for (size_t vidx = 0; vidx < num_of_vertices; vidx++) {
        ...
        if (!(iss >> vx >> vy >> vz)) {
            LOGW("Read OFF failed: could not read all vertex values.")
            return false;
        }
        ...
        if (parse_vertex_normals) {
            if (!(iss >> nx >> ny >> nz)) {
                LOGW("Read OFF failed: could not read all vertex normal "
                     "values.")
                return false;
            }
        ...
        if (parse_vertex_colors) {
            if (!(iss >> r >> g >> b >> alpha)) {
                LOGW("Read OFF failed: could not read all vertex color "
                     "values.")
                return false;
            }
            mesh.vertex_colors_[vidx] = Eigen::Vector3d(r / 255, g / 255, b / 255);
        }
    ...
    for (size_t tidx = 0; tidx < num_of_faces; tidx++) {
        ...
        for (size_t vidx = 0; vidx < n; vidx++) {
            if (!(iss >> vertex_index)) {
                LOGW("Read OFF failed: could not read all vertex "
                     "indices.")
                return false;
            }
        ...
        if (!AddTrianglesByEarClipping(mesh, indices)) {
            LOGW("Read OFF failed: A polygon in the mesh could not be "
                 "decomposed into triangles. Vertex indices count: {}",
                 indices.size())
            return false;
        }
        ++reporter;
...
bool WriteTriangleMeshToOFF(const std::string &filename,
    ...
    if (!file) {
        LOGW("Write OFF failed: unable to open file.")
        return false;
    }

    if (mesh.HasTriangleNormals()) {
        LOGW("Write OFF cannot include triangle normals.")
    }
    ...
    if (num_of_vertices == 0 || num_of_triangles == 0) {
        LOGW("Write OFF failed: empty file.")
        return false;
    }
    ...
    utility::ProgressBar progress_bar(num_of_vertices + num_of_triangles, "Writing OFF: ", print_progress);
    for (size_t vidx = 0; vidx < num_of_vertices; ++vidx) {
        ...
        if (write_vertex_colors) {
            ...
            file << " " << std::round(color(0) * 255.0) << " " << std::round(color(1) * 255.0) << " "
