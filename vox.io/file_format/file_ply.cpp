...
int ReadColorCallback(p_ply_argument argument) {
    ...
    double value = ply_get_argument_value(argument);
    state_ptr->pointcloud_ptr->colors_[state_ptr->color_index](index) = value / 255.0;
...
int ReadNormalCallback(p_ply_argument argument) {
    ...
    double value = ply_get_argument_value(argument);
    state_ptr->mesh_ptr->vertex_normals_[state_ptr->normal_index](index) = value;
    if (index == 2) {  // reading 'nz'
        state_ptr->normal_index++;
    }
    return 1;
...
int ReadFaceCallBack(p_ply_argument argument) {
    ...
    if (long(state_ptr->face.size()) == length) {
        if (!AddTrianglesByEarClipping(*state_ptr->mesh_ptr, state_ptr->face)) {
            LOGW("Read PLY failed: A polygon in the mesh could not be "
...
int ReadColorCallback(p_ply_argument argument) {
    ...
    double value = ply_get_argument_value(argument);
    state_ptr->lineset_ptr->colors_[state_ptr->color_index](index) = value / 255.0;
    if (index == 2) {  // reading 'blue'
        state_ptr->color_index++;
        ++(*state_ptr->progress_bar);
    }
...
FileGeometry ReadFileGeometryTypePLY(const std::string &path) {
    p_ply ply_file = ply_open(path.c_str(), nullptr, 0, nullptr);
    if (!ply_file) {
    ...
    auto nVertices = ply_set_read_cb(ply_file, "vertex", "x", nullptr, nullptr, 0);
    auto nLines = ply_set_read_cb(ply_file, "edge", "vertex1", nullptr, nullptr, 0);
    auto nFaces = ply_set_read_cb(ply_file, "face", "vertex_indices", nullptr, nullptr, 0);
    if (nFaces == 0) {
        nFaces = ply_set_read_cb(ply_file, "face", "vertex_index", nullptr, nullptr, 0);
    }
    ply_close(ply_file);
...
bool ReadPointCloudFromPLY(const std::string &filename,
    ...
    if (!ply_file) {
        LOGW("Read PLY failed: unable to open file: {}", filename.c_str())
        return false;
    }
    if (!ply_read_header(ply_file)) {
        LOGW("Read PLY failed: unable to parse header.")
        ply_close(ply_file);
        return false;
    }
    ...
    if (state.vertex_num <= 0) {
        LOGW("Read PLY failed: number of vertex <= 0.")
        ply_close(ply_file);
        return false;
    }
    ...
    if (!ply_read(ply_file)) {
        LOGW("Read PLY failed: unable to read file: {}", filename)
        ply_close(ply_file);
        return false;
    }
...
bool WritePointCloudToPLY(const std::string &filename,
    ...
    if (pointcloud.IsEmpty()) {
        LOGW("Write PLY failed: point cloud has 0 points.")
        return false;
    }

    p_ply ply_file =
            ply_create(filename.c_str(), bool(params.write_ascii) ? PLY_ASCII : PLY_LITTLE_ENDIAN, nullptr, 0, nullptr);
    if (!ply_file) {
        LOGW("Write PLY failed: unable to open file: {}", filename)
        return false;
    }
    ply_add_comment(ply_file, "Created by Open3D");
    ply_add_element(ply_file, "vertex", static_cast<long>(pointcloud.points_.size()));
    ...
    if (!ply_write_header(ply_file)) {
        LOGW("Write PLY failed: unable to write header.")
        ply_close(ply_file);
        return false;
    }
    ...
    for (size_t i = 0; i < pointcloud.points_.size(); i++) {
        ...
        if (pointcloud.HasColors()) {
            ...
            if (!printed_color_warning &&
                ...
                LOGW("Write Ply clamped color value to valid range")
...
bool ReadTriangleMeshFromPLY(const std::string &filename,
    ...
    p_ply ply_file = ply_open(filename.c_str(), nullptr, 0, nullptr);
    if (!ply_file) {
        LOGW("Read PLY failed: unable to open file: {}", filename)
        return false;
    }
    if (!ply_read_header(ply_file)) {
        LOGW("Read PLY failed: unable to parse header.")
    ...
    state.face_num = ply_set_read_cb(ply_file, "face", "vertex_indices", ReadFaceCallBack, &state, 0);
    if (state.face_num == 0) {
        state.face_num = ply_set_read_cb(ply_file, "face", "vertex_index", ReadFaceCallBack, &state, 0);
    }
...
bool WriteTriangleMeshToPLY(const std::string &filename,
    ...
    if (write_triangle_uvs && mesh.HasTriangleUvs()) {
        LOGW("This file format currently does not support writing textures "
             "and uv coordinates. Consider using .obj")
    }

    if (mesh.IsEmpty()) {
        LOGW("Write PLY failed: mesh has 0 vertices.")
        return false;
    }

    p_ply ply_file = ply_create(filename.c_str(), write_ascii ? PLY_ASCII : PLY_LITTLE_ENDIAN, nullptr, 0, nullptr);
    if (!ply_file) {
        LOGW("Write PLY failed: unable to open file: {}", filename)
        return false;
    }
    ...
    ply_add_comment(ply_file, "Created by Open3D");
    ply_add_element(ply_file, "vertex", static_cast<long>(mesh.vertices_.size()));
    ply_add_property(ply_file, "x", PLY_DOUBLE, PLY_DOUBLE, PLY_DOUBLE);
    ply_add_property(ply_file, "y", PLY_DOUBLE, PLY_DOUBLE, PLY_DOUBLE);
    ply_add_property(ply_file, "z", PLY_DOUBLE, PLY_DOUBLE, PLY_DOUBLE);
    ...
    ply_add_element(ply_file, "face", static_cast<long>(mesh.triangles_.size()));
    ...
    utility::ProgressBar progress_bar(static_cast<size_t>(mesh.vertices_.size() + mesh.triangles_.size()),
...
bool ReadLineSetFromPLY(const std::string &filename, geometry::LineSet &lineset, bool print_progress) {
    ...
    state.line_num = ply_set_read_cb(ply_file, "edge", "vertex1", ReadLineCallback, &state, 0);
    ply_set_read_cb(ply_file, "edge", "vertex2", ReadLineCallback, &state, 1);

    state.color_num = ply_set_read_cb(ply_file, "edge", "red", ReadColorCallback, &state, 0);
    ...
    if (state.line_num <= 0) {
        LOGW("Read PLY failed: number of edges <= 0.")
        ply_close(ply_file);
        return false;
    }
    ...
    utility::ProgressBar progress_bar(state.vertex_num + state.line_num + state.color_num,
...
bool WriteLineSetToPLY(const std::string &filename,
                       ...
                       bool print_progress) {
    if (lineset.IsEmpty()) {
        LOGW("Write PLY failed: line set has 0 points.")
        return false;
    }
    if (!lineset.HasLines()) {
        LOGW("Write PLY failed: line set has 0 lines.")
        return false;
    }
    ...
    ply_add_comment(ply_file, "Created by Open3D");
    ply_add_element(ply_file, "vertex", static_cast<long>(lineset.points_.size()));
    ply_add_property(ply_file, "x", PLY_DOUBLE, PLY_DOUBLE, PLY_DOUBLE);
    ply_add_property(ply_file, "y", PLY_DOUBLE, PLY_DOUBLE, PLY_DOUBLE);
    ply_add_property(ply_file, "z", PLY_DOUBLE, PLY_DOUBLE, PLY_DOUBLE);
    ...
    utility::ProgressBar progress_bar(static_cast<size_t>(lineset.points_.size() + lineset.lines_.size()),
                                      "Writing PLY: ", print_progress);

    for (const auto &point : lineset.points_) {
        ply_write(ply_file, point(0));
        ply_write(ply_file, point(1));
        ply_write(ply_file, point(2));
        ++progress_bar;
...
bool ReadVoxelGridFromPLY(const std::string &filename, geometry::VoxelGrid &voxelgrid, bool print_progress) {
    ...
    state.voxel_num = ply_set_read_cb(ply_file, "vertex", "x", ReadVoxelCallback, &state, 0);
    ...
    utility::ProgressBar progress_bar(state.voxel_num + state.color_num, "Reading PLY: ", print_progress);
    state.progress_bar = &progress_bar;
...
bool WriteVoxelGridToPLY(const std::string &filename,
    ...
    if (voxelgrid.IsEmpty()) {
        LOGW("Write PLY failed: voxelgrid has 0 voxels.")
        return false;
    }
    ...
    ply_add_element(ply_file, "vertex", static_cast<long>(voxelgrid.voxels_.size()));
    ...
    utility::ProgressBar progress_bar(static_cast<size_t>(voxelgrid.voxels_.size()), "Writing PLY: ", print_progress);
