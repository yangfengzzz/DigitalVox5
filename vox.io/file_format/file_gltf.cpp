...
struct IntArray : public IntArrayBase {
    ...
    [[nodiscard]] size_t size() const override { return adapter.elem_count; }
};
...
bool ReadTriangleMeshFromGLTF(const std::string& filename,
    ...
    if (!warn.empty() || !err.empty()) {
        LOGW("Read GLTF failed: unable to open file {}", filename)
    }
    if (!ret) {
        return false;
    }
    ...
    if (model.meshes.size() > 1) {
        LOGI("The file contains more than one mesh. All meshes will be "
        ...
        if (gltf_node.mesh != -1) {
            ...
            for (const tinygltf::Primitive& primitive : gltf_mesh.primitives) {
                for (const auto& attribute : primitive.attributes) {
                    if (attribute.first == "POSITION") {
                        ...
                        const auto* positions = reinterpret_cast<const float*>(
                                &positions_buffer.data[positions_view.byteOffset + positions_accessor.byteOffset]);

                        for (size_t i = 0; i < positions_accessor.count; ++i) {
                            mesh_temp.vertices_.emplace_back(positions[i * 3 + 0], positions[i * 3 + 1],
                                                             positions[i * 3 + 2]);
                        }
                    }
                    ...
                    if (attribute.first == "NORMAL") {
                        ...
                        const auto* normals = reinterpret_cast<const float*>(
                                &normals_buffer.data[normals_view.byteOffset + normals_accessor.byteOffset]);

                        for (size_t i = 0; i < normals_accessor.count; ++i) {
                            mesh_temp.vertex_normals_.emplace_back(normals[i * 3 + 0], normals[i * 3 + 1],
                                                                   normals[i * 3 + 2]);
                        }
                    }
                    ...
                    if (attribute.first == "COLOR_0") {
                        ...
                        if (byte_stride == 0) {
                            ...
                            byte_stride = colors_accessor.type *
                        ...
                        switch (colors_accessor.componentType) {
                            case TINYGLTF_COMPONENT_TYPE_FLOAT: {
                                for (size_t i = 0; i < colors_accessor.count; ++i) {
                                    const auto* colors = reinterpret_cast<const float*>(
                                            colors_buffer.data.data() + colors_view.byteOffset + i * byte_stride);
                                    mesh_temp.vertex_colors_.emplace_back(colors[0], colors[1], colors[2]);
                                }
                                break;
                            }
                            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE: {
                                auto max_val = (double)std::numeric_limits<uint8_t>::max();
                                for (size_t i = 0; i < colors_accessor.count; ++i) {
                                    const auto* colors = reinterpret_cast<const uint8_t*>(
                                            colors_buffer.data.data() + colors_view.byteOffset + i * byte_stride);
                                    mesh_temp.vertex_colors_.emplace_back(colors[0] / max_val, colors[1] / max_val,
                                                                          colors[2] / max_val);
                                }
                                break;
                            }
                            case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT: {
                                auto max_val = (double)std::numeric_limits<uint16_t>::max();
                                for (size_t i = 0; i < colors_accessor.count; ++i) {
                            ...
                            default: {
                                LOGW("Unrecognized component type for "
                                     "vertex colors")
                                break;
                            }
                        }
                ...
                std::unique_ptr<IntArrayBase> indices_array_pointer = nullptr;
                {
                    ...
                    const auto data_address =
                            indices_buffer.data.data() + indices_view.byteOffset + indices_accessor.byteOffset;
                    ...
                    switch (indices_accessor.componentType) {
                        case TINYGLTF_COMPONENT_TYPE_BYTE:
                            indices_array_pointer = std::make_unique<IntArray<char>>(
                                    ArrayAdapter<char>(data_address, count, byte_stride));
                            break;
                        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
                            indices_array_pointer = std::make_unique<IntArray<unsigned char>>(
                                    ArrayAdapter<unsigned char>(data_address, count, byte_stride));
                            break;
                        case TINYGLTF_COMPONENT_TYPE_SHORT:
                            indices_array_pointer = std::make_unique<IntArray<short>>(
                                    ArrayAdapter<short>(data_address, count, byte_stride));
                            break;
                        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
                            indices_array_pointer = std::make_unique<IntArray<unsigned short>>(
                                    ArrayAdapter<unsigned short>(data_address, count, byte_stride));
                            break;
                        case TINYGLTF_COMPONENT_TYPE_INT:
                            indices_array_pointer = std::make_unique<IntArray<int>>(
                                    ArrayAdapter<int>(data_address, count, byte_stride));
                            break;
                        case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT:
                            indices_array_pointer = std::make_unique<IntArray<unsigned int>>(
                                    ArrayAdapter<unsigned int>(data_address, count, byte_stride));
                            break;
                        default:
                            break;
                    }
                    ...
                    switch (primitive.mode) {
                        case TINYGLTF_MODE_TRIANGLES:
                            for (size_t i = 0; i < indices_accessor.count; i += 3) {
                                mesh_temp.triangles_.emplace_back(indices[i], indices[i + 1], indices[i + 2]);
                            }
                            break;
                        case TINYGLTF_MODE_TRIANGLE_STRIP:
                            for (size_t i = 2; i < indices_accessor.count; ++i) {
                                mesh_temp.triangles_.emplace_back(indices[i - 2], indices[i - 1], indices[i]);
                            }
                            break;
                        case TINYGLTF_MODE_TRIANGLE_FAN:
                            for (size_t i = 2; i < indices_accessor.count; ++i) {
                                mesh_temp.triangles_.emplace_back(indices[0], indices[i - 1], indices[i]);
                            }
                            break;
                    }
                }
            ...
            if (!gltf_node.matrix.empty()) {
                ...
                if (!gltf_node.rotation.empty()) {
                    ...
                    // qy, qz.
                    transform.topLeftCorner<3, 3>() = Eigen::Quaterniond(gltf_node.rotation[3], gltf_node.rotation[0],
                                                                         gltf_node.rotation[1], gltf_node.rotation[2])
                                                              .toRotationMatrix();
                    mesh_temp.Transform(transform);
                }
                if (!gltf_node.translation.empty()) {
                    mesh_temp.Translate(Eigen::Vector3d(gltf_node.translation[0], gltf_node.translation[1],
...
bool WriteTriangleMeshToGLTF(const std::string& filename,
    ...
    indices_accessor.componentType =
            save_indices_as_uint32 ? TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT : TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT;
