...
std::unique_ptr<RenderContext> Platform::CreateRenderContext(Device &device, VkSurfaceKHR surface,
...
void Platform::OnPostDraw(RenderContext &context) {
    HOOK(Hook::POST_DRAW, OnPostDraw(context))
}

void Platform::OnAppError(const std::string &app_id) {
    HOOK(Hook::ON_APP_ERROR, OnAppError(app_id))
}

void Platform::OnUpdate(float delta_time) {
    HOOK(Hook::ON_UPDATE, OnUpdate(delta_time))
}

void Platform::OnAppStart(const std::string &app_id) {
    HOOK(Hook::ON_APP_START, OnAppStart(app_id))
}

void Platform::OnAppClose(const std::string &app_id) {
    HOOK(Hook::ON_APP_CLOSE, OnAppClose(app_id))
}

void Platform::OnPlatformClose() {
    HOOK(Hook::ON_PLATFORM_CLOSE, OnPlatformClose())
}
