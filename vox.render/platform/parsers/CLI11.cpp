...
bool Cli11CommandContext::HasGroupName() const {
    return !state_.group_name.empty();
}

const std::string &Cli11CommandContext::GetGroupName() const {
    return state_.group_name;
}

CLI11CommandContextState Cli11CommandContext::GetState() const {
    return state_;
}
...
void Cli11CommandParser::Parse(Cli11CommandContext *context, FlagCommand *command) {
    ...
    switch (command->GetFlagType()) {
        case FlagType::FLAG_ONLY:flag = context->cli_11_->add_flag(command->GetName(), command->GetHelpLine());
            break;
        case FlagType::ONE_VALUE:
        case FlagType::MANY_VALUES:flag = context->cli_11_->add_option(command->GetName(), command->GetHelpLine());
            break;
    }
