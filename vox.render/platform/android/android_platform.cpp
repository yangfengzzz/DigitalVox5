...
Java_com_khronos_vulkan_1samples_SampleLauncherActivity_initFilePath(JNIEnv * env , jobject thiz, jstring
                                                                     external_dir , jstring temp_dir )
{
...
JNICALL
Java_com_khronos_vulkan_1samples_SampleLauncherActivity_getSamples(JNIEnv * env, jobject
                                                                   thiz)
{
    auto sample_list = apps::get_samples();
    
    jclass c = env->FindClass("com/khronos/vulkan_samples/model/Sample");
    jmethodID constructor = env->GetMethodID(c,
                                             "<init>",
         ...
         sample_index++)
    {
        ...
        {
            env->
            SetObjectArrayElement(j_tag_list, tag_index, env
                                  ->
                                  NewStringUTF(sample_info
                                               ->tags[tag_index].
        ...
        env->
        SetObjectArrayElement(j_sample_list, sample_index, env
                              ->
                              NewObject(c, constructor, id, category, author, name, desc, j_tag_list
                                        ));
    }
...
JNIEXPORT void JNICALL
Java_com_khronos_vulkan_1samples_SampleLauncherActivity_sendArgumentsToPlatform(JNIEnv
                                                                                *env,
                                                                                ...
                                                                                arg_strings)
{
         ...
         i++)
    {
        ...
        env->
        ReleaseStringUTFChars(arg_string, arg
                              );
    }
...
void AndroidPlatform::terminate(ExitCode code) {
    switch (code) {
        ...
        case ExitCode::Close:log_output.clear();
            break;
        case ExitCode::FatalError:send_notification(log_output);
            break;
        default:break;
    }
...
std::unique_ptr<RenderContext> AndroidPlatform::create_render_context(Device &device, VkSurfaceKHR surface,
