...
SpringJoint::SpringJoint(Collider *collider_0, Collider *collider_1) : Joint() {
    ...
    native_joint_ = PxDistanceJointCreate(*PhysicsManager::native_physics_(),
                                          actor_0, PxTransform(PxVec3(), PxQuat(0, 0, 0, 1)),
                                          actor_1, PxTransform(PxVec3(), PxQuat(0, 0, 0, 1)));
}

float SpringJoint::distance() const {
    return static_cast<PxDistanceJoint *>(native_joint_)->getDistance();
}
...
float SpringJoint::min_distance() const {
    return static_cast<PxDistanceJoint *>(native_joint_)->getMinDistance();
}
...
float SpringJoint::max_distance() const {
    return static_cast<PxDistanceJoint *>(native_joint_)->getMaxDistance();
}
...
float SpringJoint::tolerance() const {
    return static_cast<PxDistanceJoint *>(native_joint_)->getTolerance();
}
...
float SpringJoint::stiffness() const {
    return static_cast<PxDistanceJoint *>(native_joint_)->getStiffness();
}

void SpringJoint::set_damping(float damping) {
    static_cast<PxDistanceJoint *>(native_joint_)->setDamping(damping);
}

float SpringJoint::damping() const {
    return static_cast<PxDistanceJoint *>(native_joint_)->getDamping();
}
