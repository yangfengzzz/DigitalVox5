...
VkResult
CommandBuffer::begin(VkCommandBufferUsageFlags flags, const RenderPass *render_pass, const Framebuffer *framebuffer,
...
void CommandBuffer::bind_vertex_buffers(uint32_t first_binding,
    ...
    vkCmdBindVertexBuffers(get_handle(), first_binding, to_u32(buffer_handles.size()), buffer_handles.data(),
                           offsets.data());
}
...
void CommandBuffer::draw_indexed_indirect(const core::Buffer &buffer, VkDeviceSize offset, uint32_t draw_count,
                                          uint32_t stride) {
    flush(VK_PIPELINE_BIND_POINT_GRAPHICS);
...
void
CommandBuffer::update_buffer(const core::Buffer &buffer, VkDeviceSize offset, const std::vector<uint8_t> &data) {
    vkCmdUpdateBuffer(get_handle(), buffer.get_handle(), offset, data.size(), data.data());
}

void CommandBuffer::blit_image(const core::Image &src_img, const core::Image &dst_img,
...
void CommandBuffer::resolve_image(const core::Image &src_img, const core::Image &dst_img,
...
void CommandBuffer::copy_image(const core::Image &src_img, const core::Image &dst_img,
...
void CommandBuffer::copy_image_to_buffer(const core::Image &image, VkImageLayout image_layout,
...
void CommandBuffer::image_memory_barrier(const core::ImageView &image_view,
...
RenderPass &CommandBuffer::get_render_pass(const vox::RenderTarget &render_target,
                                           const std::vector<LoadStoreInfo> &load_store_infos,
                                           const std::vector<std::unique_ptr<Subpass>> &subpasses) {
    // Create render pass
