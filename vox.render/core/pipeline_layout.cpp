...
shader_modules_{shader_modules} {
    ...
    for (auto &shader_set_it : shader_sets_) {
        descriptor_set_layouts_.emplace_back(
                                             &device.get_resource_cache().request_descriptor_set_layout(shader_set_it.first, shader_modules,
    ...
    for (auto &push_constant_resource : get_resources(ShaderResourceType::PUSH_CONSTANT)) {
    ...
    auto result = vkCreatePipelineLayout(device.get_handle(), &create_info, nullptr, &handle_);
...
PipelineLayout::~PipelineLayout() {
    ...
    if (handle_ != VK_NULL_HANDLE) {
        vkDestroyPipelineLayout(device_.get_handle(), handle_, nullptr);
    }
}
