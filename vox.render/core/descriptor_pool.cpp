...
void DescriptorPool::reset() {
    ...
    for (auto pool : pools_) {
        vkResetDescriptorPool(device_.get_handle(), pool, 0);
    }
...
VkResult DescriptorPool::free(VkDescriptorSet descriptor_set) {
    ...
    vkFreeDescriptorSets(device_.get_handle(), pools_[desc_pool_index], 1, &descriptor_set);
...
std::uint32_t DescriptorPool::find_available_pool(std::uint32_t search_index) {
    ...
    if (pools_.size() <= search_index) {
        ...
        create_info.poolSizeCount = to_u32(pool_sizes_.size());
