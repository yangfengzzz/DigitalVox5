...
get_depth_resolve_reference(const VkSubpassDescription &subpass_description) {
...
inline VkResult
create_vk_renderpass(VkDevice device, VkRenderPassCreateInfo &create_info, VkRenderPass *handle) {
    return vkCreateRenderPass(device, &create_info, nullptr, handle);
}

inline VkResult
create_vk_renderpass(VkDevice device, VkRenderPassCreateInfo2KHR &create_info, VkRenderPass *handle) {
...
std::vector<T> get_attachment_descriptions(const std::vector<Attachment> &attachments,
    ...
    for (size_t i = 0U; i < attachments.size(); ++i) {
        ...
        attachment.finalLayout = is_depth_stencil_format(attachment.format)
...
void RenderPass::create_renderpass(const std::vector<Attachment> &attachments,
    ...
    auto attachment_descriptions = get_attachment_descriptions<T_AttachmentDescription>(attachments,
                                                                                        load_store_infos);
    ...
    for (size_t i = 0; i < subpasses.size(); ++i) {
        ...
        for (auto i_attachment: subpass.input_attachments) {
            ...
            auto initial_layout =
            attachments[i_attachment].initial_layout == VK_IMAGE_LAYOUT_UNDEFINED ? default_layout
        ...
        if (!subpass.disable_depth_stencil_attachment) {
            ...
            auto it = find_if(attachments.begin(), attachments.end(), [](const Attachment attachment) {
                return is_depth_stencil_format(attachment.format);
            });
            if (it != attachments.end()) {
                ...
                if (subpass.depth_stencil_resolve_mode != VK_RESOLVE_MODE_NONE) {
                    ...
                    initial_layout =
                    attachments[i_depth_stencil_resolve].initial_layout == VK_IMAGE_LAYOUT_UNDEFINED
                    ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
                    : attachments[i_depth_stencil_resolve].initial_layout;
                    depth_resolve_attachments[i].push_back(
                                                           get_attachment_reference<T_AttachmentReference>(i_depth_stencil_resolve,
                                                                                                           initial_layout));
                }
            }
        }
    ...
    for (size_t i = 0; i < subpasses.size(); ++i) {
        ...
        subpass_description.pInputAttachments = input_attachments[i].empty() ? nullptr
        : input_attachments[i].data();
        subpass_description.inputAttachmentCount = to_u32(input_attachments[i].size());
        
        subpass_description.pColorAttachments = color_attachments[i].empty() ? nullptr
        : color_attachments[i].data();
