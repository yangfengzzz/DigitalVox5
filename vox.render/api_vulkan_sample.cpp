...
bool ApiVulkanSample::prepare(vox::Platform &platform) {
    ...
    depth_format = vox::get_suitable_depth_format(device->get_gpu().get_handle());
    ...
    VK_CHECK(
             vkCreateSemaphore(device->get_handle(), &semaphore_create_info, nullptr,
                               &semaphores.acquired_image_ready));
    ...
    gui = std::make_unique<vox::Gui>(*this, platform.get_window(), /*stats=*/nullptr, 15.0f, true);
...
bool ApiVulkanSample::resize(const uint32_t _width, const uint32_t _height) {
    ...
    if (width == get_render_context().get_surface_extent().width &&
        height == get_render_context().get_surface_extent().height) {
        return false;
    }
        ...
        if (input_event.get_source() == vox::EventSource::Mouse) {
            ...
            if (touch_event.get_action() == vox::TouchAction::Down) {
                ...
                if (!handled) {
                    ...
                    camera.rotate(Vector3F(deltaX, 0.0f, 0.0f));
                    camera.rotate(Vector3F(0.0f, -deltaY, 0.0f));
...
void ApiVulkanSample::handle_mouse_move(int32_t x, int32_t y) {
    ...
    if (handled) {
        mouse_pos = Vector2F((float) x, (float) y);
        return;
    }
    ...
    if (mouse_buttons.left) {
        ...
        camera.rotate(Vector3F(dy * camera.rotation_speed, -dx * camera.rotation_speed, 0.0f));
        view_updated = true;
    }
    if (mouse_buttons.right) {
        zoom += dy * .005f * zoom_speed;
        camera.translate(Vector3F(-0.0f, 0.0f, dy * .005f * zoom_speed));
        view_updated = true;
    }
    if (mouse_buttons.middle) {
        camera_pos.x -= dx * 0.01f;
        camera_pos.y -= dy * 0.01f;
        camera.translate(Vector3F(-dx * 0.01f, -dy * 0.01f, 0.0f));
        view_updated = true;
    }
    mouse_pos = Vector2F((float) x, (float) y);
}
...
void ApiVulkanSample::destroy_command_buffers() {
    vkFreeCommandBuffers(device->get_handle(), cmd_pool, static_cast<uint32_t>(draw_cmd_buffers.size()),
...
void ApiVulkanSample::update_overlay(float delta_time) {
    if (gui) {
        gui->show_simple_window(get_name(), vox::to_u32(1.0f / delta_time), [this]() {
...
void ApiVulkanSample::draw_ui(const VkCommandBuffer command_buffer) {
    if (gui) {
        const VkViewport viewport = vox::initializers::viewport(static_cast<float>(width),
                                                                static_cast<float>(height),
                                                                0.0f, 1.0f);
        const VkRect2D scissor = vox::initializers::rect2D(width, height, 0, 0);
...
void ApiVulkanSample::prepare_frame() {
    if (render_context->has_swapchain()) {
        ...
        VkResult result = render_context->get_swapchain().acquire_next_image(current_buffer,
                                                                             semaphores.acquired_image_ready,
                                                                             VK_NULL_HANDLE);
...
void ApiVulkanSample::create_command_pool() {
    ...
    command_pool_info.queueFamilyIndex = device->get_queue_by_flags(VK_QUEUE_GRAPHICS_BIT | VK_QUEUE_COMPUTE_BIT,
                                                                    0).get_family_index();
...
void ApiVulkanSample::setup_depth_stencil() {
    ...
    memory_allocation.memoryTypeIndex = device->get_memory_type(memReqs.memoryTypeBits,
...
void ApiVulkanSample::create_swapchain_buffers() {
    if (render_context->has_swapchain()) {
        ...
        for (uint32_t i = 0; i < images.size(); i++) {
            ...
            VK_CHECK(vkCreateImageView(device->get_handle(), &color_attachment_view, nullptr,
...
Texture ApiVulkanSample::load_texture(const std::string &file) {
    ...
    vox::core::Buffer stage_buffer{*device,
