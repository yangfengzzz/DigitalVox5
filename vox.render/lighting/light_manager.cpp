...
void LightManager::_updateShaderData(ShaderData &shaderData) {
    ...
    for (size_t i = 0; i < pointLightCount; i++) {
		_pointLights[i]->update_shader_data(_pointLightDatas[i]);
    }
    
    for (size_t i = 0; i < spotLightCount; i++) {
		_spotLights[i]->update_shader_data(_spotLightDatas[i]);
    }
    
    for (size_t i = 0; i < directLightCount; i++) {
		_directLights[i]->update_shader_data(_directLightDatas[i]);
    }
