...
//            if (std::find(model.extensionsRequired.begin(), model.extensionsRequired.end(), used_extension) !=
...
//
//    LOGI("Time spent loading images: {} seconds across {} threads.", vox::to_string(elapsed_time), thread_count);
//
//    // Load textures
...
//    std::vector<vox::sg::Texture *> textures;
//    if (has_textures) {
...
//
//    if (!scene.has_component<vox::sg::Light>()) {
//        // Add a default light if none are present
//        vox::add_directional_light(scene, glm::quat({glm::radians(-90.0f), 0.0f, glm::radians(30.0f)}));
//    }
//
//    return scene;
//}
...
std::unique_ptr<sg::Renderer> GLTFLoader::load_model(uint32_t index) {
    ...
    pos = reinterpret_cast<const float *>(&(model.buffers[buffer_view.buffer].data[accessor.byteOffset +
    ...
    if (gltf_primitive.indices >= 0) {
        submesh->vertex_indices_ = to_u32(get_attribute_size(&model, gltf_primitive.indices));
        ...
        submesh->index_buffer_ = std::make_unique<core::Buffer>(device,
                                                                index_data.size(),
...
//                material->alpha_mode = vox::sg::AlphaMode::Blend;
//            } else if (gltf_value.second.string_value == "OPAQUE") {
//                material->alpha_mode = vox::sg::AlphaMode::Opaque;
//            } else if (gltf_value.second.string_value == "MASK") {
//                material->alpha_mode = vox::sg::AlphaMode::Mask;
//            }
...
//                    }
//
//                    properties.inner_cone_angle = static_cast<float>(khr_light.Get("spot").Get(
//                                                                                               "innerConeAngle").Get<double>());
//
//                    if (khr_light.Get("spot").Has("outerConeAngle")) {
//                        properties.outer_cone_angle = static_cast<float>(khr_light.Get("spot").Get(
//                                                                                                   "outerConeAngle").Get<double>());
//                    } else {
...
tinygltf::Value *
GLTFLoader::get_extension(tinygltf::ExtensionMap &tinygltf_extensions, const std::string &extension) {
