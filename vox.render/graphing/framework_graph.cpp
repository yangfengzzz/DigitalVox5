...
bool generate(RenderContext &context) {
	...
	while (it_graphics_pipelines != resource_cache_state.graphics_pipelines.end()) {
		...
		size_t specialization_constant_state =
			specialization_constant_state_node(graph, it_graphics_pipelines->second.get_state().get_specialization_constant_state());
		...
		size_t
			input_assembly_state = input_assembly_state_node(graph, it_graphics_pipelines->second.get_state().get_input_assembly_state());
...
size_t image_node(Graph &graph, const core::Image &image) {
	...
	nlohmann::json data = {{"VkExtent2D", {{"width", image.get_extent().width}, {"height", image.get_extent().height}}},
		...
		{"VkSubresource", {{"VkImageAspectFlags", image_aspect_to_string(subresource.aspectMask)}, {"mip_level", subresource.mipLevel},
...
size_t depth_stencil_state_node(Graph &graph, const DepthStencilState &depth_stencil_state) {
	nlohmann::json data = {
		...
		{"front", {{"fail_op", to_string(depth_stencil_state.front.fail_op)}, {"pass_op", to_string(depth_stencil_state.front.pass_op)},
			{"depth_fail_op", to_string(depth_stencil_state.front.depth_fail_op)},
			{"compare_op", to_string(depth_stencil_state.front.compare_op)}}},
		{"back", {{"fail_op", to_string(depth_stencil_state.back.fail_op)}, {"pass_op", to_string(depth_stencil_state.back.pass_op)},
			{"depth_fail_op", to_string(depth_stencil_state.back.depth_fail_op)},
			{"compare_op", to_string(depth_stencil_state.back.compare_op)}}}};
