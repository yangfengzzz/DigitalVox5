...
RenderFrame::GetCommandPools(const Queue &queue, CommandBuffer::ResetMode reset_mode) {
...
VkSemaphore RenderFrame::RequestSemaphoreWithOwnership() {
    return semaphore_pool_.RequestSemaphoreWithOwnership();
}
...
CommandBuffer &RenderFrame::RequestCommandBuffer(const Queue &queue, CommandBuffer::ResetMode reset_mode,
...
BufferAllocation
RenderFrame::AllocateBuffer(VkBufferUsageFlags usage, VkDeviceSize size, size_t thread_index) {
    assert(thread_index < thread_count_ && "Thread index is out of bounds");
