...
//MARK: - Transition
void PostProcessingComputePass::TransitionImages(CommandBuffer &command_buffer, RenderTarget &default_render_target,
    ...
    for (const auto &data : data_) {
        for (const auto &storage : data->StorageTextures()) {
            if (const uint32_t *attachment = storage.second->GetTargetAttachment()) {
                ...
                auto resource = std::find_if(pipeline_layout.GetResources().begin(), pipeline_layout.GetResources().end(),
