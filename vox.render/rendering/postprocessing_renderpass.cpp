...
void PostProcessingRenderPass::transition_attachments(
    ...
    for (uint32_t output : output_attachments) {
        ...
        vox::is_depth_only_format(attachment_format) || vox::is_depth_stencil_format(attachment_format);
        const VkImageLayout output_layout = is_depth_stencil ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL :
...
void PostProcessingRenderPass::draw(CommandBuffer &command_buffer, RenderTarget &default_render_target) {
    ...
    if (!uniform_data.empty()) {
        ...
        uniform_buffer_alloc = std::make_shared<BufferAllocation>(render_frame.allocate_buffer(
