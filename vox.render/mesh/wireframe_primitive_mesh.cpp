...
ModelMeshPtr WireframePrimitiveMesh::create_sphere_wire_frame(Device &device, float radius) {
    ...
    // X
    create_circle_wire_frame(radius, 0, vertex_count, 0, shift, positions, indices);
    
    // Y
    create_circle_wire_frame(radius, vertex_count, vertex_count, 1, shift, positions, indices);
    
    // Z
    create_circle_wire_frame(radius, 2 * vertex_count, vertex_count, 2, shift, positions, indices);
...
ModelMeshPtr WireframePrimitiveMesh::create_capsule_wire_frame(Device &device, float radius, float height) {
    ...
    // Y-Top
    shift.y = half_height;
    create_circle_wire_frame(radius, 0, vertex_count, 1, shift, positions, indices);
    ...
    // X-Elliptic
    create_elliptic_wire_frame(radius, half_height, vertex_count * 2, vertex_count, 2, positions, indices);
    
    // Z-Elliptic
    create_elliptic_wire_frame(radius, half_height, vertex_count * 3, vertex_count, 0, positions, indices);
...
void WireframePrimitiveMesh::create_circle_wire_frame(float radius,
    ...
    for (uint32_t i = 0; i < vertex_count; ++i) {
        ...
        switch (axis) {
            case 0:
                positions[global_index] = Vector3F(shift.x,
                                                   radius * std::cos(theta_delta) + shift.y,
            ...
            case 2:
                positions[global_index] = Vector3F(radius * std::cos(theta_delta) + shift.x,
                                                   radius * std::sin(theta_delta) + shift.y,
                                                   shift.z);
                break;
        }
...
void WireframePrimitiveMesh::create_elliptic_wire_frame(float radius,
                                                        float height,
                                                        uint32_t vertex_begin,
                                                        uint32_t vertex_count,
                                                        uint8_t axis,
    ...
    for (uint32_t i = 0; i < vertex_count; ++i) {
        ...
        switch (axis) {
            case 0:
                positions[global_index] = Vector3F(0,
                                                   radius * std::sin(theta_delta) + height,
                                                   radius * std::cos(theta_delta));
                break;
            case 1:
                ...
                break;
            case 2:
                positions[global_index] = Vector3F(radius * std::cos(theta_delta),
                                                   radius * std::sin(theta_delta) + height,
                                                   0);
                break;
        }
