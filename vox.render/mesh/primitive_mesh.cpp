...
ModelMeshPtr PrimitiveMesh::create_plane(Device &device,
    ...
    for (size_t i = 0; i < kVertexCount; ++i) {
        ...
        // Position
        positions[i] = Vector3F(static_cast<float>(kX) * kGridWidth - kHalfWidth,
                                0,
...
ModelMeshPtr PrimitiveMesh::create_torus(Device &device,
    ...
    for (size_t i = 0; i <= radial_segments; i++) {
        for (size_t j = 0; j <= tubular_segments; j++) {
            ...
            const auto
            kV = static_cast<float>(i) / static_cast<float>(radial_segments) * static_cast<float>(M_PI) * 2.f;
            ...
            const auto kPosition = Vector3F((radius + tube_radius * kCosV) * kCosU,
                                            (radius + tube_radius * kCosV) * kSinU,
                                            tube_radius * kSinV);
...
ModelMeshPtr PrimitiveMesh::create_capsule(Device &device,
    ...
    PrimitiveMesh::create_capsule_cap(radius,
                                      height,
                                      radial_segments,
                                      kDoublePi,
                                      kTorsoVertexCount,
    ...
    PrimitiveMesh::create_capsule_cap(radius,
                                      ...
                                      radial_segments,
                                      -kDoublePi,
                                      kTorsoVertexCount + kCapVertexCount,
                                      -1,
                                      positions,
                                      normals,
                                      uvs,
                                      indices,
                                      indices_offset + 6 * kCapRectangleCount
                                      );
...
void PrimitiveMesh::create_capsule_cap(float radius,
                                       float height,
                                       size_t radial_segments,
