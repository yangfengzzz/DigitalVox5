...
T &RequestResource(Device &device, ResourceRecord &recorder, std::mutex &resource_mutex,
...
std::vector<uint8_t> ResourceCache::Serialize() {
    return recorder_.GetData();
}

void ResourceCache::SetPipelineCache(VkPipelineCache pipeline_cache) {
    pipeline_cache_ = pipeline_cache;
}

ShaderModule &ResourceCache::RequestShaderModule(VkShaderStageFlagBits stage, const ShaderSource &glsl_source,
...
void ResourceCache::UpdateDescriptorSets(const std::vector<core::ImageView> &old_views,
    ...
    if (!set_updates.empty()) {
        vkUpdateDescriptorSets(device_.GetHandle(), ToU32(set_updates.size()), set_updates.data(),
                               0, nullptr);
    }
    ...
    for (auto &match : matches) {
        ...
        // Generate new key
        size_t new_key = 0U;
        HashParam(new_key, descriptor_set.GetLayout(), descriptor_set.GetBufferInfos(),
                   descriptor_set.GetImageInfos());
