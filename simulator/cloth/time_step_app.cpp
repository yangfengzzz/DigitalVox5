...
#include "simulator/cloth/time_step_app.h"
...
void TimeStepApp::InitializeCloth(Entity* entity, int index, const physx::PxVec3& offset) {
    ...
    cloth_mesh.GeneratePlaneCloth(4.f, 4.f, 1, 1, false, transform);
    cloth_mesh.AttachClothPlaneByAngles(1, 1);
    ...
    for (int i = 0; i < (int)cloth_mesh.m_vertices.size(); i++) {
        ...
        particles_copy[i] = physx::PxVec4(
                ...
                0.5f * cloth_mesh.m_inv_masses[i]);  // w component is 1/mass, or 0.0f for anchored/fixed particles
    }
    ...
    cloth_actor_[index].cloth->setGravity(physx::PxVec3(0.0f, -9.8f * 0.0f, 0.0f));
    cloth_actor_[index].cloth->setTetherConstraintStiffness(0.01);
    cloth_actor_[index].cloth->setStiffnessFrequency(10);
    cloth_actor_[index].cloth->setTetherConstraintScale(0.7);
    if (index == 0) {
        cloth_actor_[index].cloth->setSolverFrequency(60);
    } else {
        cloth_actor_[index].cloth->setSolverFrequency(300);
    }
