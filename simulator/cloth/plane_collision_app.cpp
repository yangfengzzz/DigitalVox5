...
#include "simulator/cloth/plane_collision_app.h"
...
void PlaneCollisionApp::InitializeCloth(Entity* entity, const physx::PxVec3& offset) {
    ...
    planes.emplace_back(physx::PxVec3(-0.5f, 0.4f, 0.0f).getNormalized(), -4.0f);
    planes.emplace_back(physx::PxVec3(0.0f, 0.4f, 0.5f).getNormalized(), -4.0f);
    uint32_t mask = 3;

    // assign as collision data
    nv::cloth::Range<const physx::PxVec4> planes_r(&planes[0], &planes[0] + planes.size());
    cloth_actor_.cloth->setPlanes(planes_r, planes_r);
