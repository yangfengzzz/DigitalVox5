...
#include "simulator/cloth/ccd_app.h"
...
void CCDApp::InitializeCloth(Entity* entity, const physx::PxVec3& offset) {
    offset_ = offset;
    ...
    cloth_actor_.cloth->setSelfCollisionDistance(0.07f);  // new item
    cloth_actor_.cloth->enableContinuousCollision(true);  // new item
    cloth_actor_.cloth->setSolverFrequency(240);
    ...
    cloth_actor_.cloth->setSpheres(nv::cloth::Range<physx::PxVec4>(spheres, spheres + 1), 0,
...
void CCDApp::Update(float delta_time) {
    ...
    physx::PxTransform rotation(
            physx::PxQuat(std::cosf(time) * physx::PxHalfPi - physx::PxHalfPi, physx::PxVec3(0.0f, 1.0f, 0.0f)));
    ...
    physx::PxVec4 spheres[2] = {physx::PxVec4(total_transform.transform(physx::PxVec3(0.f, 10.f, -2.f) + offset_), 2.0),
                                physx::PxVec4(total_transform.transform(physx::PxVec3(0.f, 11.f, 3.f) + offset_), 0.5)};

    // mClothActor[0]->mCloth->setSpheres(nv::cloth::Range<physx::PxVec4>(spheres, spheres + 2), 0,
    // mClothActor[0]->mCloth->getNumSpheres()); mCollisionMesh->setTransform(totalTransform);
