...
#include "simulator/cloth/wind_app.h"
...
void WindApp::InitializeCloth(Entity* entity, int index, const physx::PxVec3& offset) {
    ...
    cloth_mesh.AttachClothPlaneByAngles(69, 79);
    ...
    cloth_actor_[index].cloth->setDragCoefficient(0.4f);
    cloth_actor_[index].cloth->setLiftCoefficient(0.25f);
...
void WindApp::Update(float delta_time) {
    ...
    if (time_ > 3.7f) {
        float dvx = 6.0f * sin((1.f + sinf(time_) * 0.5f) * time_);
        float vy = 0;
        float dvz = 50.0f + 7.0f * sin((9.f + sinf(time_ + 2.0f) * 0.5f) * time_) +
                    4.0f * sin((7.f + sinf(time_ * 0.9f) * 0.5f) * time_);

        for (auto& i : cloth_actor_) {
            physx::PxVec3 wind = physx::PxVec3(dvx, vy, dvz) / 5.0f;
            i.cloth->setWindVelocity(wind);
        }
    }
