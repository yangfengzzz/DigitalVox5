...
#include "simulator/cloth/multi_solver_app.h"
...
void MultiSolverApp::InitializeCloth(Entity* entity, int index, const physx::PxVec3& offset) {
    ...
    cloth_actor_[index].cloth->setDamping(physx::PxVec3(0.1f, 0.1f, 0.1f));
    cloth_actor_[index].cloth->setFriction(0.5f);
    ...
    cloth_actor_[index].cloth->setSpheres(nv::cloth::Range<physx::PxVec4>(spheres, spheres + 2), 0,
                                          cloth_actor_[index].cloth->getNumSpheres());

    uint32_t caps[4];
    caps[0] = 0;
    caps[1] = 1;
    cloth_actor_[index].cloth->setCapsules(nv::cloth::Range<uint32_t>(caps, caps + 2), 0,
                                           cloth_actor_[index].cloth->getNumCapsules());
    ...
    cloth_actor_[index].cloth->setDragCoefficient(0.5f);
    cloth_actor_[index].cloth->setLiftCoefficient(0.6f);
