...
#include "simulator/cloth/convex_collision_app.h"
...
void ConvexCollisionApp::InitializeCloth(Entity* entity, const physx::PxVec3& offset) {
    ...
    uint32_t mask_1 = GenerateConvexPolyhedronPlanes(4, 4, physx::PxVec3(0.7f, 5.0f, -1.0f), 1.0f, &planes);
    uint32_t mask_2 = GenerateConvexPolyhedronPlanes(4, 4, physx::PxVec3(-0.7f, 5.0f, 0.0f), 1.0f, &planes);
    // assign as collision data
    nv::cloth::Range<const physx::PxVec4> planes_r(&planes[0], &planes[0] + planes.size());
    cloth_actor_.cloth->setPlanes(planes_r, 0, cloth_actor_.cloth->getNumPlanes());
    // assign convex indices
    std::vector<uint32_t> indices;
    indices.push_back(mask_1);
    indices.push_back(mask_2);
    nv::cloth::Range<uint32_t> cind(&indices[0], &indices[0] + indices.size());
    cloth_actor_.cloth->setConvexes(cind, 0, cloth_actor_.cloth->getNumConvexes());
...
void ConvexCollisionApp::LoadScene() {
