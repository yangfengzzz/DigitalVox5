...
#include "simulator/cloth/virtual_particle_app.h"
...
void VirtualParticleApp::InitializeCloth(Entity* entity, const physx::PxVec3& offset) {
    ...
    cloth_mesh.GeneratePlaneCloth(5.f, 6.f, 9, 1, false, transform);
    cloth_mesh.AttachClothPlaneByAngles(8, 1);
    ...
    auto* virtual_particle_indices = new uint32_t[cloth_mesh.m_triangles.size()][4];
    for (int i = 0; i < (int)cloth_mesh.m_triangles.size(); i++) {
        virtual_particle_indices[i][0] = cloth_mesh.m_triangles[i].a;
        virtual_particle_indices[i][1] = cloth_mesh.m_triangles[i].b;
        virtual_particle_indices[i][2] = cloth_mesh.m_triangles[i].c;
        virtual_particle_indices[i][3] = 0;
    }
    physx::PxVec3 weights[1] = {physx::PxVec3(1.0f, 1.0f, 1.0f) / 3.0f};
    cloth_actor_.cloth->setVirtualParticles(
            nv::cloth::Range<const uint32_t[4]>(virtual_particle_indices,
                                                (virtual_particle_indices + cloth_mesh.m_triangles.size())),
            nv::cloth::Range<physx::PxVec3>(weights, weights + 1));

    physx::PxVec4 spheres[2] = {physx::PxVec4(physx::PxVec3(-4.f, 10.f, 0.f) + offset_, 1.5),
                                physx::PxVec4(physx::PxVec3(4.f, 10.f, 0.f) + offset_, 1.5)};
    cloth_actor_.cloth->setSpheres(nv::cloth::Range<physx::PxVec4>(spheres, spheres + 2), 0,
                                   cloth_actor_.cloth->getNumSpheres());

    uint32_t caps[4];
    caps[0] = 0;
    caps[1] = 1;
    cloth_actor_.cloth->setCapsules(nv::cloth::Range<uint32_t>(caps, caps + 2), 0,
                                    cloth_actor_.cloth->getNumCapsules());
...
void VirtualParticleApp::Update(float delta_time) {
    ...
    time += delta_time * 0.75f;

    physx::PxTransform inv_translation(-offset_ - physx::PxVec3(0.f, 10.f, -2.f));
    physx::PxTransform rotation(
            physx::PxQuat(cosf(time) * physx::PxHalfPi - physx::PxHalfPi, physx::PxVec3(0.0f, 1.0f, 0.0f)));
    physx::PxTransform translation(offset_ + physx::PxVec3(0.f, 10.f, -2.f) +
                                   physx::PxVec3(0.0f, 0.0f, 10.0f * sinf(time)));
    physx::PxTransform total_transform = translation.transform(rotation.transform(inv_translation));

    physx::PxVec4 spheres[2] = {physx::PxVec4(total_transform.transform(physx::PxVec3(-4.f, 10.f, 0.f) + offset_), 1.5),
                                physx::PxVec4(total_transform.transform(physx::PxVec3(4.f, 10.f, 0.f) + offset_), 1.5)};

    cloth_actor_.cloth->setSpheres(nv::cloth::Range<physx::PxVec4>(spheres, spheres + 2), 0,
                                   cloth_actor_.cloth->getNumSpheres());

    // mCollisionMesh->setTransform(totalTransform);
