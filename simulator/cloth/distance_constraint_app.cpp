...
void DistanceConstraintApp::InitializeCloth(Entity* entity, const physx::PxVec3& offset) {
    ...
    fabric_ = NvClothCookFabricFromMesh(factory_, mesh_desc, physx::PxVec3(0.0f, -1.0f, 0.0f), &phase_type_info, false);
    TrackFabric(fabric_);
    ...
    nv::cloth::Range<physx::PxVec4> distance_constraints = cloth_actor_.cloth->getMotionConstraints();
    for (int i = 0; i < (int)distance_constraints.size(); i++) {
        distance_constraints[i] = physx::PxVec4(particles_copy[i].getXYZ(), 0.002f * (i % 800) * 0.002f * (i % 800));
    }
