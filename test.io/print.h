...
void Print(const Eigen::Matrix<T, M, N>& matrix, const int& tabs = 1, const char& terminator = ';') {
    int precision = 0;
    int width = 5;
...
template <class T, int M, int N>
void Print(const std::vector<Eigen::Matrix<T, M, N>>& v, const int& tabs = 1, const char& terminator = ';') {
    std::cout << std::setw(tabs * TAB_SIZE) << "{";
    std::cout << std::endl;
...
template <class T, int M, int N>
void Print(const std::vector<Eigen::Matrix<T, M, N>, Eigen::aligned_allocator<Eigen::Matrix<T, M, N>>>& v,
           const int& tabs = 1,
