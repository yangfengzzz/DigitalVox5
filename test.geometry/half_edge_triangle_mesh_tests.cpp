...
geometry::TriangleMesh get_mesh_two_triangles() {
    std::vector<Eigen::Vector3d> vertices{Eigen::Vector3d(-1, 2, 0), Eigen::Vector3d(1, 2, 0), Eigen::Vector3d(0, 0, 0),
                                          Eigen::Vector3d(2, 0, 0)};
    std::vector<Eigen::Vector3i> triangles{Eigen::Vector3i(0, 2, 1), Eigen::Vector3i(1, 2, 3)};
...
geometry::TriangleMesh get_mesh_four_triangles_disconnect() {
    std::vector<Eigen::Vector3d> vertices{
            Eigen::Vector3d(-1, 2, 0), Eigen::Vector3d(1, 2, 0),  Eigen::Vector3d(0, 0, 0),  Eigen::Vector3d(2, 0, 0),
            Eigen::Vector3d(9, 2, 0),  Eigen::Vector3d(11, 2, 0), Eigen::Vector3d(10, 0, 0), Eigen::Vector3d(12, 0, 0)};
    std::vector<Eigen::Vector3i> triangles{Eigen::Vector3i(0, 2, 1), Eigen::Vector3i(1, 2, 3), Eigen::Vector3i(4, 6, 5),
...
geometry::TriangleMesh get_mesh_two_triangles_flipped() {
    std::vector<Eigen::Vector3d> vertices{Eigen::Vector3d(-1, 2, 0), Eigen::Vector3d(1, 2, 0), Eigen::Vector3d(0, 0, 0),
                                          Eigen::Vector3d(2, 0, 0)};
    std::vector<Eigen::Vector3i> triangles{Eigen::Vector3i(0, 2, 1), Eigen::Vector3i(1, 3, 2)};
    geometry::TriangleMesh mesh;
    mesh.vertices_ = vertices;
    mesh.triangles_ = triangles;
    return mesh;
...
geometry::TriangleMesh get_mesh_two_triangles_invalid_vertex() {
    ...
    std::vector<Eigen::Vector3i> triangles{Eigen::Vector3i(0, 2, 1), Eigen::Vector3i(2, 3, 4)};
    geometry::TriangleMesh mesh;
    mesh.vertices_ = vertices;
    mesh.triangles_ = triangles;
    return mesh;
...
geometry::TriangleMesh get_mesh_hexagon() {
    std::vector<Eigen::Vector3d> vertices{
            Eigen::Vector3d(-1, 2, 0), Eigen::Vector3d(1, 2, 0),   Eigen::Vector3d(-2, 0, 0), Eigen::Vector3d(0, 0, 0),
            Eigen::Vector3d(2, 0, 0),  Eigen::Vector3d(-1, -2, 0), Eigen::Vector3d(1, -2, 0)};
    std::vector<Eigen::Vector3i> triangles{Eigen::Vector3i(0, 2, 3), Eigen::Vector3i(0, 3, 1),
...
geometry::TriangleMesh get_mesh_partial_hexagon() {
    std::vector<Eigen::Vector3d> vertices{
            Eigen::Vector3d(-1, 2, 0), Eigen::Vector3d(1, 2, 0),   Eigen::Vector3d(-2, 0, 0), Eigen::Vector3d(0, 0, 0),
            Eigen::Vector3d(2, 0, 0),  Eigen::Vector3d(-1, -2, 0), Eigen::Vector3d(1, -2, 0)};
    std::vector<Eigen::Vector3i> triangles{Eigen::Vector3i(0, 2, 3), Eigen::Vector3i(0, 3, 1), Eigen::Vector3i(1, 3, 4),
                                           Eigen::Vector3i(2, 5, 3), Eigen::Vector3i(3, 5, 6)};
    geometry::TriangleMesh mesh;
    mesh.vertices_ = vertices;
    mesh.triangles_ = triangles;
    return mesh;
...
void assert_ordreded_neighbor(const std::shared_ptr<geometry::HalfEdgeTriangleMesh>& het_mesh,
                              int vertex_index,
                              const std::vector<int>& expected_ordered_neighbors,
                              bool allow_rotation = false) {
    std::vector<int> actual_ordered_neighbors;
    for (int half_edge_index : het_mesh->ordered_half_edge_from_vertex_[vertex_index]) {
    ...
    if (allow_rotation) {
        ...
        auto find_it = std::find(actual_ordered_neighbors.begin(), actual_ordered_neighbors.end(),
                                 expected_ordered_neighbors[0]);
        if (find_it == actual_ordered_neighbors.end()) {
            FAIL();
        } else {
            size_t offset = find_it - actual_ordered_neighbors.begin();
            std::rotate(actual_ordered_neighbors.begin(), actual_ordered_neighbors.begin() + offset,
...
void assert_vector_eq(const std::vector<int>& actual, const std::vector<int>& expect, bool allow_rotation = false) {
    std::vector<int> actual_copied = actual;
    if (allow_rotation) {
        // E.g. Actual 0, 1, 2, 3, 4
        //      Expect 2, 3, 4, 0, 1
        // Then left-rotate actual by 2
        auto find_it = std::find(actual_copied.begin(), actual_copied.end(), expect[0]);
        if (find_it == actual_copied.end()) {
            FAIL();
        } else {
            size_t offset = find_it - actual_copied.begin();
            std::rotate(actual_copied.begin(), actual_copied.begin() + offset, actual_copied.end());
        }
    }
    EXPECT_EQ(actual_copied, expect);
}

void assert_ordreded_edges(const std::shared_ptr<geometry::HalfEdgeTriangleMesh>& het_mesh,
    ...
    for (int half_edge_index : half_edge_indices) {
        ...
        actual_half_edge_vertices.push_back(std::vector<int>({he.vertex_indices_[0], he.vertex_indices_[1]}));
    }
...
void assert_same_vertices_and_triangles(const geometry::TriangleMesh& mesh,
...
TEST(HalfEdgeTriangleMesh, Constructor_TwoTriangles) {
    ...
    auto het_mesh = geometry::HalfEdgeTriangleMesh::CreateFromTriangleMesh(mesh);
...
TEST(HalfEdgeTriangleMesh, BoundaryHalfEdgesFromVertex_TwoTriangles) {
    ...
    assert_ordreded_edges(het_mesh, het_mesh->BoundaryHalfEdgesFromVertex(0), {{0, 2}, {2, 3}, {3, 1}, {1, 0}});
    assert_ordreded_edges(het_mesh, het_mesh->BoundaryHalfEdgesFromVertex(1), {{1, 0}, {0, 2}, {2, 3}, {3, 1}});
    assert_ordreded_edges(het_mesh, het_mesh->BoundaryHalfEdgesFromVertex(2), {{2, 3}, {3, 1}, {1, 0}, {0, 2}});
    assert_ordreded_edges(het_mesh, het_mesh->BoundaryHalfEdgesFromVertex(3), {{3, 1}, {1, 0}, {0, 2}, {2, 3}});
}
