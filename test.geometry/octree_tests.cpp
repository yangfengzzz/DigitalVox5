...
TEST(Octree, ZeroDepth) {
    ...
    if (auto leaf_node = std::dynamic_pointer_cast<geometry::OctreeColorLeafNode>(octree.root_node_)) {
...
TEST(Octree, ZeroDepthOutOfBound) {
    ...
    octree.InsertPoint(point_out, geometry::OctreeColorLeafNode::GetInitFunction(),
    ...
    octree.InsertPoint(point_in, geometry::OctreeColorLeafNode::GetInitFunction(),
...
TEST(Octree, ZeroDepthValueOverwrite) {
    ...
    octree.InsertPoint(point_old, geometry::OctreeColorLeafNode::GetInitFunction(),
    ...
    octree.InsertPoint(point_new, geometry::OctreeColorLeafNode::GetInitFunction(),
...
TEST(Octree, EightCubes) {
    ...
    std::vector<Eigen::Vector3d> points{
            Eigen::Vector3d(0.5, 0.5, 0.5), Eigen::Vector3d(1.5, 0.5, 0.5), Eigen::Vector3d(0.5, 1.5, 0.5),
            Eigen::Vector3d(1.5, 1.5, 0.5), Eigen::Vector3d(0.5, 0.5, 1.5), Eigen::Vector3d(1.5, 0.5, 1.5),
            Eigen::Vector3d(0.5, 1.5, 1.5), Eigen::Vector3d(1.5, 1.5, 1.5),
    };
    std::vector<Eigen::Vector3d> colors{
            Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d(0.1, 0.0, 0.0), Eigen::Vector3d(0.0, 0.1, 0.0),
            Eigen::Vector3d(0.1, 0.1, 0.0), Eigen::Vector3d(0.0, 0.0, 0.1), Eigen::Vector3d(0.1, 0.0, 0.1),
    ...
    // Check node values
    if (auto root_node = std::dynamic_pointer_cast<geometry::OctreeInternalNode>(octree.root_node_)) {
        for (size_t i = 0; i < 8; ++i) {
            if (auto leaf_node = std::dynamic_pointer_cast<geometry::OctreeColorLeafNode>(root_node->children_[i])) {
...
TEST(Octree, EightCubesLeafPointIndices) {
    ...
    geometry::Octree octree(1, Eigen::Vector3d(0, 0, 0), 2);
    for (size_t i = 0; i < points.size(); ++i) {
        octree.InsertPoint(points[i], geometry::OctreePointColorLeafNode::GetInitFunction(),
                           geometry::OctreePointColorLeafNode::GetUpdateFunction(i, colors[i]));
    }
    ...
    if (auto root_node = std::dynamic_pointer_cast<geometry::OctreeInternalNode>(octree.root_node_)) {
        for (size_t i = 0; i < 8; ++i) {
            if (auto leaf_node =
                        std::dynamic_pointer_cast<geometry::OctreePointColorLeafNode>(root_node->children_[i])) {
                ExpectEQ(leaf_node->color_, colors[i]);
...
TEST(Octree, EightCubesLeafAndInternalPointIndices) {
    ...
    // Check node values
    if (auto root_node = std::dynamic_pointer_cast<geometry::OctreeInternalPointNode>(octree.root_node_)) {
...
TEST(Octree, EightCubesTraverse) {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        octree_1.InsertPoint(points[i], geometry::OctreeColorLeafNode::GetInitFunction(),
                             geometry::OctreeColorLeafNode::GetUpdateFunction(colors[i]));
    }
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        octree_5.InsertPoint(points[i], geometry::OctreeColorLeafNode::GetInitFunction(),
                             geometry::OctreeColorLeafNode::GetUpdateFunction(colors[i]));
    }
    colors_traversed.clear();
    child_indices_traversed.clear();
...
//         EXPECT_TRUE(geometry::Octree::IsPointInBound(point, node_info->origin_, node_info->size_));
