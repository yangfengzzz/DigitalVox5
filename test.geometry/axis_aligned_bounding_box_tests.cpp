...
TEST(AxisAlignedBoundingBox, GetBoxPoints) {
    ...
    for (auto exp : expected) {
        auto locate = [&exp](const Vector3d& v) { return (v - exp).norm() < 1e-6; };
...
TEST(AxisAlignedBoundingBox, PointIndiciesInBounds) {
    AxisAlignedBoundingBox box({1, 2, 3}, {4, 5, 6});
    std::vector<Vector3d> points{{2, 3, 4},  {1, 2, 3}, {1, 2, 6},  {1, 5, 6}, {1, 5, 3},
                                 {4, 2, 3},  {4, 2, 6}, {4, 5, 3},  {4, 5, 6}, {0, 3, 4},
                                 {10, 3, 4}, {2, 0, 4}, {2, 10, 4}, {2, 3, 0}, {2, 3, 10}};
...
TEST(AxisAlignedBoundingBox, CreateFromPoints) {
    std::vector<Vector3d> points{{2, 3, 4}, {1, 2, 6}, {1, 5, 6}, {1, 5, 3}, {4, 2, 3}, {4, 2, 6}, {4, 5, 3}};
...
TEST_P(IsEmptyTests, CheckBoxEmpty) {
    AxisAlignedBoundingBox box(std::get<0>(GetParam()), std::get<1>(GetParam()));
