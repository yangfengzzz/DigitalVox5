...
TEST(Image, CreateImage) {
    ...
    EXPECT_EQ(size_t(width * height * num_of_channels * bytes_per_channel), image.data_.size());
    ...
    EXPECT_EQ(width * num_of_channels * bytes_per_channel, image.BytesPerLine());
}
...
TEST(Image, DISABLED_MemberData) {
    ...
    EXPECT_EQ(temp_width * height * num_of_channels * bytes_per_channel, int(image.data_.size()));

    image.width_ = width;
    image.height_ = temp_height;
    EXPECT_EQ(width * temp_height * num_of_channels * bytes_per_channel, int(image.data_.size()));

    image.height_ = height;
    image.num_of_channels_ = temp_num_of_channels;
    EXPECT_EQ(width * height * temp_num_of_channels * bytes_per_channel, int(image.data_.size()));

    image.num_of_channels_ = num_of_channels;
    image.bytes_per_channel_ = temp_bytes_per_channel;
    EXPECT_EQ(width * height * num_of_channels * temp_bytes_per_channel, int(image.data_.size()));

    image.bytes_per_channel_ = bytes_per_channel;
    image.data_ = std::vector<uint8_t>();
    EXPECT_EQ(width * height * num_of_channels * bytes_per_channel, int(image.data_.size()));
}
...
//
//     auto image = geometry::Image::CreateDepthToCameraDistanceMultiplierFloatImage(intrinsic);
//
...
void TEST_CreateFloatImage(const int& num_of_channels,
...
TEST(Image, ConvertDepthToFloatImage) {
    ...
    std::vector<uint8_t> ref = {208, 254, 91,  58,  103, 154, 205, 57,  59,  147, 76,  58,  236, 175, 80,  58, 232,
                                127, 110, 58,  103, 154, 77,  57,  62,  195, 174, 57,  139, 118, 72,  58,  22, 236,
                                143, 57,  66,  243, 16,  58,  161, 199, 248, 57,  134, 123, 36,  58,  255, 53, 191,
                                57,  93,  164, 5,   58,  161, 199, 120, 58,  20,  135, 111, 58,  222, 137, 38, 58,
                                79,  25,  59,  58,  198, 8,   20,  57,  126, 80,  30,  58,  5,   150, 131, 55, 249,
                                213, 122, 57,  101, 207, 11,  57,  68,  190, 82,  58,  214, 94,  32,  57};
...
TEST(Image, TransposeFloat) {
    ...
    std::vector<float> transpose_image_data(transpose_image_floats, transpose_image_floats + transposed_ref.size());
...
void TEST_Filter(const std::vector<uint8_t>& ref, const geometry::Image::FilterType& filter) {
    geometry::Image image;
...
TEST(Image, Filter_Gaussian3) {
    ...
    std::vector<uint8_t> ref = {41,  194, 49,  204, 116, 56,  130, 211, 198, 225, 181, 232, 198, 225, 53,  233, 198,
                                225, 181, 232, 177, 94,  205, 232, 47,  90,  77,  233, 240, 252, 4,   233, 93,  130,
                                114, 232, 93,  130, 242, 231, 177, 94,  77,  233, 47,  90,  205, 233, 72,  89,  77,
                                233, 6,   134, 220, 88,  128, 234, 129, 89,  60,  96,  205, 232, 167, 91,  77,  233,
                                2,   196, 171, 233, 229, 149, 243, 233, 12,  159, 128, 233, 36,  49,  20,  226, 223,
                                39,  141, 226, 137, 164, 52,  234, 108, 176, 182, 234, 146, 238, 64,  234};
...
TEST(Image, Filter_Gaussian5) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {61,  94,  205, 231, 230, 96,  109, 232, 15,  16,  218, 232, 2,   118, 3,   233, 160,
                                185, 166, 232, 61,  94,  205, 232, 46,  125, 35,  233, 60,  145, 12,  233, 110, 3,
                                165, 232, 122, 145, 23,  232, 223, 6,   26,  233, 23,  249, 119, 233, 159, 37,  94,
                                233, 234, 229, 13,  233, 99,  24,  143, 232, 41,  96,  205, 232, 206, 73,  101, 233,
                                15,  186, 202, 233, 62,  231, 242, 233, 76,  236, 159, 233, 35,  111, 205, 231, 102,
                                26,  76,  233, 255, 241, 44,  234, 32,  174, 126, 234, 84,  234, 47,  234};
...
TEST(Image, Filter_Gaussian7) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {71,  19,  68,  232, 29,  11,  169, 232, 178, 140, 214, 232, 35,  21,  214, 232, 245,
                                42,  147, 232, 66,  168, 175, 232, 125, 101, 5,   233, 242, 119, 15,  233, 60,  92,
                                246, 232, 131, 231, 154, 232, 226, 75,  240, 232, 83,  18,  69,  233, 128, 68,  108,
                                233, 67,  141, 98,  233, 63,  199, 27,  233, 108, 191, 244, 232, 122, 49,  127, 233,
                                20,  166, 194, 233, 176, 46,  222, 233, 32,  207, 168, 233, 187, 237, 232, 232, 99,
                                40,  161, 233, 128, 206, 18,  234, 108, 135, 55,  234, 187, 97,  17,  234};
...
TEST(Image, Filter_Sobel3Dx) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {172, 2,   109, 77,  136, 55,  130, 213, 198, 225, 181, 234, 254, 55,  130, 85,  198,
                                225, 181, 106, 122, 87,  205, 234, 134, 196, 102, 99,  177, 184, 144, 106, 254, 55,
                                2,   86,  93,  130, 242, 105, 122, 87,  77,  235, 138, 196, 230, 99,  72,  89,  77,
                                107, 214, 220, 163, 90,  34,  71,  135, 90,  231, 88,  205, 234, 63,  133, 106, 99,
                                73,  45,  10,  235, 101, 207, 174, 232, 44,  100, 107, 107, 28,  239, 8,   228, 119,
                                32,  52,  97,  114, 163, 52,  236, 140, 27,  131, 233, 33,  139, 48,  108};
...
TEST(Image, Filter_Sobel3Dy) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {151, 248, 205, 205, 67,  56,  130, 213, 93,  130, 242, 105, 93,  130, 114, 106, 93,
                                130, 242, 105, 177, 94,  205, 234, 47,  90,  77,  235, 177, 184, 144, 234, 93,  130,
                                114, 106, 93,  130, 242, 105, 108, 57,  173, 217, 91,  238, 228, 216, 254, 55,  2,
                                86,  214, 220, 163, 90,  108, 154, 117, 91,  38,  93,  205, 106, 183, 88,  77,  107,
                                189, 46,  10,  235, 229, 149, 243, 235, 12,  159, 128, 235, 189, 150, 69,  227, 36,
                                53,  188, 227, 97,  219, 112, 235, 229, 149, 243, 235, 12,  159, 128, 235};
...
TEST(Image, FilterHorizontal) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {187, 139, 149, 203, 171, 101, 199, 202, 93,  130, 242, 232, 93,  130, 114, 233, 93,
                                130, 242, 232, 134, 91,  243, 204, 79,  56,  130, 212, 254, 55,  2,   213, 254, 55,
                                130, 212, 94,  58,  24,  196, 177, 94,  205, 233, 47,  90,  77,  234, 72,  89,  205,
                                233, 49,  169, 99,  88,  49,  169, 227, 87,  109, 57,  173, 216, 60,  247, 230, 215,
                                97,  137, 95,  192, 72,  188, 163, 89,  108, 154, 117, 90,  211, 150, 69,  226, 40,
                                53,  188, 226, 97,  219, 112, 234, 229, 149, 243, 234, 12,  159, 128, 234};
...
TEST(Image, Downsample) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {172, 41, 59, 204, 93, 130, 242, 232, 22, 91, 205, 233, 49, 169, 227, 87};
...
TEST(Image, Dilate) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {255, 255, 0,   0,   0,   0,   0,   255, 255, 255, 255, 255, 0,   0,   0,   0,   255,
                                255, 255, 255, 0,   0,   0,   0,   0,   255, 255, 255, 255, 255, 0,   0,   0,   0,
                                255, 255, 255, 255, 255, 0,   0,   0,   0,   255, 255, 255, 255, 255, 0,   0,   0,
                                0,   255, 255, 255, 255, 255, 0,   0,   0,   0,   255, 255, 255, 255, 255, 0,   0,
                                0,   0,   255, 255, 255, 255, 255, 0,   0,   0,   0,   0,   255, 255, 255, 255, 0,
                                0,   0,   0,   255, 255, 255, 255, 255, 0,   0,   0,   0,   0,   255, 255};
...
TEST(Image, LinearTransform) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {144, 77,  101, 204, 139, 26,  245, 195, 154, 153, 25,  62,  92,  113, 139, 234, 154,
                                153, 25,  62,  248, 146, 186, 205, 154, 153, 25,  62,  100, 192, 21,  214, 154, 153,
                                25,  62,  195, 101, 233, 196, 0,   177, 4,   228, 172, 38,  108, 235, 154, 153, 25,
                                62,  175, 231, 130, 89,  154, 153, 25,  62,  41,  206, 132, 217, 218, 221, 255, 193,
                                154, 153, 25,  62,  128, 136, 25,  62,  185, 75,  60,  91,  139, 24,  10,  225, 243,
                                71,  214, 227, 154, 153, 25,  62,  186, 125, 10,  236, 27,  8,   73,  233};
...
TEST(Image, ClipIntensity) {
    // reference data used to validate the filtering of an image
    std::vector<uint8_t> ref = {195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195,
                                245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245,
                                168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168,
                                62,  143, 194, 53,  63,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,
                                195, 245, 168, 62,  195, 245, 168, 62,  143, 194, 53,  63,  195, 245, 168, 62,  195,
                                245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62,  195, 245, 168, 62};
...
TEST(Image, CreateImageFromFloatImage_8bit) { TEST_CreateImageFromFloatImage<uint8_t>(); }

TEST(Image, CreateImageFromFloatImage_16bit) { TEST_CreateImageFromFloatImage<uint16_t>(); }

TEST(Image, FilterPyramid) {
    // reference data used to validate the filtering of an image
    std::vector<std::vector<uint8_t>> ref = {
            {110, 56,  130, 211, 17,  56,  2,   212, 198, 225, 181, 232, 173, 226, 53,  233, 84,  159, 65,  233, 105,
             3,   154, 233, 112, 151, 223, 86,  113, 151, 95,  87,  93,  130, 242, 231, 147, 137, 114, 232, 47,  173,
             107, 233, 105, 3,   26,  234, 224, 16,  192, 88,  171, 250, 111, 222, 215, 213, 65,  225, 189, 203, 23,
             226, 233, 196, 171, 233, 217, 210, 128, 234, 47,  127, 9,   233, 201, 240, 161, 108, 7,   103, 35,  109,
             31,  224, 163, 108, 22,  49,  241, 233, 69,  228, 180, 234, 36,  127, 137, 233, 201, 240, 33,  109, 9,
             103, 163, 109, 36,  224, 35,  109, 54,  50,  114, 233, 29,  165, 53,  234, 237, 126, 9,   233, 202, 240,
             161, 108, 9,   103, 35,  109, 37,  224, 163, 108, 141, 106, 43,  229, 234, 143, 0,   230},
            {57, 48,  241, 106, 168, 116, 5,  107, 106, 200, 26,  106, 115, 252, 23,  108, 93, 29,
             48, 108, 107, 19,  140, 107, 48, 19,  152, 108, 201, 182, 177, 108, 145, 200, 20, 108}};
    ...
    auto output_pyramid = geometry::Image::FilterPyramid(pyramid, FilterType::Gaussian3);
...
TEST(Image, CreatePyramid) {
    // reference data used to validate the filtering of an image
    std::vector<std::vector<uint8_t>> ref = {
            {214, 100, 199, 203, 232, 50,  85,  195, 70,  141, 121, 160, 93,  130, 242, 233, 162, 182, 36,  154, 4,
             61,  34,  205, 39,  102, 33,  27,  254, 55,  130, 213, 156, 75,  162, 133, 125, 248, 74,  196, 134, 196,
             102, 227, 72,  89,  205, 234, 17,  242, 134, 21,  49,  169, 227, 88,  16,  5,   116, 16,  60,  247, 230,
             216, 67,  137, 95,  193, 130, 170, 135, 10,  111, 237, 237, 183, 72,  188, 163, 90,  175, 42,  112, 224,
             211, 84,  58,  227, 89,  175, 243, 150, 167, 218, 112, 235, 101, 207, 174, 232, 123, 55,  242, 234, 37,
             224, 163, 110, 157, 71,  200, 78,  113, 57,  47,  70,  141, 106, 43,  231, 26,  32,  126, 193, 251, 238,
             174, 97,  191, 94,  75,  59,  149, 62,  38,  186, 31,  202, 41,  189, 19,  242, 13,  132},
            {236, 42,  166, 86,  32, 227, 181, 232, 31,  44,  169, 233, 203, 221, 160, 107, 20,  87,
             117, 108, 78,  122, 78, 234, 177, 76,  113, 108, 85,  1,   56,  109, 21,  221, 114, 233}};
