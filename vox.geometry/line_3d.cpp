...
Line3D::Line3D(const Eigen::Vector3d& origin, const Eigen::Vector3d& direction, Line3D::LineType type)
...
void Line3D::Transform(const Eigen::Transform<double, 3, Eigen::Affine>& t) { this->transform(t); }

std::pair<double, double> Line3D::SlabAABBBase(const AxisAlignedBoundingBox& box) const {
...
std::optional<double> Line3D::ExactAABB(const AxisAlignedBoundingBox& box) const {
    ...
    // eliminate the issue.
    double tol = 1e-10;
    AxisAlignedBoundingBox b_tol{box.min_bound_ - Vector3d(tol, tol, tol), box.max_bound_ + Vector3d(tol, tol, tol)};
...
std::optional<double> Line3D::SlabAABB(const AxisAlignedBoundingBox& box) const {
...
std::optional<double> Line3D::IntersectionParameter(const Eigen::Hyperplane<double, 3>& plane) const {
...
std::pair<Eigen::Vector3d, Eigen::Vector3d> Line3D::ClosestPoints(const Line3D& other) const {
    ...
    return {Line().pointAt(std::get<0>(result)), other.Line().pointAt(std::get<1>(result))};
}
...
std::optional<double> Ray3D::IntersectionParameter(const Eigen::Hyperplane<double, 3>& plane) const {
...
Segment3D::Segment3D(const Eigen::Vector3d& start_point, const Eigen::Vector3d& end_point)
    : Line3D(start_point, (end_point - start_point).normalized(), LineType::SEGMENT),
...
std::optional<double> Segment3D::SlabAABB(const AxisAlignedBoundingBox& box) const {
...
std::optional<double> Segment3D::ExactAABB(const AxisAlignedBoundingBox& box) const {
...
AxisAlignedBoundingBox Segment3D::GetBoundingBox() const {
    Eigen::Vector3d min{std::min(origin().x(), end_point_.x()), std::min(origin().y(), end_point_.y()),
                        std::min(origin().z(), end_point_.z())};

    Eigen::Vector3d max{std::max(origin().x(), end_point_.x()), std::max(origin().y(), end_point_.y()),
                        std::max(origin().z(), end_point_.z())};
    return {min, max};
}

std::optional<double> Segment3D::IntersectionParameter(const Eigen::Hyperplane<double, 3>& plane) const {
