...
TriangleMesh &TriangleMesh::Rotate(const Eigen::Matrix3d &R, const Eigen::Vector3d &center) {
...
TriangleMesh &TriangleMesh::operator+=(const TriangleMesh &mesh) {
    ...
    bool add_textures = HasTriangleUvs() && HasTextures() && HasTriangleMaterialIds() && mesh.HasTriangleUvs() &&
    ...
    if ((!HasTriangles() || HasTriangleNormals()) && mesh.HasTriangleNormals()) {
        triangle_normals_.resize(new_tri_num);
        for (size_t i = 0; i < add_tri_num; i++) triangle_normals_[old_tri_num + i] = mesh.triangle_normals_[i];
    } else {
        triangle_normals_.clear();
    }
    triangles_.resize(triangles_.size() + mesh.triangles_.size());
    Eigen::Vector3i index_shift((int)old_vert_num, (int)old_vert_num, (int)old_vert_num);
    ...
    if (add_textures) {
        ...
        for (size_t i = 0; i < mesh.triangle_material_ids_.size(); i++) {
            triangle_material_ids_[old_mat_id_num + i] = mesh.triangle_material_ids_[i] + (int)old_tex_num;
        }
    } else {
        triangle_uvs_.clear();
        textures_.clear();
...
TriangleMesh TriangleMesh::operator+(const TriangleMesh &mesh) const { return (TriangleMesh(*this) += mesh); }
...
std::shared_ptr<TriangleMesh> TriangleMesh::FilterSharpen(int number_of_iterations,
                                                          double strength,
                                                          FilterScope scope) const {
    bool filter_vertex = scope == FilterScope::All || scope == FilterScope::Vertex;
    bool filter_normal = (scope == FilterScope::All || scope == FilterScope::Normal) && HasVertexNormals();
    bool filter_color = (scope == FilterScope::All || scope == FilterScope::Color) && HasVertexColors();
    ...
    for (int iter = 0; iter < number_of_iterations; ++iter) {
        for (size_t vidx = 0; vidx < mesh->vertices_.size(); ++vidx) {
            ...
            if (filter_vertex) {
                mesh->vertices_[vidx] = prev_vertices[vidx] + strength * (prev_vertices[vidx] * nb_size - vertex_sum);
            }
            if (filter_normal) {
                mesh->vertex_normals_[vidx] =
                        prev_vertex_normals[vidx] + strength * (prev_vertex_normals[vidx] * nb_size - normal_sum);
            }
            if (filter_color) {
                mesh->vertex_colors_[vidx] =
                        prev_vertex_colors[vidx] + strength * (prev_vertex_colors[vidx] * nb_size - color_sum);
            }
        }
...
std::shared_ptr<TriangleMesh> TriangleMesh::FilterSmoothSimple(int number_of_iterations, FilterScope scope) const {
    bool filter_vertex = scope == FilterScope::All || scope == FilterScope::Vertex;
    bool filter_normal = (scope == FilterScope::All || scope == FilterScope::Normal) && HasVertexNormals();
    bool filter_color = (scope == FilterScope::All || scope == FilterScope::Color) && HasVertexColors();
    ...
    for (int iter = 0; iter < number_of_iterations; ++iter) {
        for (size_t vidx = 0; vidx < mesh->vertices_.size(); ++vidx) {
            ...
            size_t nb_size = mesh->adjacency_list_[vidx].size();
            if (filter_vertex) {
                mesh->vertices_[vidx] = (prev_vertices[vidx] + vertex_sum) / (1 + nb_size);
            }
            if (filter_normal) {
                mesh->vertex_normals_[vidx] = (prev_vertex_normals[vidx] + normal_sum) / (1 + nb_size);
            }
            if (filter_color) {
                mesh->vertex_colors_[vidx] = (prev_vertex_colors[vidx] + color_sum) / (1 + nb_size);
            }
        }
        if (iter < number_of_iterations - 1) {
            std::swap(mesh->vertices_, prev_vertices);
...
void TriangleMesh::FilterSmoothLaplacianHelper(std::shared_ptr<TriangleMesh> &mesh,
    ...
    for (size_t vidx = 0; vidx < mesh->vertices_.size(); ++vidx) {
        ...
        if (filter_vertex) {
            mesh->vertices_[vidx] =
                    prev_vertices[vidx] + lambda_filter * (vertex_sum / total_weight - prev_vertices[vidx]);
        }
        if (filter_normal) {
            mesh->vertex_normals_[vidx] =
                    prev_vertex_normals[vidx] + lambda_filter * (normal_sum / total_weight - prev_vertex_normals[vidx]);
        }
        if (filter_color) {
            mesh->vertex_colors_[vidx] =
                    prev_vertex_colors[vidx] + lambda_filter * (color_sum / total_weight - prev_vertex_colors[vidx]);
        }
    }
}

std::shared_ptr<TriangleMesh> TriangleMesh::FilterSmoothLaplacian(int number_of_iterations,
                                                                  double lambda_filter,
                                                                  FilterScope scope) const {
    bool filter_vertex = scope == FilterScope::All || scope == FilterScope::Vertex;
    bool filter_normal = (scope == FilterScope::All || scope == FilterScope::Normal) && HasVertexNormals();
    ...
    for (int iter = 0; iter < number_of_iterations; ++iter) {
        FilterSmoothLaplacianHelper(mesh, prev_vertices, prev_vertex_normals, prev_vertex_colors, mesh->adjacency_list_,
                                    lambda_filter, filter_vertex, filter_normal, filter_color);
        if (iter < number_of_iterations - 1) {
            std::swap(mesh->vertices_, prev_vertices);
            std::swap(mesh->vertex_normals_, prev_vertex_normals);
            std::swap(mesh->vertex_colors_, prev_vertex_colors);
...
std::shared_ptr<TriangleMesh> TriangleMesh::FilterSmoothTaubin(int number_of_iterations,
                                                               double lambda_filter,
                                                               double mu,
                                                               FilterScope scope) const {
    bool filter_vertex = scope == FilterScope::All || scope == FilterScope::Vertex;
...
std::shared_ptr<PointCloud> TriangleMesh::SamplePointsUniformlyImpl(size_t number_of_points,
                                                                    std::vector<double> &triangle_areas,
                                                                    double surface_area,
                                                                    bool use_triangle_normal,
                                                                    int seed) {
    if (surface_area <= 0) {
        LOGE("Invalid surface area {}, it must be > 0.", surface_area)
    }

    // triangle areas to cdf
    triangle_areas[0] /= surface_area;
    for (size_t tidx = 1; tidx < triangles_.size(); ++tidx) {
        triangle_areas[tidx] = triangle_areas[tidx] / surface_area + triangle_areas[tidx - 1];
    }
    ...
    for (size_t tidx = 0; tidx < triangles_.size(); ++tidx) {
        auto n = size_t(std::round(triangle_areas[tidx] * number_of_points));
        while (point_idx < n) {
            double r1 = dist(mt);
            double r2 = dist(mt);
            ...
            pcd->points_[point_idx] =
                    a * vertices_[triangle(0)] + b * vertices_[triangle(1)] + c * vertices_[triangle(2)];
            if (has_vert_normal && !use_triangle_normal) {
                pcd->normals_[point_idx] = a * vertex_normals_[triangle(0)] + b * vertex_normals_[triangle(1)] +
            ...
            if (has_vert_color) {
                pcd->colors_[point_idx] = a * vertex_colors_[triangle(0)] + b * vertex_colors_[triangle(1)] +
...
std::shared_ptr<PointCloud> TriangleMesh::SamplePointsUniformly(size_t number_of_points,
                                                                bool use_triangle_normal /* = false */,
                                                                int seed /* = -1 */) {
    if (number_of_points <= 0) {
        LOGE("number_of_points <= 0")
    }
    if (triangles_.empty()) {
        LOGE("Input mesh has no triangles.")
    }
    ...
    return SamplePointsUniformlyImpl(number_of_points, triangle_areas, surface_area, use_triangle_normal, seed);
}
...
std::shared_ptr<PointCloud> TriangleMesh::SamplePointsPoissonDisk(
    ...
    if (pcl_init == nullptr && init_factor < 1) {
        LOGE("Either pass pcl_init with #points > number_of_points or "
             "init_factor > 1")
    }
    ...
    if (pcl_init == nullptr) {
        pcl = SamplePointsUniformlyImpl(size_t(init_factor * number_of_points), triangle_areas, surface_area,
    ...
    auto WeightCmp = [](const QueueEntry &a, const QueueEntry &b) { return std::get<1>(a) < std::get<1>(b); };
...
TriangleMesh &TriangleMesh::RemoveDuplicatedVertices() {
    ...
    std::unordered_map<Coordinate3, size_t, utility::hash_tuple<Coordinate3>> point_to_old_index;
    ...
    for (size_t i = 0; i < old_vertex_num; i++) {  // old index
        Coordinate3 coord = std::make_tuple(vertices_[i](0), vertices_[i](1), vertices_[i](2));
    ...
    LOGD("[RemoveDuplicatedVertices] {:d} vertices have been removed.", (int)(old_vertex_num - k))
...
TriangleMesh &TriangleMesh::RemoveDuplicatedTriangles() {
    if (HasTriangleUvs()) {
        LOGW("[RemoveDuplicatedTriangles] This mesh contains triangle uvs "
             "that are not handled in this function")
    }
    typedef std::tuple<int, int, int> Index3;
    std::unordered_map<Index3, size_t, utility::hash_tuple<Index3>> triangle_to_old_index;
    ...
    for (size_t i = 0; i < old_triangle_num; i++) {
        ...
        if (triangles_[i](0) <= triangles_[i](1)) {
            if (triangles_[i](0) <= triangles_[i](2)) {
                index = std::make_tuple(triangles_[i](0), triangles_[i](1), triangles_[i](2));
            } else {
                index = std::make_tuple(triangles_[i](2), triangles_[i](0), triangles_[i](1));
            }
        } else {
            if (triangles_[i](1) <= triangles_[i](2)) {
                index = std::make_tuple(triangles_[i](1), triangles_[i](2), triangles_[i](0));
            } else {
                index = std::make_tuple(triangles_[i](2), triangles_[i](0), triangles_[i](1));
            }
        }
    ...
    LOGD("[RemoveDuplicatedTriangles] {:d} triangles have been removed.", (int)(old_triangle_num - k))
...
TriangleMesh &TriangleMesh::RemoveUnreferencedVertices() {
    ...
    LOGD("[RemoveUnreferencedVertices] {:d} vertices have been removed.", (int)(old_vertex_num - k))
...
TriangleMesh &TriangleMesh::RemoveDegenerateTriangles() {
    if (HasTriangleUvs()) {
        LOGW("[RemoveDegenerateTriangles] This mesh contains triangle uvs "
             "that are not handled in this function")
    }
    bool has_tri_normal = HasTriangleNormals();
    size_t old_triangle_num = triangles_.size();
    ...
    LOGD("[RemoveDegenerateTriangles] {:d} triangles have been "
         "removed.",
...
TriangleMesh &TriangleMesh::RemoveNonManifoldEdges() {
    if (HasTriangleUvs()) {
        LOGW("[RemoveNonManifoldEdges] This mesh contains triangle uvs that "
...
#pragma omp parallel for schedule(static) num_threads(utility::EstimateMaxThreads()) default(none) \
    ...
    LOGD("Done Precompute Neighbours")
    ...
    LOGD("Merged {} vertices", vertices_.size() - new_vertices.size())
...
bool OrientTriangleHelper(const std::vector<Eigen::Vector3i> &triangles, F &swap) {
    std::unordered_map<Eigen::Vector2i, Eigen::Vector2i, utility::hash_eigen<Eigen::Vector2i>> edge_to_orientation;
    std::unordered_set<int> unvisited_triangles;
    std::unordered_map<Eigen::Vector2i, std::unordered_set<int>, utility::hash_eigen<Eigen::Vector2i>>
            adjacent_triangles;
    ...
    for (size_t tidx = 0; tidx < triangles.size(); ++tidx) {
        ...
        adjacent_triangles[TriangleMesh::GetOrderedEdge(vidx0, vidx1)].insert(int(tidx));
        adjacent_triangles[TriangleMesh::GetOrderedEdge(vidx1, vidx2)].insert(int(tidx));
        adjacent_triangles[TriangleMesh::GetOrderedEdge(vidx2, vidx0)].insert(int(tidx));
    }
...
bool TriangleMesh::IsWatertight() const { return IsEdgeManifold(false) && IsVertexManifold() && !IsSelfIntersecting(); }
...
std::unordered_map<Eigen::Vector2i, std::vector<int>, utility::hash_eigen<Eigen::Vector2i>>
TriangleMesh::GetEdgeToTrianglesMap() const {
    std::unordered_map<Eigen::Vector2i, std::vector<int>, utility::hash_eigen<Eigen::Vector2i>> trias_per_edge;
...
double TriangleMesh::GetVolume() const {
    ...
    if (!IsWatertight()) {
        LOGE("The mesh is not watertight, and the volume cannot be "
             "computed.")
    }
    if (!IsOrientable()) {
        LOGE("The mesh is not orientable, and the volume cannot be "
             "computed.")
    }
...
int TriangleMesh::EulerPoincareCharacteristic() const {
    std::unordered_set<Eigen::Vector2i, utility::hash_eigen<Eigen::Vector2i>> edges;
    for (auto triangle : triangles_) {
...
std::vector<Eigen::Vector2i> TriangleMesh::GetNonManifoldEdges(bool allow_boundary_edges /* = true */) const {
    auto edges = GetEdgeToTrianglesMap();
    std::vector<Eigen::Vector2i> non_manifold_edges;
    for (auto &kv : edges) {
        if ((allow_boundary_edges && (kv.second.empty() || kv.second.size() > 2)) ||
...
bool TriangleMesh::IsEdgeManifold(bool allow_boundary_edges /* = true */) const {
    auto edges = GetEdgeToTrianglesMap();
    for (auto &kv : edges) {
        if ((allow_boundary_edges && (kv.second.empty() || kv.second.size() > 2)) ||
            (!allow_boundary_edges && kv.second.size() != 2)) {
...
bool TriangleMesh::IsVertexManifold() const { return GetNonManifoldVertices().empty(); }

std::vector<Eigen::Vector2i> TriangleMesh::GetSelfIntersectingTriangles() const {
    ...
    for (size_t tidx0 = 0; tidx0 < triangles_.size() - 1; ++tidx0) {
        ...
        const Eigen::Vector3d bb_min1 = p0.array().min(p1.array().min(p2.array()));
        const Eigen::Vector3d bb_max1 = p0.array().max(p1.array().max(p2.array()));
        ...
        for (size_t tidx1 = tidx0 + 1; tidx1 < triangles_.size(); ++tidx1) {
            ...
            if (tria_p(0) == tria_q(0) || tria_p(0) == tria_q(1) || tria_p(0) == tria_q(2) || tria_p(1) == tria_q(0) ||
                tria_p(1) == tria_q(1) || tria_p(1) == tria_q(2) || tria_p(2) == tria_q(0) || tria_p(2) == tria_q(1) ||
                tria_p(2) == tria_q(2)) {
                continue;
            }
            ...
            const Eigen::Vector3d bb_min2 = q0.array().min(q1.array().min(q2.array()));
            const Eigen::Vector3d bb_max2 = q0.array().max(q1.array().max(q2.array()));
            if (IntersectionTest::AABBAABB(bb_min1, bb_max1, bb_min2, bb_max2) &&
                IntersectionTest::TriangleTriangle3d(p0, p1, p2, q0, q1, q2)) {
                self_intersecting_triangles.emplace_back(Eigen::Vector2i(tidx0, tidx1));
            }
        }
    }
...
bool TriangleMesh::IsSelfIntersecting() const { return !GetSelfIntersectingTriangles().empty(); }

bool TriangleMesh::IsBoundingBoxIntersecting(const TriangleMesh &other) const {
    return IntersectionTest::AABBAABB(GetMinBound(), GetMaxBound(), other.GetMinBound(), other.GetMaxBound());
}
...
std::tuple<std::vector<int>, std::vector<size_t>, std::vector<double>> TriangleMesh::ClusterConnectedTriangles() const {
...
#pragma omp parallel for schedule(static) num_threads(utility::EstimateMaxThreads()) default(none) \
    ...
    for (int tidx = 0; tidx < int(triangles_.size()); ++tidx) {
        ...
        for (auto tnb : edges_to_triangles[GetOrderedEdge(triangle(0), triangle(2))]) {
            adjacency_list[tidx].insert(tnb);
        }
        for (auto tnb : edges_to_triangles[GetOrderedEdge(triangle(1), triangle(2))]) {
            adjacency_list[tidx].insert(tnb);
        }
    }
    LOGD("[ClusterConnectedTriangles] Done computing triangle adjacency")
    ...
    LOGD("[ClusterConnectedTriangles] Done clustering, #clusters={}", cluster_idx)
    return std::make_tuple(triangle_clusters, num_triangles, areas);
}

void TriangleMesh::RemoveTrianglesByIndex(const std::vector<size_t> &triangle_indices) {
    ...
    for (auto tidx : triangle_indices) {
        if (tidx < triangles_.size()) {
            ...
            LOGW("[RemoveTriangles] contains triangle index {} that is not "
                 "within the bounds",
                 tidx)
        }
    }
...
void TriangleMesh::RemoveTrianglesByMask(const std::vector<bool> &triangle_mask) {
    if (triangle_mask.size() != triangles_.size()) {
        LOGE("triangle_mask has a different size than triangles_")
    }
    ...
    for (size_t from_tidx = 0; from_tidx < triangles_.size(); ++from_tidx) {
        if (!triangle_mask[from_tidx]) {
            ...
            if (has_tri_uvs) {
                ...
                triangle_uvs_[to_tidx * 3 + 1] = triangle_uvs_[from_tidx * 3 + 1];
                triangle_uvs_[to_tidx * 3 + 2] = triangle_uvs_[from_tidx * 3 + 2];
            }
            to_tidx++;
        }
    }
...
void TriangleMesh::RemoveVerticesByIndex(const std::vector<size_t> &vertex_indices) {
    std::vector<bool> vertex_mask(vertices_.size(), false);
    for (auto vidx : vertex_indices) {
        if (vidx < vertices_.size()) {
            vertex_mask[vidx] = true;
        } else {
            LOGW("[RemoveVerticessByIndex] contains vertex index {} that is "
...
void TriangleMesh::RemoveVerticesByMask(const std::vector<bool> &vertex_mask) {
    if (vertex_mask.size() != vertices_.size()) {
        LOGE("vertex_mask has a different size than vertices_")
    }
    ...
    for (size_t tidx = 0; tidx < triangles_.size(); ++tidx) {
        ...
        triangle_mask[tidx] = vertex_mask[tria(0)] || vertex_mask[tria(1)] || vertex_mask[tria(2)];
...
std::shared_ptr<TriangleMesh> TriangleMesh::SelectByIndex(const std::vector<size_t> &indices, bool cleanup) const {
    if (HasTriangleUvs()) {
        LOGW("[SelectByIndex] This mesh contains triangle uvs that are "
    ...
    for (const auto &sel_vidx : indices) {
        if (sel_vidx >= vertices_.size()) {
            LOGW("[SelectByIndex] indices contains index {} out of range. "
                 "It is ignored.",
                 sel_vidx)
            continue;
        }
    ...
    for (size_t tidx = 0; tidx < triangles_.size(); ++tidx) {
        ...
        if (nvidx0 >= 0 && nvidx1 >= 0 && nvidx2 >= 0) {
            output->triangles_.emplace_back(Eigen::Vector3i(nvidx0, nvidx1, nvidx2));
    ...
    LOGD("Triangle mesh sampled from {:d} vertices and {:d} triangles to "
         "{:d} vertices and {:d} triangles.",
         (int)vertices_.size(), (int)triangles_.size(), (int)output->vertices_.size(), (int)output->triangles_.size())
    return output;
}  // namespace geometry

std::shared_ptr<TriangleMesh> TriangleMesh::Crop(const AxisAlignedBoundingBox &bbox) const {
    if (bbox.IsEmpty()) {
        LOGE("AxisAlignedBoundingBox either has zeros "
             "size, or has wrong bounds.")
    }
    return SelectByIndex(bbox.GetPointIndicesWithinBoundingBox(vertices_));
}

std::shared_ptr<TriangleMesh> TriangleMesh::Crop(const OrientedBoundingBox &bbox) const {
    if (bbox.IsEmpty()) {
        LOGE("AxisAlignedBoundingBox either has zeros "
             "size, or has wrong bounds.")
...
std::unordered_map<Eigen::Vector2i, double, utility::hash_eigen<Eigen::Vector2i>> TriangleMesh::ComputeEdgeWeightsCot(
        const std::unordered_map<Eigen::Vector2i, std::vector<int>, utility::hash_eigen<Eigen::Vector2i>>
                &edges_to_vertices,
        double min_weight) const {
    std::unordered_map<Eigen::Vector2i, double, utility::hash_eigen<Eigen::Vector2i>> weights;
