...
struct WeightedEdge {
    WeightedEdge(size_t v0, size_t v1, double weight) : v0_(v0), v1_(v1), weight_(weight) {}
    size_t v0_;
    size_t v1_;
    double weight_;
};
...
void PointCloud::EstimateNormals(const KDTreeSearchParam &search_param /* = KDTreeSearchParamKNN()*/,
...
void PointCloud::OrientNormalsConsistentTangentPlane(size_t k) {
    ...
    auto NormalWeight = [&](size_t v0, size_t v1) -> double { return 1.0 - std::abs(normals_[v0].dot(normals_[v1])); };
    for (auto &edge : mst) {
    ...
    auto TestAndOrientNormal = [&](const Eigen::Vector3d &n0, Eigen::Vector3d &n1) {
        if (n0.dot(n1) < 0) {
            n1 *= -1;
        }
    };
