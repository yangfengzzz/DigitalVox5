...
VoxelGrid::VoxelGrid(const VoxelGrid &src_voxel_grid)
    : Geometry3D(Geometry::GeometryType::VOXEL_GRID),
...
Eigen::Vector3d VoxelGrid::GetCenter() const {
    ...
    const Eigen::Vector3d half_voxel_size(0.5 * voxel_size_, 0.5 * voxel_size_, 0.5 * voxel_size_);
    for (const auto &it : voxels_) {
        const geometry::Voxel &voxel = it.second;
        center += voxel.grid_index_.cast<double>() * voxel_size_ + origin_ + half_voxel_size;
    }
...
VoxelGrid &VoxelGrid::Transform(const Eigen::Matrix4d &transformation) {
    LOGE("VoxelGrid::Transform is not supported")
    return *this;
}

VoxelGrid &VoxelGrid::Translate(const Eigen::Vector3d &translation, bool relative) {
...
VoxelGrid &VoxelGrid::operator+=(const VoxelGrid &voxelgrid) {
    if (voxel_size_ != voxelgrid.voxel_size_) {
        LOGE("Could not combine VoxelGrid because voxel_size "
             "differs (this=%f, other=%f)",
             voxel_size_, voxelgrid.voxel_size_)
    }
    if (origin_ != voxelgrid.origin_) {
        LOGE("Could not combine VoxelGrid because origin "
             "differs (this=%f,%f,%f, other=%f,%f,%f)",
             origin_(0), origin_(1), origin_(2), voxelgrid.origin_(0), voxelgrid.origin_(1), voxelgrid.origin_(2))
    }
    if (this->HasColors() != voxelgrid.HasColors()) {
        LOGE("Could not combine VoxelGrid one has colors and "
             "the other not.")
    }
    ...
    for (const auto &it : voxelgrid.voxels_) {
        const geometry::Voxel &voxel = it.second;
        if (has_colors) {
            voxelindex_to_accpoint[voxel.grid_index_].Add(voxel.grid_index_, voxel.color_);
        } else {
    ...
    for (const auto &accpoint : voxelindex_to_accpoint) {
        this->AddVoxel(Voxel(accpoint.second.GetVoxelIndex(), accpoint.second.GetAverageColor()));
    }
    return *this;
}

VoxelGrid VoxelGrid::operator+(const VoxelGrid &voxelgrid) const { return (VoxelGrid(*this) += voxelgrid); }
...
std::vector<Eigen::Vector3d> VoxelGrid::GetVoxelBoundingPoints(const Eigen::Vector3i &index) const {
...
void VoxelGrid::AddVoxel(const Voxel &voxel) { voxels_[voxel.grid_index_] = voxel; }

std::vector<bool> VoxelGrid::CheckIfIncluded(const std::vector<Eigen::Vector3d> &queries) const {
...
void VoxelGrid::CreateFromOctree(const Octree &octree) {
    ...
    std::unordered_map<std::shared_ptr<OctreeColorLeafNode>, std::shared_ptr<OctreeNodeInfo>> map_node_to_node_info;
    auto f_collect_nodes = [&map_node_to_node_info](const std::shared_ptr<OctreeNode> &node,
    ...
    for (const auto &it : map_node_to_node_info) {
        ...
        Eigen::Array3d node_center = Eigen::Array3d(node_info->origin_) + node_info->size_ / 2.0;
        Eigen::Vector3i grid_index = Eigen::floor((node_center - Eigen::Array3d(origin_)) / voxel_size_).cast<int>();
...
// VoxelGrid &VoxelGrid::CarveDepthMap(const Image &depth_map,
...
//         LOGE("provided depth_map dimensions are not compatible "
...
//             if ((!within_boundary && keep_voxels_outside_image) || (within_boundary && d > 0 && z >= d)) {
//                 carve = false;
//                 break;
//             }
//         }
...
//             if ((!within_boundary && keep_voxels_outside_image) || (within_boundary && d > 0)) {
//                 carve = false;
//                 break;
//             }
//         }
