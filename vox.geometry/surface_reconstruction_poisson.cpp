...
public:
    explicit Open3DPointStream(const vox::geometry::PointCloud* pcd) : pcd_(pcd), xform_(nullptr), current_(0) {}
...
public:
    const vox::geometry::PointCloud* pcd_;
    XForm<Real, 4>* xform_;
    size_t current_;
};
...
void ExtractMesh(float datax,
                 ...
                 std::shared_ptr<vox::geometry::TriangleMesh>& out_mesh,
...
void Execute(const vox::geometry::PointCloud& pcd,
             std::shared_ptr<vox::geometry::TriangleMesh>& out_mesh,
             std::vector<double>& out_densities,
             int depth,
             float width,
