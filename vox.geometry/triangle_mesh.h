...
public:
    ...
    TriangleMesh() : MeshBase(Geometry::GeometryType::TRIANGLE_MESH) {}
    /// \brief Parameterized Constructor.
    ///
    /// \param vertices list of vertices.
    /// \param triangles list of triangles.
    TriangleMesh(const std::vector<Eigen::Vector3d> &vertices, std::vector<Eigen::Vector3i> triangles)
        : MeshBase(Geometry::GeometryType::TRIANGLE_MESH, vertices), triangles_(std::move(triangles)) {}
    ~TriangleMesh() override = default;

public:
    TriangleMesh &Clear() override;
    TriangleMesh &Transform(const Eigen::Matrix4d &transformation) override;
    TriangleMesh &Rotate(const Eigen::Matrix3d &R, const Eigen::Vector3d &center) override;
...
public:
    ...
    [[nodiscard]] bool HasTriangles() const { return !vertices_.empty() && triangles_.size() > 0; }
    ...
    [[nodiscard]] bool HasAdjacencyList() const {
        return !vertices_.empty() && adjacency_list_.size() == vertices_.size();
    }

    [[nodiscard]] bool HasTriangleUvs() const {
    ...
    [[nodiscard]] bool HasTextures() const {
        bool is_all_texture_valid = std::accumulate(textures_.begin(), textures_.end(), true,
    ...
    [[nodiscard]] bool HasTriangleMaterialIds() const {
    ...
    TriangleMesh &NormalizeNormals() {
        ...
        for (auto &triangle_normal : triangle_normals_) {
            ...
            if (std::isnan(triangle_normal(0))) {
                triangle_normal = Eigen::Vector3d(0.0, 0.0, 1.0);
            }
        }
        return *this;
    }
    ...
    [[nodiscard]] std::shared_ptr<TriangleMesh> FilterSharpen(int number_of_iterations,
                                                              double strength,
    ...
    /// \param number_of_iterations defines the number of repetitions
    /// of this operation.
    [[nodiscard]] std::shared_ptr<TriangleMesh> FilterSmoothSimple(int number_of_iterations,
                                                                   FilterScope scope = FilterScope::All) const;
    ...
    [[nodiscard]] std::shared_ptr<TriangleMesh> FilterSmoothLaplacian(int number_of_iterations,
    ...
    [[nodiscard]] std::shared_ptr<TriangleMesh> FilterSmoothTaubin(int number_of_iterations,
    ...
    [[nodiscard]] int EulerPoincareCharacteristic() const;
    ...
    /// edges are returned.
    [[nodiscard]] std::vector<Eigen::Vector2i> GetNonManifoldEdges(bool allow_boundary_edges = true) const;
    ...
    [[nodiscard]] bool IsEdgeManifold(bool allow_boundary_edges = true) const;
    ...
    [[nodiscard]] std::vector<int> GetNonManifoldVertices() const;

    /// Function that checks if all vertices in the triangle mesh are manifold.
    /// A vertex is manifold if its star is edge‐manifold and edge‐connected.
    /// (Two or more faces connected only by a vertex and not by an edge.)
    [[nodiscard]] bool IsVertexManifold() const;

    /// Function that returns a list of triangles that are intersecting the
    /// mesh.
    [[nodiscard]] std::vector<Eigen::Vector2i> GetSelfIntersectingTriangles() const;
    ...
    /// intersecting.
    [[nodiscard]] bool IsBoundingBoxIntersecting(const TriangleMesh &other) const;

    /// Function that tests if the triangle mesh intersects another triangle
    /// mesh. Tests each triangle against each other triangle.
    [[nodiscard]] bool IsIntersecting(const TriangleMesh &other) const;
    ...
    /// towards the outside.
    [[nodiscard]] bool IsOrientable() const;
    ...
    [[nodiscard]] bool IsWatertight() const;
    ...
    [[nodiscard]] std::unordered_map<Eigen::Vector2i, std::vector<int>, utility::hash_eigen<Eigen::Vector2i>>
    ...
    /// triangle index
    [[nodiscard]] double GetTriangleArea(size_t triangle_idx) const;

    static inline Eigen::Vector3i GetOrderedTriangle(int vidx0, int vidx1, int vidx2) {
        if (vidx0 > vidx2) {
    ...
    [[nodiscard]] Eigen::Vector4d GetTrianglePlane(size_t triangle_idx) const;
    ...
    [[nodiscard]] std::shared_ptr<TriangleMesh> SubdivideMidpoint(int number_of_iterations) const;
    ...
    /// \param number_of_iterations defines a single iteration splits each
    /// triangle into four triangles that cover the same surface.
    [[nodiscard]] std::shared_ptr<TriangleMesh> SubdivideLoop(int number_of_iterations) const;
    ...
    /// adjacent planes.
    [[nodiscard]] std::shared_ptr<TriangleMesh> SimplifyVertexClustering(
            double voxel_size, SimplificationContraction contraction = SimplificationContraction::Average) const;
    ...
    /// preserve boundaries
    [[nodiscard]] std::shared_ptr<TriangleMesh> SimplifyQuadricDecimation(int target_number_of_triangles,
    ...
    /// TriangleMesh::RemoveDegenerateTriangles
    [[nodiscard]] std::shared_ptr<TriangleMesh> SelectByIndex(const std::vector<size_t> &indices,
    ...
    [[nodiscard]] std::shared_ptr<TriangleMesh> Crop(const AxisAlignedBoundingBox &bbox) const;

    /// Function to crop pointcloud into output pointcloud
    /// All points with coordinates outside the bounding box \p bbox are
    /// clipped.
    /// \param bbox defines the input Oriented Bounding Box.
    [[nodiscard]] std::shared_ptr<TriangleMesh> Crop(const OrientedBoundingBox &bbox) const;
    ...
    [[nodiscard]] std::tuple<std::vector<int>, std::vector<size_t>, std::vector<double>> ClusterConnectedTriangles()
            const;
    ...
    [[nodiscard]] std::shared_ptr<TriangleMesh> DeformAsRigidAsPossible(
    ...
    static std::shared_ptr<TriangleMesh> CreateFromPointCloudAlphaShape(const PointCloud &pcd,
                                                                        double alpha,
    ...
    static std::shared_ptr<TriangleMesh> CreateFromPointCloudBallPivoting(const PointCloud &pcd,
    ...
    static std::tuple<std::shared_ptr<TriangleMesh>, std::vector<double>> CreateFromPointCloudPoisson(
    ...
    static std::shared_ptr<TriangleMesh> CreateCylinder(
            double radius = 1.0, double height = 2.0, int resolution = 20, int split = 4, bool create_uv_map = false);
    ...
    /// \param split defines that the height will be split into split segments.
    /// \param create_uv_map add default UV map to the mesh.
    static std::shared_ptr<TriangleMesh> CreateCone(
            double radius = 1.0, double height = 2.0, int resolution = 20, int split = 1, bool create_uv_map = false);
...
protected:
    ...
    [[nodiscard]] std::unordered_map<Eigen::Vector2i, double, utility::hash_eigen<Eigen::Vector2i>>
...
public:
    ...
    struct Material {
        struct MaterialParameter {
            ...
            [[nodiscard]] float r() const { return f4[0]; }
            [[nodiscard]] float g() const { return f4[1]; }
            [[nodiscard]] float b() const { return f4[2]; }
            [[nodiscard]] float a() const { return f4[3]; }
        };
