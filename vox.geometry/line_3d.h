...
public:
    ...
    /// p0 to p1
    static Line3D Through(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1) { return {p0, (p1 - p0).normalized()}; }
    ...
    [[nodiscard]] const Eigen::Vector3d& Direction() const { return m_direction; }
    ...
    [[nodiscard]] virtual double Length() const { return std::numeric_limits<double>::infinity(); }
    ...
    [[nodiscard]] const Eigen::ParametrizedLine<double, 3>& Line() const { return *this; }
    ...
    [[nodiscard]] virtual std::optional<double> IntersectionParameter(const Eigen::Hyperplane<double, 3>& plane) const;
    ...
    [[nodiscard]] double ProjectionParameter(const Eigen::Vector3d& point) const;
    ...
    [[nodiscard]] virtual Eigen::Vector3d Projection(const Eigen::Vector3d& point) const;
    ...
    /// method
    [[nodiscard]] virtual std::optional<double> SlabAABB(const AxisAlignedBoundingBox& box) const;
    ...
    [[nodiscard]] virtual std::optional<double> ExactAABB(const AxisAlignedBoundingBox& box) const;
    ...
    [[nodiscard]] std::pair<double, double> ClosestParameters(const Line3D& other) const;
    ...
    [[nodiscard]] std::pair<Eigen::Vector3d, Eigen::Vector3d> ClosestPoints(const Line3D& other) const;
    ...
    /// the value of Length()
    [[nodiscard]] virtual double ClampParameter(double parameter) const { return parameter; }
    ...
    [[nodiscard]] virtual bool IsParameterValid(double parameter) const { return true; }
...
protected:
    ...
    [[nodiscard]] std::pair<double, double> SlabAABBBase(const AxisAlignedBoundingBox& box) const;
...
public:
    ...
    static Ray3D Through(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1) { return {p0, (p1 - p0).normalized()}; }
    ...
    /// \brief Gets the length of the line, which for lines and rays will return
    /// positive infinity, but for segments will return a finite positive value.
    [[nodiscard]] double Length() const override { return std::numeric_limits<double>::infinity(); }
    ...
    /// positive.
    [[nodiscard]] std::optional<double> IntersectionParameter(const Eigen::Hyperplane<double, 3>& plane) const override;
    ...
    /// double floating point precision will not intersect correctly by this
    /// method
    [[nodiscard]] std::optional<double> SlabAABB(const AxisAlignedBoundingBox& box) const override;
    ...
    /// will be made 0 and a positive value greater than Length() will take
    /// the value of Length()
    [[nodiscard]] double ClampParameter(double parameter) const override { return std::max(parameter, 0.); }

    /// \brief Verifies that a given parameter value is valid for the semantics
    /// of the line object. For lines, any parameter is valid, for rays any
    /// positive parameter is valid, and for segments any parameter between 0
    /// and the segment length is valid.
    [[nodiscard]] bool IsParameterValid(double parameter) const override { return parameter >= 0; }
};
...
public:
    ...
    static Segment3D Through(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1) { return {p0, p1}; }
    ...
    [[nodiscard]] Eigen::Vector3d MidPoint() const { return 0.5 * (origin() + end_point_); }
    ...
    [[nodiscard]] std::optional<double> ExactAABB(const AxisAlignedBoundingBox& box) const override;
