...
public:
    BallPivotingTriangle(BallPivotingVertexPtr vert0,
        ...
        : vert0_(vert0), vert1_(vert1), vert2_(vert2), ball_center_(std::move(ball_center)) {}
...
void BallPivotingEdge::AddAdjacentTriangle(const BallPivotingTrianglePtr& triangle) {
    if (triangle != triangle0_ && triangle != triangle1_) {
        if (triangle0_ == nullptr) {
            ...
            if (BallPivotingVertexPtr opp = GetOppositeVertex()) {
                ...
                Eigen::Vector3d pt_norm = source_->normal_ + target_->normal_ + opp->normal_;
                pt_norm /= pt_norm.norm();
                if (pt_norm.dot(tr_norm) < 0) {
                    std::swap(target_, source_);
                }
            } else {
                LOGE("GetOppositeVertex() returns nullptr.")
            }
        } else if (triangle1_ == nullptr) {
            triangle1_ = triangle;
            type_ = Type::Inner;
        } else {
            LOGD("!!! This case should not happen")
        }
    }
}
...
BallPivotingVertexPtr BallPivotingEdge::GetOppositeVertex() const {
    if (triangle0_ != nullptr) {
        if (triangle0_->vert0_->idx_ != source_->idx_ && triangle0_->vert0_->idx_ != target_->idx_) {
            return triangle0_->vert0_;
        } else if (triangle0_->vert1_->idx_ != source_->idx_ && triangle0_->vert1_->idx_ != target_->idx_) {
...
class BallPivoting {
public:
    explicit BallPivoting(const PointCloud& pcd) : has_normals_(pcd.HasNormals()), kdtree_(pcd) {
        ...
        for (size_t vidx = 0; vidx < pcd.points_.size(); ++vidx) {
            vertices.emplace_back(
                    new BallPivotingVertex(static_cast<int>(vidx), pcd.points_[vidx], pcd.normals_[vidx]));
        }
    }
    ...
    bool ComputeBallCenter(int vidx1, int vidx2, int vidx3, double radius, Eigen::Vector3d& center) {
        ...
        b = std::sqrt(b);
        c = std::sqrt(c);
        circ_radius2 = circ_radius2 / ((a + b + c) * (b + c - a) * (c + a - b) * (a + b - c));

        double height = radius * radius - circ_radius2;
        if (height >= 0.0) {
            Eigen::Vector3d tr_norm = (v2 - v1).cross(v3 - v1);
            tr_norm /= tr_norm.norm();
            Eigen::Vector3d pt_norm = vertices[vidx1]->normal_ + vertices[vidx2]->normal_ + vertices[vidx3]->normal_;
            pt_norm /= pt_norm.norm();
    ...
    static BallPivotingEdgePtr GetLinkingEdge(const BallPivotingVertexPtr& v0, const BallPivotingVertexPtr& v1) {
        for (BallPivotingEdgePtr edge0 : v0->edges_) {
            for (const BallPivotingEdgePtr& edge1 : v1->edges_) {
                if (edge0->source_->idx_ == edge1->source_->idx_ && edge0->target_->idx_ == edge1->target_->idx_) {
                    return edge0;
                }
            }
        }
    ...
    void CreateTriangle(const BallPivotingVertexPtr& v0,
        ...
        LOGD("[CreateTriangle] with v0.idx={}, v1.idx={}, v2.idx={}", v0->idx_, v1->idx_, v2->idx_)
        BallPivotingTrianglePtr triangle = std::make_shared<BallPivotingTriangle>(v0, v1, v2, center);
        ...
        if (face_normal.dot(v0->normal_) > -1e-16) {
            mesh_->triangles_.emplace_back(Eigen::Vector3i(v0->idx_, v1->idx_, v2->idx_));
        } else {
            mesh_->triangles_.emplace_back(Eigen::Vector3i(v0->idx_, v2->idx_, v1->idx_));
        }
        mesh_->triangle_normals_.push_back(face_normal);
    }

    static Eigen::Vector3d ComputeFaceNormal(const Eigen::Vector3d& v0,
    ...
    static bool IsCompatible(const BallPivotingVertexPtr& v0,
                             const BallPivotingVertexPtr& v1,
                             const BallPivotingVertexPtr& v2) {
        LOGD("[IsCompatible] v0.idx={}, v1.idx={}, v2.idx={}", v0->idx_, v1->idx_, v2->idx_)
        Eigen::Vector3d normal = ComputeFaceNormal(v0->point_, v1->point_, v2->point_);
        if (normal.dot(v0->normal_) < -1e-16) {
            normal *= -1;
        }
        bool ret = normal.dot(v0->normal_) > -1e-16 && normal.dot(v1->normal_) > -1e-16 &&
                   normal.dot(v2->normal_) > -1e-16;
        LOGD("[IsCompatible] returns = {}", ret)
        return ret;
    }

    BallPivotingVertexPtr FindCandidateVertex(const BallPivotingEdgePtr& edge,
                                              double radius,
                                              Eigen::Vector3d& candidate_center) {
        LOGD("[FindCandidateVertex] edge=({}, {}), radius={}", edge->source_->idx_, edge->target_->idx_, radius)
        ...
        if (opp == nullptr) {
            LOGE("edge->GetOppositeVertex() returns nullptr.")
            assert(opp == nullptr);
        }
        ...
        LOGD("[FindCandidateVertex] found {} potential candidates", indices.size())

        BallPivotingVertexPtr min_candidate = nullptr;
        double min_angle = 2 * M_PI;
        for (auto nbidx : indices) {
            LOGD("[FindCandidateVertex] nbidx {:d}", nbidx)
            ...
            if (candidate->idx_ == src->idx_ || candidate->idx_ == tgt->idx_ || candidate->idx_ == opp->idx_) {
                LOGD("[FindCandidateVertex] candidate {:d} is a triangle "
                     "vertex of the edge",
                     candidate->idx_)
                continue;
            }
            ...
            if (coplanar && (IntersectionTest::LineSegmentsMinimumDistance(mp, candidate->point_, src->point_,
                                                                           opp->point_) < 1e-12 ||
                             IntersectionTest::LineSegmentsMinimumDistance(mp, candidate->point_, tgt->point_,
                                                                           opp->point_) < 1e-12)) {
                LOGD("[FindCandidateVertex] candidate {:d} is intersecting "
            ...
            if (!ComputeBallCenter(src->idx_, tgt->idx_, candidate->idx_, radius, new_center)) {
                LOGD("[FindCandidateVertex] candidate {:d} can not compute "
                     "ball",
                     candidate->idx_)
                continue;
            }
            ...
            LOGD("[FindCandidateVertex] candidate {:d} cosinus={:f}", candidate->idx_, cosinus)
            ...
            if (angle >= min_angle) {
                LOGD("[FindCandidateVertex] candidate {:d} angle {:f} > "
                     "min_angle {:f}",
            ...
            for (auto nbidx2 : indices) {
                ...
                if ((new_center - nb->point_).norm() < radius - 1e-16) {
                    LOGD("[FindCandidateVertex] candidate {:d} not an empty "
                         "ball",
                         candidate->idx_)
                    empty_ball = false;
                    break;
            ...
            if (empty_ball) {
                LOGD("[FindCandidateVertex] candidate {:d} works", candidate->idx_)
                min_angle = angle;
        ...
        if (min_candidate == nullptr) {
            LOGD("[FindCandidateVertex] returns nullptr")
        } else {
            LOGD("[FindCandidateVertex] returns {:d}", min_candidate->idx_)
        }
        return min_candidate;
    }

    void ExpandTriangulation(double radius) {
        LOGD("[ExpandTriangulation] radius={}", radius)
        while (!edge_front_.empty()) {
            ...
            BallPivotingVertexPtr candidate = FindCandidateVertex(edge, radius, center);
            if (candidate == nullptr || candidate->type_ == BallPivotingVertex::Type::Inner ||
    ...
    bool TryTriangleSeed(const BallPivotingVertexPtr& v0,
                         ...
                         double radius,
                         Eigen::Vector3d& center) {
        LOGD("[TryTriangleSeed] v0.idx={}, v1.idx={}, v2.idx={}, "
             "radius={}",
        ...
        if (e0 != nullptr && e0->type_ == BallPivotingEdge::Type::Inner) {
            LOGD("[TryTriangleSeed] returns {} because e0 is inner edge", false)
            return false;
        }
        if (e1 != nullptr && e1->type_ == BallPivotingEdge::Type::Inner) {
            LOGD("[TryTriangleSeed] returns {} because e1 is inner edge", false)
            return false;
        }

        if (!ComputeBallCenter(v0->idx_, v1->idx_, v2->idx_, radius, center)) {
            LOGD("[TryTriangleSeed] returns {} could not compute ball "
                 "center",
                 false)
            return false;
        }
        ...
        for (const auto& nbidx : nb_indices) {
            ...
            if ((center - v->point_).norm() < radius - 1e-16) {
                LOGD("[TryTriangleSeed] returns {} computed ball is not "
                     "empty",
                     false)
                return false;
            }
        ...
        LOGD("[TryTriangleSeed] returns {}", true)
        return true;
    }

    bool TrySeed(BallPivotingVertexPtr& v, double radius) {
        LOGD("[TrySeed] with v.idx={}, radius={}", v->idx_, radius)
        std::vector<int> indices;
        std::vector<double> dists2;
        ...
        for (size_t nbidx0 = 0; nbidx0 < indices.size(); ++nbidx0) {
            ...
            Eigen::Vector3d center;
            for (size_t nbidx1 = nbidx0 + 1; nbidx1 < indices.size(); ++nbidx1) {
            ...
            if (candidate_vidx2 >= 0) {
                ...
                if (e1 != nullptr && e1->type_ != BallPivotingEdge::Type::Front) {
                    continue;
                }
                BallPivotingEdgePtr e2 = GetLinkingEdge(v, nb0);
                if (e2 != nullptr && e2->type_ != BallPivotingEdge::Type::Front) {
                    continue;
                }
                ...
                if (!edge_front_.empty()) {
                    LOGD("[TrySeed] edge_front_.size() > 0 => return "
                         "true")
                    return true;
                }
            }
    ...
    std::shared_ptr<TriangleMesh> Run(const std::vector<double>& radii) {
        if (!has_normals_) {
            LOGE("ReconstructBallPivoting requires normals")
        }
        ...
        for (double radius : radii) {
            LOGD("[Run] ################################")
            LOGD("[Run] change to radius {:.4f}", radius)
            if (radius <= 0) {
                LOGE("got an invalid, negative radius as parameter")
            }
            ...
            for (auto it = border_edges_.begin(); it != border_edges_.end();) {
                ...
                BallPivotingTrianglePtr triangle = edge->triangle0_;
                LOGD("[Run] try edge {:d}-{:d} of triangle {:d}-{:d}-{:d}", edge->source_->idx_, edge->target_->idx_,
                     triangle->vert0_->idx_, triangle->vert1_->idx_, triangle->vert2_->idx_)

                Eigen::Vector3d center;
                if (ComputeBallCenter(triangle->vert0_->idx_, triangle->vert1_->idx_, triangle->vert2_->idx_, radius,
                                      center)) {
                    LOGD("[Run]   yes, we can work on this")
                    std::vector<int> indices;
                    std::vector<double> dists2;
                    kdtree_.SearchRadius(center, radius, indices, dists2);
                    bool empty_ball = true;
                    for (auto idx : indices) {
                        if (idx != triangle->vert0_->idx_ && idx != triangle->vert1_->idx_ &&
                            idx != triangle->vert2_->idx_) {
                            LOGD("[Run]   but no, the ball is not empty")
                            empty_ball = false;
                            break;
                        }
                    }
                    ...
                    if (empty_ball) {
                        LOGD("[Run]   yeah, add edge to edge_front_: {:d}", edge_front_.size())
            ...
            LOGD("[Run] mesh_ has {:d} triangles", mesh_->triangles_.size())
            LOGD("[Run] ################################")
        }
        return mesh_;
    }
...
std::shared_ptr<TriangleMesh> TriangleMesh::CreateFromPointCloudBallPivoting(const PointCloud& pcd,
