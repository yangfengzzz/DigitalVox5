...
public:
    ...
    [[nodiscard]] Eigen::Vector3d GetVoxelCenterCoordinate(const Eigen::Vector3i &idx) const {
        ...
        if (it != voxels_.end()) {
            ...
            return ((voxel.grid_index_.cast<double>() + Eigen::Vector3d(0.5, 0.5, 0.5)) * voxel_size_) + origin_;
        } else {
    ...
    VoxelGrid &CarveDepthMap(const Image &depth_map,
    ...
    /// \param camera_parameter Input Camera Parameters.
    /// \param keep_voxels_outside_image Project all voxels to a valid location.
    VoxelGrid &CarveSilhouette(const Image &silhouette_mask,
                               const camera::PinholeCameraParameters &camera_parameter,
                               bool keep_voxels_outside_image);
    ...
    static std::shared_ptr<VoxelGrid> CreateFromPointCloudWithinBounds(const PointCloud &input,
                                                                       double voxel_size,
    ...
    /// \param min_bound Minimum boundary point for the VoxelGrid to create.
    /// \param max_bound Maximum boundary point for the VoxelGrid to create.
    static std::shared_ptr<VoxelGrid> CreateFromTriangleMeshWithinBounds(const TriangleMesh &input,
                                                                         double voxel_size,
                                                                         const Eigen::Vector3d &min_bound,
                                                                         const Eigen::Vector3d &max_bound);
...
public:
    void Add(const Eigen::Vector3i &voxel_index) {
        if (num_of_points_ > 0 && voxel_index != voxel_index_) {
            LOGW("Tried to aggregate ColorVoxel with different "
                 "voxel_index")
        }
