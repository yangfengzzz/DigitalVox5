...
void cloth::SwSolver::removeCloth(Cloth* cloth) {
    ...
    // Remove from mSimulatedCloths
    {
        ...
        while (tIt != tEnd && tIt->mCloth != &swCloth) ++tIt;
...
int cloth::SwSolver::getNumCloths() const { return mCloths.size(); }
...
bool cloth::SwSolver::beginSimulation(float dt) {
    if (mSimulatedCloths.empty()) return false;
...
int cloth::SwSolver::getSimulationChunkCount() const { return static_cast<int>(mSimulatedCloths.size()); }

void cloth::SwSolver::interCollision() {
    if (!mInterCollisionIterations || mInterCollisionDistance == 0.0f) return;
    if (mInterCollisionFilter == nullptr) {
        NV_CLOTH_LOG_WARNING(
                "Inter collision will not work unless an inter collision filter is set using "
                "Solver::setInterCollisionFilter.");
        return;
    }
    ...
    if (mInterCollisionScratchMemSize < requiredTempMemorySize) {
        ...
        mInterCollisionScratchMem =
                NV_CLOTH_ALLOC(requiredTempMemorySize, "cloth::SwSolver::mInterCollisionScratchMem");
...
void cloth::SwSolver::SimulatedCloth::Simulate() {
    ...
    if (mParent->mCurrentDt == 0.0f) return;
