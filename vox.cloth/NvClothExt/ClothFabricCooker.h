...
PX_INLINE ClothFabricPhase::ClothFabricPhase(ClothFabricPhaseType::Enum type, physx::PxU32 index)
    : phaseType(type), setIndex(index) {}
...
PX_INLINE void ClothFabricDesc::setToDefault() { memset(this, 0, sizeof(ClothFabricDesc)); }

PX_INLINE bool ClothFabricDesc::isValid() const {
    return nbParticles && nbPhases && phases && restvalues && nbSets && sets && indices &&
           (!nbTethers || (tetherAnchors && tetherLengths)) && (!nbTriangles || triangles);
}
...
public:
    ...
    The gravity vector should point in the direction gravity will be pulling towards in the most common situation/at
    rest. e.g. For flags it might be beneficial to set the gravity horizontal if they are cooked in landscape
    orientation, as a flag will hang in portrait orientation at rest. \param useGeodesicTether A flag to indicate
    whether to compute geodesic distance for tether constraints. \note The geodesic option for tether only works for
    manifold input.  For non-manifold input, a simple Euclidean distance will be used. For more detailed cooker status
    for such cases, try running ClothGeodesicTetherCooker directly.
    */
...
NvClothCookFabricFromMesh(nv::cloth::Factory* factory,
                          ...
                          nv::cloth::Vector<int32_t>::Type* phaseTypes = nullptr,
