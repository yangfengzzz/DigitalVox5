...
struct StridedData {
    ...
    */
    ...
    PX_INLINE const TDataType& at(physx::PxU32 idx) const {
        ...
        if (theStride == 0) theStride = sizeof(TDataType);
...
PX_INLINE void ClothMeshDesc::setToDefault() { *this = ClothMeshDesc(); }
...
PX_INLINE bool ClothMeshDesc::isValid() const {
    ...
    if (points.count > 0xffff && flags & MeshFlag::e16_BIT_INDICES) return false;
    if (!points.data) return false;
    if (points.stride < sizeof(physx::PxVec3))  // should be at least one point
        return false;

    if (invMasses.data && invMasses.stride < sizeof(float)) return false;
    if (invMasses.data && invMasses.count != points.count) return false;

    if (!triangles.count && !quads.count)  // no support for non-indexed mesh
        return false;
    if (triangles.count && !triangles.data) return false;
    if (quads.count && !quads.data) return false;

    physx::PxU32 indexSize = (flags & MeshFlag::e16_BIT_INDICES) ? sizeof(physx::PxU16) : sizeof(physx::PxU32);
    if (triangles.count && triangles.stride < indexSize * 3) return false;
    if (quads.count && quads.stride < indexSize * 4) return false;
