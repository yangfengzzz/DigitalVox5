...
/** \file Callbacks.h
        ...
        The other functions defined in this header are used to access the functionality provided by the callbacks, and
...
/** \brief Initialize the library by passing in callback functions.
        ...
        */
NV_CLOTH_API(void)
InitializeNvCloth(physx::PxAllocatorCallback* allocatorCallback,
...
// ASSERT
NV_CLOTH_API(nv::cloth::PxAssertHandler*)
GetNvClothAssertHandler();  // This function needs to be exposed to properly inline asserts outside this dll
...
#define PX_CODE_ANALYSIS_ASSUME(exp) \
    __analysis_assume(!!(exp))  // This macro will be used to get rid of analysis warning messages if a NV_CLOTH_ASSERT
                                // is used to "guard" illegal mem access, for example.
#else
#define PX_CODE_ANALYSIS_ASSUME(exp)
#endif
#define NV_CLOTH_ASSERT(exp)                                                                                         \
    {                                                                                                                \
        ...
        ((void)((!!(exp)) ||                                                                                         \
                (!_ignore && ((*nv::cloth::GetNvClothAssertHandler())(#exp, __FILE__, __LINE__, _ignore), false)))); \
...
#define NV_CLOTH_ASSERT_WITH_MESSAGE(message, exp)                                                          \
    {                                                                                                       \
        ...
        ((void)((!!(exp)) ||                                                                                \
                (!_ignore &&                                                                                \
                 ((*nv::cloth::GetNvClothAssertHandler())(message, __FILE__, __LINE__, _ignore), false)))); \
        PX_CODE_ANALYSIS_ASSUME(exp);                                                                       \
    }
#endif
...
public:
    PX_FORCE_INLINE NvClothProfileScoped(const char* eventName,
                                         bool detached,
                                         uint64_t contextId,
                                         const char* fileName,
                                         int lineno,
                                         physx::PxProfilerCallback* callback)
        : mCallback(callback) {
        PX_UNUSED(fileName);
        PX_UNUSED(lineno);
...
#define NV_CLOTH_PROFILE_ZONE(x, y)                                                               \
    nv::cloth::NvClothProfileScoped PX_CONCAT(_scoped, __LINE__)(x, false, y, __FILE__, __LINE__, \
                                                                 nv::cloth::GetNvClothProfiler())
#define NV_CLOTH_PROFILE_START_CROSSTHREAD(x, y) \
    (GetNvClothProfiler() != nullptr ? GetNvClothProfiler()->zoneStart(x, true, y) : nullptr)
#define NV_CLOTH_PROFILE_STOP_CROSSTHREAD(profilerData, x, y) \
    if (GetNvClothProfiler()) GetNvClothProfiler()->zoneEnd(profilerData, x, true, y)
#else
