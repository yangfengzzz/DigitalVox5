...
public:
    ...
    [[nodiscard]] virtual Fabric& getFabric() const = 0;
    ...
    /** \brief Returns the simulation particles of the current frame.
            Each PxVec4 element contains the particle position in the XYZ components and the inverse mass in the W
       component. The returned memory may be overwritten (to change attachment point locations for animation for
       example). Setting the inverse mass to 0 locks the particle in place.
            */
    ...
    /** \brief Returns the simulation particles of the current frame, read only.
            ...
            This version is preferred as it doesn't wake up the cloth to account for the possibility that particles were
       changed.
    */
    [[nodiscard]] virtual MappedRange<const physx::PxVec4> getCurrentParticles() const = 0;
    ...
    */
    [[nodiscard]] virtual MappedRange<const physx::PxVec4> getPreviousParticles() const = 0;
    ...
    /** \brief Set the rotation of the local space simulation after next call to simulate().
            ...
            The applied force is proportional to the value set with Cloth::setAngularInertia() and
       Cloth::setCentrifugalInertia().
            */
    ...
    [[nodiscard]] virtual const physx::PxVec3& getTranslation() const = 0;
    /** \brief Returns the current rotation value that was set using setRotation().*/
    [[nodiscard]] virtual const physx::PxQuat& getRotation() const = 0;

    /** \brief Set inertia derived from setTranslation() and setRotation() to zero (once).*/
    virtual void clearInertia() = 0;

    /** \brief Adjust the position of the cloth without affecting the dynamics (to call after a world origin shift, for
     * example). */
    virtual void teleport(const physx::PxVec3& delta) = 0;

    /** \brief Adjust the position and rotation of the cloth without affecting the dynamics (to call after a world
       origin shift, for example). The velocity will be set to zero this frame, unless setTranslation/setRotation is
       called with a different value after this function is called. The correct order to use this is: \code
    ...
    /** \brief Don't recalculate the velocity based on the values provided by setTranslation and setRotation for one
       frame (so it acts as if the velocity was the same as last frame). This is useful when the cloth is moving while
       teleported, but the integration of the cloth position for that frame is already included in the teleport.
            Example:
            \code
            pos += velocity * dt;
            rot += 0.5 * angularVelocity * rot * dt;
    ...
    */
    ...
    [[nodiscard]] virtual physx::PxVec3 getDamping() const = 0;
    ...
    [[nodiscard]] virtual physx::PxVec3 getLinearDrag() const = 0;
    virtual void setAngularDrag(const physx::PxVec3&) = 0;
    [[nodiscard]] virtual physx::PxVec3 getAngularDrag() const = 0;
    ...
    [[nodiscard]] virtual physx::PxVec3 getLinearInertia() const = 0;
    ...
    [[nodiscard]] virtual physx::PxVec3 getAngularInertia() const = 0;
    ...
    [[nodiscard]] virtual physx::PxVec3 getCentrifugalInertia() const = 0;
    ...
    [[nodiscard]] virtual float getSolverFrequency() const = 0;

    // damp, drag, stiffness exponent per second
    virtual void setStiffnessFrequency(float) = 0;
    [[nodiscard]] virtual float getStiffnessFrequency() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getAccelerationFilterWidth() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getNumSpheres() const = 0;
    ...
    */
    ...
    [[nodiscard]] virtual uint32_t getNumCapsules() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getNumPlanes() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getNumConvexes() const = 0;
    ...
    virtual void setTriangles(Range<const physx::PxVec3> startTriangles,
    ...
    [[nodiscard]] virtual bool isContinuousCollisionEnabled() const = 0;
    ...
    [[nodiscard]] virtual float getCollisionMassScale() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getNumVirtualParticles() const = 0;
    [[nodiscard]] virtual uint32_t getNumVirtualParticleWeights() const = 0;
    ...
    [[nodiscard]] virtual float getTetherConstraintScale() const = 0;
    ...
    */
    ...
    [[nodiscard]] virtual float getTetherConstraintStiffness() const = 0;
    ...
    */
    ...
    [[nodiscard]] virtual uint32_t getNumMotionConstraints() const = 0;
    virtual void setMotionConstraintScaleBias(float scale, float bias) = 0;
    [[nodiscard]] virtual float getMotionConstraintScale() const = 0;
    [[nodiscard]] virtual float getMotionConstraintBias() const = 0;
    virtual void setMotionConstraintStiffness(float stiffness) = 0;
    [[nodiscard]] virtual float getMotionConstraintStiffness() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getNumSeparationConstraints() const = 0;
    ...
    [[nodiscard]] virtual uint32_t getNumParticleAccelerations() const = 0;
    ...
    [[nodiscard]] virtual float getFluidDensity() const = 0;
    ...
    [[nodiscard]] virtual float getSelfCollisionDistance() const = 0;
    ...
    [[nodiscard]] virtual float getSelfCollisionStiffness() const = 0;
...
inline MappedRange<const physx::PxVec4> readCurrentParticles(const Cloth& cloth) { return cloth.getCurrentParticles(); }
