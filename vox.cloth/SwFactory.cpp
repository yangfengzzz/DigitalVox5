...
cloth::Fabric* cloth::SwFactory::createFabric(uint32_t numParticles,
    ...
    return NV_CLOTH_NEW(SwFabric)(*this, numParticles, phaseIndices, sets, restvalues, stiffnessValues, indices,
...
cloth::Solver* cloth::SwFactory::createSolver() { return NV_CLOTH_NEW(SwSolver)(); }

cloth::Cloth* cloth::SwFactory::clone(const Cloth& cloth) {
    if (cloth.getFactory().getPlatform() != Platform::CPU) return cloth.clone(*this);  // forward to CuCloth
...
void cloth::SwFactory::extractFabricData(const Fabric& fabric,
    ...
    for (sIt = swFabric.mSets.begin(); ++sIt != sEnd;) {
        ...
        for (; rIt != rEnd; ++rIt, ++stIt) {
            ...
            if (std::max(i0, i1) >= swFabric.mNumParticles) continue;

            if (!restvalues.empty()) *rDst++ = *rIt;
            if (!stiffnessValues.empty()) *stDst++ = *stIt;
        ...
        if (!sets.empty()) *sDst++ = numConstraints;
    }

    for (uint32_t i = 0; !anchors.empty(); ++i, anchors.popFront()) anchors.front() = swFabric.mTethers[i].mAnchor;
    ...
    for (uint32_t i = 0; !triangles.empty(); ++i, triangles.popFront()) triangles.front() = swFabric.mTriangles[i];
}

void cloth::SwFactory::extractCollisionData(const Cloth& cloth,
                                            Range<PxVec4> spheres,
...
void cloth::SwFactory::extractVirtualParticles(const Cloth& cloth,
    ...
    if (indices.size() == numIndices) {
        // convert indices
        auto* iDestIt = reinterpret_cast<Vec4u*>(indices.begin());
        ...
        auto numParticles = uint32_t(swCloth.mCurParticles.size());
