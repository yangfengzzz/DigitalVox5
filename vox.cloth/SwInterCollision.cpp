...
#include "vox.cloth/BoundingBox.h"
#include "vox.cloth/foundation/PxBounds3.h"
...
#include "vox.cloth/SwCollisionHelpers.h"
...
void radixSort(const uint32_t* first, const uint32_t* last, uint32_t* out) {
    ...
    for (uint32_t i = 0; i != n; ++i) indices[1][histograms[0][0xff & first[i]]++] = i;
...
template <typename T4f>
cloth::SwInterCollision<T4f>::SwInterCollision(const cloth::SwInterCollisionData* instances,
                                               uint32_t n,
                                               float colDist,
                                               float stiffness,
                                               uint32_t iterations,
    ...
    : mInstances(instances),
      mNumInstances(n),
      mClothIndices(nullptr),
      mParticleIndices(nullptr),
      mNumParticles(0),
      mTotalParticles(0),
      mFilter(filter),
      mAllocator(alloc) {
    ...
    for (uint32_t i = 0; i < n; ++i) mTotalParticles += instances[i].mNumParticles;
}

template <typename T4f>
cloth::SwInterCollision<T4f>::~SwInterCollision() = default;
...
template <typename T4f>
uint32_t calculatePotentialColliders(const cloth::SwInterCollisionData* cBegin,
                                     const cloth::SwInterCollisionData* cEnd,
                                     const T4f& colDist,
                                     uint16_t* clothIndices,
                                     uint32_t* particleIndices,
                                     cloth::BoundingBox<T4f>& bounds,
                                     uint32_t* overlapMasks,
                                     cloth::InterCollisionFilter filter,
                                     cloth::SwKernelAllocator& allocator) {
    using namespace cloth;

    typedef BoundingBox<T4f> BoundingBox;

    uint32_t numParticles = 0;
    const auto numCloths = uint32_t(cEnd - cBegin);

    // bounds of each cloth objects in world space
    auto* const clothBounds = static_cast<BoundingBox*>(allocator.allocate(numCloths * sizeof(BoundingBox)));
    auto* const overlapBounds = static_cast<BoundingBox*>(allocator.allocate(numCloths * sizeof(BoundingBox)));

    // union of all cloth world bounds
    BoundingBox totalClothBounds = emptyBounds<T4f>();

    auto* sortedIndices = static_cast<uint32_t*>(allocator.allocate(numCloths * sizeof(uint32_t)));
    ...
    for (uint32_t i = 0; i < numCloths; ++i) {
        ...
        for (uint32_t j = startIndex; j < numCloths; ++j) {
            ...
            if (array(clothBounds[sortedIndices[j]].mLower)[sweepAxis] > axisMax) break;

            const SwInterCollisionData& b = cBegin[sortedIndices[j]];

            // check if collision between these shapes is filtered
            if (!filter(a.mUserData, b.mUserData)) continue;
            ...
            if (!isEmptyBounds(iBounds)) overlapBounds[numOverlaps++] = iBounds;
        }
        ...
        for (uint32_t k = 0; k < a.mNumParticles; ++k) {
            ...
            for (const BoundingBox *oIt = overlapBounds, *oEnd = overlapBounds + numOverlaps; oIt != oEnd; ++oIt) {
                // point in box test
                if (anyGreater(oIt->mLower, p) != 0) continue;
                if (anyGreater(p, oIt->mUpper) != 0) continue;
                ...
                *qIt = rotate(xform, impulse);
                ...
                ++numParticles;
                break;  // the particle only has to be inside one of the bounds, it doesn't matter if they are in more
                        // than one
            }
        }
    }
...
void cloth::SwInterCollision<T4f>::operator()() {
    ...
    for (uint32_t k = 0; k < mNumIterations; ++k) {
        ...
        // calculate potentially colliding set (based on bounding boxes)
        {
            ...
            mNumParticles = calculatePotentialColliders(mInstances, mInstances + mNumInstances, mCollisionDistance,
                                                        mClothIndices, mParticleIndices, bounds, mOverlapMasks, mFilter,
                                                        mAllocator);
        }
        ...
        if (mNumParticles) {
            ...
            auto* __restrict sortedIndices = reinterpret_cast<uint32_t*>(buffer);
            ...
            // sort keys
            for (uint32_t i = 0; i < mNumParticles; ++i) sortedKeys[i] = keys[sortedIndices[i]];
        ...
        */
        ...
        // transform back to local space
        {
            ...
            auto lastCloth = uint16_t(0xffff);
...
size_t cloth::SwInterCollision<T4f>::estimateTemporaryMemory(SwInterCollisionData* cloths, uint32_t n) {
    ...
    uint32_t numParticles = 0;
    for (uint32_t i = 0; i < n; ++i) numParticles += cloths[i].mNumParticles;
...
void cloth::SwInterCollision<T4f>::collideParticle(uint32_t index) {
    ...
    if ((1 << clothIndex) & ~mClothMask) return;
    ...
    if (allGreater(distSqr, mCollisionSquareDistance)) return;
...
template <typename T4f>
void cloth::SwInterCollision<T4f>::collideParticles(const uint32_t* keys,
...

    {
        ...
        kFirst[0] = kIt;
        while (*kIt < lastKey) ++kIt;
        kLast[0] = kIt;

        for (uint32_t k = 1; k < 5; ++k) {
            for (uint32_t n = firstKey + keyOffsets[k]; *kIt < n;) ++kIt;
            kFirst[k] = kIt;

            for (uint32_t n = lastKey + keyOffsets[k]; *kIt < n;) ++kIt;
            kLast[k] = kIt;

            // jump forward once to second column to go from cell offset 1 to 2 quickly
            if (firstColumnSize) kIt = keys + firstColumnSize;
            firstColumnSize = 0;
        }
    }
    ...
    for (; iIt != iEnd; ++iIt, ++kFirst[0]) {
        ...
        while (*kLast[0] < lastKey) ++kLast[0];

        // process potential colliders of same cell
        jEnd = indices + (kLast[0] - keys);
        for (jIt = iIt + 1; jIt != jEnd; ++jIt) collideParticle(*jIt);

        // process neighbor cells
        for (uint32_t k = 1; k < 5; ++k) {
            // scan forward start point
            for (uint32_t n = firstKey + keyOffsets[k]; *kFirst[k] < n;) ++kFirst[k];

            // scan forward end point
            for (uint32_t n = lastKey + keyOffsets[k]; *kLast[k] < n;) ++kLast[k];

            // process potential colliders
            jEnd = indices + (kLast[k] - keys);
            for (jIt = indices + (kFirst[k] - keys); jIt != jEnd; ++jIt) collideParticle(*jIt);
        }
