...
MutexUnixImpl* getMutex(MutexImpl* impl) { return reinterpret_cast<MutexUnixImpl*>(impl); }
}  // namespace
...
MutexImpl::~MutexImpl() { pthread_mutex_destroy(&getMutex(this)->lock); }
...
#if PX_DEBUG
    if (success) getMutex(this)->owner = Thread::getId();
#endif
    return success;
}
...
uint32_t MutexImpl::getSize() { return sizeof(MutexUnixImpl); }
