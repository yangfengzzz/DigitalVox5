...
#include "vox.cloth/NvClothExt/ClothMeshQuadifier.h"
...
namespace {
struct UniqueEdge {
    PX_FORCE_INLINE bool operator()(const UniqueEdge& e1, const UniqueEdge& e2) const { return e1 < e2; }
...
struct SortHiddenEdges {
    ...
    bool operator()(PxU32 a, PxU32 b) const { return mUniqueEdges[a].maxAngle < mUniqueEdges[b].maxAngle; }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename T>
void copyIndices(const ClothMeshDesc& desc,
                 nv::cloth::Vector<PxU32>::Type& triangles,
                 nv::cloth::Vector<PxU32>::Type& quads) {
    triangles.resize(desc.triangles.count * 3);
    PxStrideIterator<const T> tIt =
            PxMakeIterator(reinterpret_cast<const T*>(desc.triangles.data), desc.triangles.stride);
    for (PxU32 i = 0; i < desc.triangles.count; ++i, ++tIt)
        for (PxU32 j = 0; j < 3; ++j) triangles[i * 3 + j] = tIt.ptr()[j];
    ...
    for (PxU32 i = 0; i < desc.quads.count; ++i, ++qIt)
        for (PxU32 j = 0; j < 4; ++j) quads[i * 4 + j] = qIt.ptr()[j];
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void computeUniqueEdges(nv::cloth::Vector<UniqueEdge>::Type& uniqueEdges,
                        const PxVec3* positions,
                        const nv::cloth::Vector<PxU32>::Type& triangles) {
    ...
    for (PxU32 i = 0; i < triangles.size(); i += 3) {
        ...
        for (PxU32 j = 0; j < 3; j++) {
            edges[j] = UniqueEdge(triangles[i + indexMap[j][0]], triangles[i + indexMap[j][1]],
                                  triangles[i + indexMap[j][2]]);
        ...
        PxU32 longest = 0;
        for (PxU32 j = 1; j < 3; j++) {
            if (edgeLengths[j] > edgeLengths[longest]) longest = j;
        }
    ...
    while (readEnd < uniqueEdges.size()) {
        while (readEnd < uniqueEdges.size() && uniqueEdges[readStart] == uniqueEdges[readEnd]) readEnd++;
        ...
        for (PxU32 i = 1; i < count; i++) uniqueEdge.isQuadDiagonal &= uniqueEdges[readStart + i].isQuadDiagonal;
...
void refineUniqueEdges(nv::cloth::Vector<UniqueEdge>::Type& uniqueEdges, const PxVec3* positions) {
    ...
    for (unsigned int hideEdge : hideEdges) {
        ...
        PxU32 indices[5] = {uniqueEdge.vertex0, uniqueEdge.vertex2, uniqueEdge.vertex1, uniqueEdge.vertex3,
                            uniqueEdge.vertex0};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void quadifyTriangles(const nv::cloth::Vector<UniqueEdge>::Type& uniqueEdges,
                      nv::cloth::Vector<PxU32>::Type& triangles,
                      nv::cloth::Vector<PxU32>::Type& quads) {
    ...
    // compute adjacency from each edge to triangle, the value also encodes which side of the triangle this edge belongs
    // to
    ...
    for (PxU32 i = 0; i < uniqueEdges.size(); i++) {
        ...
        if (edge.isQuadDiagonal) {
            PxU32 vi = valency[i];
            if ((valency[i + 1] - vi) != 2) continue;  // we do not quadify around non-manifold edges
...
bool ClothMeshQuadifierImpl::quadify(const ClothMeshDesc& desc) {
    ...
    for (PxU32 i = 0; i < desc.points.count; ++i) particles[i] = *pIt++;
...
///////////////////////////////////////////////////////////////////////////////
ClothMeshDesc ClothMeshQuadifierImpl::getDescriptor() const {
