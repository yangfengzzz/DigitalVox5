...
cloth::SwFabric::SwFabric(SwFactory& factory,
    ...
    for (iIt = triangles.begin(); iIt != iEnd; ++iIt) mTriangles.pushBack(uint16_t(*iIt));
...
cloth::Factory& cloth::SwFabric::getFactory() const { return mFactory; }

uint32_t cloth::SwFabric::getNumPhases() const { return uint32_t(mPhases.size()); }

uint32_t cloth::SwFabric::getNumRestvalues() const { return mOriginalNumRestvalues; }

uint32_t cloth::SwFabric::getNumStiffnessValues() const { return mStiffnessValues.size() ? mOriginalNumRestvalues : 0; }

uint32_t cloth::SwFabric::getNumSets() const { return uint32_t(physx::PxMax(0, static_cast<int>(mSets.size() - 1))); }

uint32_t cloth::SwFabric::getNumIndices() const { return 2 * mOriginalNumRestvalues; }

uint32_t cloth::SwFabric::getNumParticles() const { return mNumParticles; }

uint32_t cloth::SwFabric::getNumTethers() const { return uint32_t(mTethers.size()); }

uint32_t cloth::SwFabric::getNumTriangles() const { return uint32_t(mTriangles.size()) / 3; }

void cloth::SwFabric::scaleRestvalues(float scale) {
    RestvalueContainer::Iterator rIt, rEnd = mRestvalues.end();
    for (rIt = mRestvalues.begin(); rIt != rEnd; ++rIt) *rIt *= scale;
}

void cloth::SwFabric::scaleTetherLengths(float scale) { mTetherLengthScale *= scale; }
