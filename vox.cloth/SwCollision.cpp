...
#include "vox.cloth/IterationState.h"
#include "vox.cloth/PointInterpolator.h"
...
void generateSpheres(T4f* dIt, const SrcIterator& src, uint32_t count) {
    ...
    for (SrcIterator sIt = src; 0 < count--; ++sIt, ++dIt) *dIt = max(sMinusFloatMaxXYZ, *sIt);  // clamp radius to 0
}

void generateCones(cloth::ConeData* dst,
                   const cloth::SphereData* sourceSpheres,
                   const cloth::IndexPair* capsuleIndices,
                   uint32_t numCones) {
    ...
    for (const cloth::IndexPair *iIt = capsuleIndices, *iEnd = iIt + numCones; iIt != iEnd; ++iIt, ++cIt) {
        ...
        if (sqrConeHalfLength <= 0.0f) invAxisHalfLength = invConeHalfLength = 0.0f;
...
void generatePlanes(T4f* dIt, const SrcIterator& src, uint32_t count) {
    // have to copy out iterator to ensure alignment is maintained
    for (SrcIterator sIt = src; 0 < count--; ++sIt, ++dIt) *dIt = *sIt;
}
...
void cloth::SwCollision<T4f>::operator()(const IterationState<T4f>& state) {
    ...
    if (!mClothData.mNumSpheres) return;
    ...
    if (mPrevData.mSpheres) ps::swap(mCurData, mPrevData);
}
...
void cloth::SwCollision<T4f>::buildSphereAcceleration(const SphereData* sIt) {
    ...
    for (; sIt != sEnd; ++sIt, mask <<= 1) {
        ...
        T4i first = intFloor(min(max((sphere - radius) * mGridScale + mGridBias, gSimd4fZero),
                                 sGridLength));  // use both min and max to deal with bad grid scales
        ...
        for (uint32_t i = 0; i < 3; ++i, firstIt += sGridSize, lastIt += sGridSize) {
            ...
            for (int j = firstIdx[i]; j <= maxIndex; ++j) firstIt[j] |= mask;

            // mark the sphere and everything to the left
            for (int j = lastIdx[i]; j >= 0; --j) lastIt[j] |= mask;
        }
    }
}
...
void cloth::SwCollision<T4f>::buildConeAcceleration() {
    ...
    for (uint32_t coneMask = 0x1; coneIt != coneEnd; ++coneIt, coneMask <<= 1) {
        if (coneIt->radius == 0.0f) continue;
...
void cloth::SwCollision<T4f>::mergeAcceleration(uint32_t* firstIt) {
    ...
    for (; firstIt != firstEnd; ++firstIt, ++lastIt) *firstIt &= *lastIt;
}
...
bool cloth::SwCollision<T4f>::buildAcceleration() {
    ...
    if (!allGreaterEqual(edgeLength, gSimd4fZero)) return false;
...
template <typename T4f>
FORCE_INLINE typename cloth::SwCollision<T4f>::ShapeMask& cloth::SwCollision<T4f>::ShapeMask::operator=(
        const ShapeMask& right) {
    mCones = right.mCones;
...
template <typename T4f>
FORCE_INLINE typename cloth::SwCollision<T4f>::ShapeMask cloth::SwCollision<T4f>::getShapeMask(
        const T4f& position, const T4i* __restrict sphereGrid, const T4i* __restrict coneGrid) {
...
template <typename T4f>
FORCE_INLINE typename cloth::SwCollision<T4f>::ShapeMask cloth::SwCollision<T4f>::getShapeMask(
        const T4f* __restrict positions) const {
...
template <typename T4f>
FORCE_INLINE typename cloth::SwCollision<T4f>::ShapeMask cloth::SwCollision<T4f>::getShapeMask(
        const T4f* __restrict prevPos, const T4f* __restrict curPos) const {
...
template <typename T4f>
FORCE_INLINE void cloth::SwCollision<T4f>::collideSpheres(const T4i& sphereMask,
                                                          const T4f* positions,
    ...
    while (mask) {
        ...
        T4f contactMask;
        if (!anyGreater(gSimd4fZero, negativeScale, contactMask)) continue;
...
template <typename T4f>
FORCE_INLINE typename cloth::SwCollision<T4f>::T4i cloth::SwCollision<T4f>::collideCones(
    ...
    while (mask) {
        ...
        if (!anyTrue(contactMask)) continue;
...
template <typename T4f>
FORCE_INLINE void cloth::SwCollision<T4f>::collideSpheres(const T4i& sphereMask,
                                                          const T4f* __restrict prevPos,
                                                          T4f* __restrict curPos,
                                                          ImpulseAccumulator& accum) const {
...
template <typename T4f>
PX_INLINE void calculateFrictionImpulse(const T4f& deltaX,
                                        const T4f& deltaY,
                                        const T4f& deltaZ,
                                        const T4f& velX,
                                        const T4f& velY,
...
void cloth::SwCollision<T4f>::collideParticles() {
    ...
    for (; pIt < pEnd; pIt += 16, prevIt += 16) {
        ...
        T4f mask;
        if (!anyGreater(accum.mNumCollisions, gSimd4fEpsilon, mask)) continue;
...
void cloth::SwCollision<T4f>::collideConvexes(const IterationState<T4f>& state) {
    if (!mClothData.mNumConvexes) return;
...
template <typename T4f>
void cloth::SwCollision<T4f>::collideConvexes(const T4f* __restrict planes,
                                              T4f* __restrict curPos,
                                              ImpulseAccumulator& accum) {
    ...
    if (allEqual(result, gSimd4iZero)) return;
    ...
    for (; cIt != cEnd; ++cIt) {
        ...
        if (!anyEqual(mask4 & result, mask4, mask4)) continue;
...
void cloth::SwCollision<T4f>::collideTriangles(const IterationState<T4f>& state) {
    ...
    if (state.mRemainingIterations != 1) {
        ...
        LerpIterator<T4f, UnalignedIterator<T4f, 3>> triangleIter(mClothData.mStartCollisionTriangles, targetTriangles,
                                                                  state.getCurrentAlpha());
...
template <typename T4f>
void cloth::SwCollision<T4f>::collideTriangles(const TriangleData* __restrict triangles,
                                               T4f* __restrict curPos,
                                               ImpulseAccumulator& accum) {
    ...
    T4f mask;
    if (!anyGreater(gSimd4fZero, normalD, mask)) return;
