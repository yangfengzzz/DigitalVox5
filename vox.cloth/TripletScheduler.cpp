...
#include "vox.cloth/TripletScheduler.h"
...
void cloth::TripletScheduler::simd(uint32_t numParticles, uint32_t simdWidth) {
    ...
    for (TripletIter tIt = mTriplets.begin(), tEnd = mTriplets.end(); tIt != tEnd; tIt++) {
        ...
        if (tSwap == tEnd) {
            ...
            amountOfPaddingNeeded +=
                    (static_cast<int>(simdWidth) - (setSize % static_cast<int>(simdWidth))) %
                    static_cast<int>(simdWidth);  // last modulo avoids adding padding when setSize%simdWidth==0
            setSize = 0;
            ...
            std::iter_swap(
                    tIt, tSwap);  // Place triplet at the end of the current set, so that they in sequence in mTriplets
            setSize++;
        }
    }
    if (setSize) {
        ...
        mSetSizes.pushBack(setSize);
        amountOfPaddingNeeded +=
                (static_cast<int>(simdWidth) - (setSize % static_cast<int>(simdWidth))) % static_cast<int>(simdWidth);
    }
    ...
    {
        Vec4us paddingDummy(static_cast<uint16_t>(numParticles), static_cast<uint16_t>(numParticles) + 1,
                            static_cast<uint16_t>(numParticles) + 2, 0);
        ...
        for (; setIt < setEnd; ++setIt) {
            ...
            unsigned int padding = (static_cast<int>(simdWidth) - (*setIt % static_cast<int>(simdWidth))) %
                                   static_cast<int>(simdWidth);
    ...
    AdjacencyQuerier(nv::cloth::Vector<nv::cloth::Vec4u>::Type const& triplets, int particleCount) {
        {
            ...
            adjacencyCount.resize(particleCount + 1,
...
void cloth::TripletScheduler::warp(uint32_t numParticles, uint32_t warpWidth) {
    ...
    for (tIt = mTriplets.begin(); tIt != tEnd; ++tIt, ++tripletIndex) {
        ...
        for (uint32_t setIndex = 0; setIndex < numSets && minAdditionalReplays; ++setIndex) {
            ...
            if (set.mMark == tripletIndex) continue;  // triplet collision (this set contains an adjacent triplet)
    ...
    for (tIt = mTriplets.begin(), tripletIndex = 0; tIt != tEnd; ++tIt, ++iIt) triplets[--setOffsets[*iIt]] = *tIt;
...
cloth::TripletSchedulerTestInterface::~TripletSchedulerTestInterface() { delete mScheduler; }
...
NvClothCreateTripletScheduler(nv::cloth::Range<const uint32_t[4]> triplets) {
