...
inline Range<const PhaseConfig> getPhaseConfigs(const SwCloth& cloth) { return makeRange(cloth.mPhaseConfigs); }
...
typename DstFactoryType::ClothType* convertCloth(DstFactoryType& dstFactory, const SrcClothType& srcCloth) {
    ...
    memcpy(dstCloth->getPreviousParticles().begin(), prevParticles.begin(),
           prevParticles.size() * sizeof(physx::PxVec4));
