...
void ClothSimpleTetherCooker::createTetherData(const ClothMeshDesc& desc) {
    ...
    for (PxU32 i = 0; i < numParticles; ++i) particles.pushBack(PxVec4(*pIt++, wIt.ptr() ? *wIt++ : 1.0f));

    // compute tether data
    nv::cloth::Vector<PxU32>::Type attachedIndices;
    for (PxU32 i = 0; i < numParticles; ++i)
        if (particles[i].w == 0.0f) attachedIndices.pushBack(i);
    ...
    for (PxU32 i = 0; i < n; ++i) {
        ...
        for (aIt = attachedIndices.begin(); aIt != aEnd; ++aIt) {
            float sqrDist = (reinterpret_cast<const PxVec3&>(particles[*aIt]) - position).magnitudeSquared();
...
///////////////////////////////////////////////////////////////////////////////
uint32_t ClothSimpleTetherCooker::getCookerStatus() const { return mCookerStatus; }

///////////////////////////////////////////////////////////////////////////////
void ClothSimpleTetherCooker::getTetherData(PxU32* userTetherAnchors, PxReal* userTetherLengths) const {
