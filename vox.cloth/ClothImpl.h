...
inline void ClothImpl<T>::setRotation(const physx::PxQuat& q) {
    if ((q - mTargetMotion.q).magnitudeSquared() == 0.0f) return;
...
inline void ClothImpl<T>::setLinearDrag(const physx::PxVec3& drag) {
    ...
    if (value == mLinearLogDrag) return;
...
inline void ClothImpl<T>::setAngularDrag(const physx::PxVec3& drag) {
    physx::PxVec3 value = safeLog2(physx::PxVec3(1.f) - drag);
    if (value == mAngularLogDrag) return;
...
inline void ClothImpl<T>::setSolverFrequency(float frequency) {
    if (frequency == mSolverFrequency) return;
...
inline void ClothImpl<T>::setStiffnessFrequency(float frequency) {
    if (frequency == mStiffnessFrequency) return;
...
void move(Iter it, uint32_t first, uint32_t last, uint32_t result) {
    if (result > first) {
        result += last - first;
        while (first < last) it[--result] = it[--last];
    } else {
        while (first < last) it[result++] = it[first++];
    }
}
...
inline void ClothImpl<T>::setSpheres(Range<const physx::PxVec4> spheres, uint32_t first, uint32_t last) {
    ...
    if (!oldSize) {
        ...
        if (std::max(oldSize, newSize) > std::min(getChildCloth()->mStartCollisionSpheres.capacity(),
        ...
        for (uint32_t i = uint32_t(target.size()); i < oldSize; ++i) target.pushBack(start[i]);
        ...
        if (auto delta = int32_t(newSize - oldSize)) {
            ...
            for (uint32_t i = last; i < last + delta; ++i) start[i] = spheres[i - first];
        ...
        for (uint32_t i = 0; i < spheres.size(); ++i) target[first + i] = spheres[i];
    }
...
inline void ClothImpl<T>::setSpheres(Range<const physx::PxVec4> startSpheres,
    ...
    startSpheres =
            Range<const physx::PxVec4>(startSpheres.begin(), std::min(startSpheres.end(), startSpheres.begin() + 32));
    targetSpheres = Range<const physx::PxVec4>(targetSpheres.begin(),
...
inline void ClothImpl<T>::setCapsules(Range<const uint32_t> capsules, uint32_t first, uint32_t last) {
    ...
    if (uint32_t delta = newSize - oldSize) {
        ...
        for (uint32_t i = last; i < last + delta; ++i) dstIndices[i] = srcIndices[i - first];
    ...
    for (uint32_t i = 0; i < srcIndicesSize; ++i) dstIndices[first + i] = srcIndices[i];
...
inline void ClothImpl<T>::setPlanes(Range<const physx::PxVec4> planes, uint32_t first, uint32_t last) {
    ...
    for (int i = 0; i < static_cast<int>(planes.size()); i++) {
        if (fabsf(planes[i].getXYZ().magnitudeSquared() - 1.0f) > 0.01f) {
            if (logCount == 0)
                NV_CLOTH_LOG_INVALID_PARAMETER(
                        "The plane normals passed to Cloth::setPlanes are not normalized. First error encounterd at "
                        "plane %d (%f, %f, %f, %f)",
                        i, static_cast<double>(planes[i].x), static_cast<double>(planes[i].y),
                        static_cast<double>(planes[i].z), static_cast<double>(planes[i].w));
            logCount++;
        }
    }
    if (logCount > 1) {
    ...
    if (!oldSize) {
        ...
        if (std::max(oldSize, newSize) > std::min(getChildCloth()->mStartCollisionPlanes.capacity(),
        ...
        // fill target from start
        for (uint32_t i = target.size(); i < oldSize; ++i) target.pushBack(start[i]);
        ...
        for (uint32_t i = 0; i < planes.size(); ++i) target[first + i] = planes[i];
    }
...
inline void ClothImpl<T>::setPlanes(Range<const physx::PxVec4> startPlanes, Range<const physx::PxVec4> targetPlanes) {
    ...
    startPlanes =
            Range<const physx::PxVec4>(startPlanes.begin(), std::min(startPlanes.end(), startPlanes.begin() + 32));
    targetPlanes =
...
inline void ClothImpl<T>::setConvexes(Range<const uint32_t> convexMasks, uint32_t first, uint32_t last) {
    ...
    for (int i = 0; i < static_cast<int>(convexMasks.size()); i++) {
        if (convexMasks[i] == 0) {
            NV_CLOTH_LOG_INVALID_PARAMETER(
                    "Cloth::setConvexes expects bit masks of the form (1<<planeIndex1)|(1<<planeIndex2). 0 is not a "
                    "valid mask/plane index. Error found in location %d",
                    i);
            continue;
        }
    }
    ...
    if (uint32_t delta = newSize - oldSize) {
        ...
        // fill new elements from capsules
        for (uint32_t i = last; i < last + delta; ++i) masks[i] = convexMasks[i - first];
...
inline void ClothImpl<T>::setTriangles(Range<const physx::PxVec3> triangles, uint32_t first, uint32_t last) {
    ...
    if (!oldSize) {
        ...
        if (std::max(oldSize, newSize) > std::min(getChildCloth()->mStartCollisionTriangles.capacity(),
                                                  getChildCloth()->mTargetCollisionTriangles.capacity())) {
            ContextLockType contextLock(getChildCloth()->mFactory);
        ...
        if (auto delta = int32_t(newSize - oldSize)) {
            ...
            for (uint32_t i = last; i < last + delta; ++i) start[i] = triangles[i - first];
        ...
        //	   std::min(getChildCloth()->mStartCollisionTriangles.capacity(),
        // getChildCloth()->mTargetCollisionTriangles.capacity()))
        //	{
        //		ContextLockType contextLock(getChildCloth()->mFactory);
        //		getChildCloth()->mStartCollisionTriangles.reserve(newSize);
        ...
        // fill target elements with triangles
        for (uint32_t i = 0; i < triangles.size(); ++i) target[first + i] = triangles[i];
    }
...
inline void ClothImpl<T>::setTriangles(Range<const physx::PxVec3> startTriangles,
    ...
    if (newSize > std::min(getChildCloth()->mStartCollisionTriangles.capacity(),
                           getChildCloth()->mTargetCollisionTriangles.capacity())) {
        ContextLockType contextLock(getChildCloth()->mFactory);
        ...
        if (newSize - oldSize) getChildCloth()->notifyChanged();
    }
...
inline void ClothImpl<T>::enableContinuousCollision(bool enable) {
    if (enable == getChildCloth()->mEnableContinuousCollision) return;
...
inline void ClothImpl<T>::setCollisionMassScale(float scale) {
    if (scale == getChildCloth()->mCollisionMassScale) return;
...
inline void ClothImpl<T>::setTetherConstraintScale(float scale) {
    if (scale == getChildCloth()->mTetherConstraintScale) return;
...
inline void ClothImpl<T>::setTetherConstraintStiffness(float stiffness) {
    ...
    if (value == getChildCloth()->mTetherConstraintLogStiffness) return;
...
inline void ClothImpl<T>::setMotionConstraintScaleBias(float scale, float bias) {
    if (scale == getChildCloth()->mMotionConstraintScale && bias == getChildCloth()->mMotionConstraintBias) return;
...
inline void ClothImpl<T>::setMotionConstraintStiffness(float stiffness) {
    float value = safeLog2(1 - stiffness);
    if (value == getChildCloth()->mMotionConstraintLogStiffness) return;
...
inline void ClothImpl<T>::setSelfCollisionDistance(float distance) {
    if (distance == getChildCloth()->mSelfCollisionDistance) return;
...
inline void ClothImpl<T>::setSleepThreshold(float threshold) {
    if (threshold == mSleepThreshold) return;
...
inline void ClothImpl<T>::setSleepTestInterval(uint32_t interval) {
    if (interval == mSleepTestInterval) return;
...
inline void ClothImpl<T>::setSleepAfterCount(uint32_t afterCount) {
    if (afterCount == mSleepAfterCount) return;
