...
void solveConstraints(float* __restrict posIt,
                      const float* __restrict rIt,
                      const float* __restrict stIt,
                      const float* __restrict rEnd,
                      const uint16_t* __restrict iIt,
                      const __m128& stiffnessEtc,
                      const __m128& stiffnessExponent);
}  // namespace avx
...
uint32_t getAvxSupport() {
    ...
    if ((cpuInfo[2] & avxFlags) != avxFlags) return 0;  // xgetbv not enabled or no AVX support

    if ((_xgetbv(_XCR_XFEATURE_ENABLED_MASK) & 0x6) != 0x6) return 0;  // OS does not save YMM registers
...
#else
    int fmaFlags = 1 << 12;
    if ((cpuInfo[2] & fmaFlags) != fmaFlags) return 1;  // no FMA3 support
...
void integrateParticles(T4f* __restrict curIt,
                        T4f* __restrict curEnd,
                        T4f* __restrict prevIt,
                        const T4f& scale,
                        const AccelerationIterator& aIt,
                        const T4f& prevBias) {
...
void integrateParticles(T4f* __restrict curIt,
                        T4f* __restrict curEnd,
                        T4f* __restrict prevIt,
                        const T4f (&prevMatrix)[3],
                        const T4f (&curMatrix)[3],
                        const AccelerationIterator& aIt,
                        const T4f& prevBias) {
    // local copy to avoid LHS
    AccelerationIterator accelIt(aIt);
...
void constrainMotion(T4f* __restrict curIt,
                     const T4f* __restrict curEnd,
                     const ConstraintIterator& spheres,
...
void solveConstraints(float* __restrict posIt,
                      ...
                      const float* __restrict rEnd,
                      const uint16_t* __restrict iIt,
                      const T4f& stiffnessEtc,
                      const T4f& stiffnessExponent) {
    ...
    for (; rIt != rEnd; rIt += 4, stIt += 4, iIt += 8) {
        ...
        T4f stij = useStiffnessPerConstraint
                           ? gSimd4fOne - exp2(stiffnessExponent * static_cast<T4f>(loadAligned(stIt)))
                           : stiffness;
...
#include "vox.cloth/sse2/SwSolveConstraints.h"
#endif
...
T4f calculateMaxDelta(const T4f* prevIt, const T4f* curIt, const T4f* curEnd) {
    ...
    for (; curIt < curEnd; ++curIt, ++prevIt) maxDelta = max(maxDelta, abs(*curIt - *prevIt));

    return maxDelta & sMaskXYZ;
}

template <bool IsTurning, typename T4f>
void applyWind(T4f* __restrict curIt,
               const T4f* __restrict prevIt,
               const uint16_t* __restrict tIt,
               const uint16_t* __restrict tEnd,
               float itrDtf,
               float dragCoefficientf,
               float liftCoefficientf,
               float fluidDensityf,
               T4f wind,
    ...
    for (; tIt < tEnd; tIt += 3) {
        ...
        T4f delta = current - previous + wind;  // wind is also already multiplied by dt in the iteration state so
...
template <typename T4f>
cloth::SwSolverKernel<T4f>::SwSolverKernel(SwCloth const& cloth,
                                           SwClothData& clothData,
                                           SwKernelAllocator& allocator,
...
void cloth::SwSolverKernel<T4f>::constrainTether() {
    if (0.0f == mClothData.mTetherConstraintStiffness || !mClothData.mNumTethers) return;
...
#if _MSC_VER >= 1700
                neutralMultiplier
                        ? avx::solveConstraints<false, 2>(pIt, rIt, stIt, rEnd, iIt, stiffness, stiffnessExponent)
                        : avx::solveConstraints<true, 2>(pIt, rIt, stIt, rEnd, iIt, stiffness, stiffnessExponent);
                break;
#endif
            case 1:
                neutralMultiplier
                        ? avx::solveConstraints<false, 1>(pIt, rIt, stIt, rEnd, iIt, stiffness, stiffnessExponent)
...
void cloth::SwSolverKernel<T4f>::applyWind() {
    if (mClothData.mDragCoefficient == 0.0f && mClothData.mLiftCoefficient == 0.0f) return;
    ...
    if (mState.mIsTurning) {
        ::applyWind<true>(curIt, prevIt, tIt, tEnd, mState.mIterDt, mClothData.mDragCoefficient,
                          mClothData.mLiftCoefficient, mClothData.mFluidDensity, mState.mWind, mState.mRotationMatrix);
    } else {
        ::applyWind<false>(curIt, prevIt, tIt, tEnd, mState.mIterDt, mClothData.mDragCoefficient,
                           mClothData.mLiftCoefficient, mClothData.mFluidDensity, mState.mWind, mState.mRotationMatrix);
    }
}
...
void cloth::SwSolverKernel<T4f>::constrainMotion() {
    if (!mClothData.mStartMotionConstraints) return;
...
void cloth::SwSolverKernel<T4f>::updateSleepState() {
    ...
    if (mClothData.mSleepTestCounter >= mCloth.mSleepTestInterval) {
        ...
        if (anyGreaterEqual(maxDelta, threshold)) mClothData.mSleepPassCounter = 0;
