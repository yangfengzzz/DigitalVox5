...
__m256 exp2(const __m256& v) {
    ...
    __m256 px = _mm256_mul_ps(
            fx, _mm256_add_ps(_mm256_add_ps(_mm256_set1_ps(1.51390680115615096133e+3f),
                                            _mm256_mul_ps(fx2, _mm256_set1_ps(2.02020656693165307700e+1f))),
                              _mm256_mul_ps(fx2, _mm256_set1_ps(2.30933477057345225087e-2f))));

    __m256 qx = _mm256_add_ps(_mm256_set1_ps(4.36821166879210612817e+3f),
                              _mm256_mul_ps(fx2, _mm256_add_ps(_mm256_set1_ps(2.33184211722314911771e+2f), fx2)));
...
void solveConstraints(float* __restrict posIt,
    ...
    if (useMultiplier) {
        ...
        // (stiffness, multiplier, compressionLimit, stretchLimit, stiffness, multiplier, compressionLimit,
        // stretchLimit)
        //  float*[0], float*[1], etc..
    ...
    for (; rIt < rEnd; rIt += 8, iIt += 16, stIt += 8) {
        ...
        __m256 stij = useStiffnessPerConstraint
                              ? _mm256_sub_ps(sOne, exp2<avx>(_mm256_mul_ps(_mm256_load_ps(stIt),
                                                                            _mm256_broadcast_ps(&stiffnessExponent))))
                              : stiffness;
...
template void solveConstraints<false, 1>(float* __restrict,
...
template void solveConstraints<true, 1>(float* __restrict,
                                        const float* __restrict,
                                        const float* __restrict,
                                        const float* __restrict,
                                        const uint16_t* __restrict,
...
template void solveConstraints<false, 2>(float* __restrict,
                                         const float* __restrict,
                                         const float* __restrict,
                                         const float* __restrict,
                                         const uint16_t* __restrict,
...
template void solveConstraints<true, 2>(float* __restrict,
                                        const float* __restrict,
                                        const float* __restrict,
                                        const float* __restrict,
                                        const uint16_t* __restrict,
