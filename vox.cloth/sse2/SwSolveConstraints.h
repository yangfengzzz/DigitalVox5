...
#pragma warning(disable : 4127)  // conditional expression is constant
        ...
        void solveConstraints(float* __restrict posIt,
                              ...
                              const Simd4f& stiffnessEtc,
                              const Simd4f& stiffnessExponent) {
    PX_UNUSED(stIt);
    ...
    for (; rIt != rEnd; rIt += 4, stIt += 4, iIt += 8) {
        ...
        __m128 e2ij = _mm_add_ps(
                gSimd4fEpsilon,
                _mm_add_ps(_mm_mul_ps(hxij, hxij), _mm_add_ps(_mm_mul_ps(hyij, hyij), _mm_mul_ps(hzij, hzij))));

        // Load/calculate the constraint stiffness
        __m128 stij =
                useStiffnessPerConstraint ? _mm_set_ps1(1.0f) - exp2(stiffnessExponent * _mm_load_ps(stIt)) : stiffness;

        __m128 mask = _mm_cmpnle_ps(rij, gSimd4fEpsilon);
        __m128 erij = _mm_and_ps(_mm_sub_ps(sOne, _mm_mul_ps(rij, _mm_rsqrt_ps(e2ij))), mask);

        if (useMultiplier) {
            erij = _mm_sub_ps(erij,
                              _mm_mul_ps(multiplier, _mm_max_ps(compressionLimit, _mm_min_ps(erij, stretchLimit))));
        }
