...
void radixSort(const uint32_t* first, const uint32_t* last, uint16_t* out) {
    ...
    for (uint16_t i = 0; i != n; ++i) indices[1][histograms[0][0xff & first[i]]++] = i;
...
void cloth::SwSelfCollision<T4f>::operator()() {
    ...
    if (!isSelfCollisionEnabled(mClothData)) return;
    ...
    for (uint32_t i = 0; i < numIndices; ++i) sortedKeys[i] = keys[sortedIndices[i]];
    ...
    if (indices) {
        ...
        for (uint32_t i = 0; i < numIndices; ++i) sortedIndices[i] = uint16_t(indices[oldSortedIndices[i]]);
    }
...
void cloth::SwSelfCollision<T4f>::collideParticles(T4f& pos0, T4f& pos1, const T4f& pos0rest, const T4f& pos1rest) {
    T4f diff = pos1 - pos0;
    ...
    if (useRestParticles) {
        ...
        if (allGreater(mCollisionSquareDistance, restDistSqr)) return;
    }
...
template <typename T4f>
template <bool useRestParticles>
void cloth::SwSelfCollision<T4f>::collideParticles(const uint32_t* keys,
                                                   uint16_t firstColumnSize,
    ...
    T4f* __restrict restParticles = useRestParticles ? reinterpret_cast<T4f*>(mClothData.mRestPositions) : particles;
