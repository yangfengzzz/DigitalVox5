...
#include "vox.cloth/foundation/PxMemory.h"
...
template <typename T>
void gatherIndices(nv::cloth::Vector<PxU32>::Type& indices, const BoundedData& triangles, const BoundedData& quads) {
    PxStrideIterator<const T> tIt, qIt;
...
struct VertexDistanceCount {
    VertexDistanceCount(int vert, float dist, int count) : vertNr(vert), distance(dist), edgeCount(count) {}

    int vertNr;
    float distance;
    int edgeCount;
    bool operator<(const VertexDistanceCount& v) const { return v.distance < distance; }
};
...
//---------------------------------------------------------------------------------------
struct VertTriangle {
    VertTriangle(int vert, int triangle) : mVertIndex(vert), mTriangleIndex(triangle) {}

    bool operator<(const VertTriangle& vt) const {
        return mVertIndex == vt.mVertIndex ? mTriangleIndex < vt.mTriangleIndex : mVertIndex < vt.mVertIndex;
    }
...
// ---------------------------------------------------------------------------------------
struct MeshEdge {
    MeshEdge(int v0, int v1, int halfEdgeIndex) : mFromVertIndex(v0), mToVertIndex(v1), mHalfEdgeIndex(halfEdgeIndex) {
        if (mFromVertIndex > mToVertIndex) ps::swap(mFromVertIndex, mToVertIndex);
    }

    bool operator<(const MeshEdge& e) const {
        return mFromVertIndex == e.mFromVertIndex ? mToVertIndex < e.mToVertIndex : mFromVertIndex < e.mFromVertIndex;
    }

    bool operator==(const MeshEdge& e) const {
        return mFromVertIndex == e.mFromVertIndex && mToVertIndex == e.mToVertIndex;
    }
...
bool checkEdgeOrientation(const MeshEdge& e, const nv::cloth::Vector<PxU32>::Type& indices) {
    ...
    if ((e.mFromVertIndex == v0) && (e.mToVertIndex == v1)) return true;
...
inline bool checkEdge(int ei0, int ei1, int ej0, int ej1) {
    return ((ei0 == ej0) && (ei1 == ej1)) || ((ei0 == ej1) && (ei1 == ej0));
}
...
void ClothGeodesicTetherCooker::createTetherData(const ClothMeshDesc& desc) {
    ...
    for (PxU32 i = 0; i < mNumParticles; ++i) {
        ...
        if (mAttached[i]) vertexIslandHeap.pushBack(VertexDistanceCount(int(i), FLT_MAX, 0));
    }
    PxU32 attachedCnt = vertexIslandHeap.size();

    // no attached vertices
    if (vertexIslandHeap.empty()) return;
    ...
    while (!vertexIslandHeap.empty()) {
        ...
        for (PxU32 j = begin; j < end; ++j) {
            ...
            if (!mAttached[vj]) continue;

            // already visited
            if (vertexIsland[vj] != PxU32(-1)) continue;
    ...
    for (PxU32 i = 0; i < mNumParticles; i++) {
        ...
        for (PxU32 j = 0; j < nbTethersPerParticle; j++) {
            ...
            if (parent != i) {
                ...
                if (errorCode < 0) geodesicDistance = dijkstraDistance;
...
///////////////////////////////////////////////////////////////////////////////
PxU32 ClothGeodesicTetherCooker::getCookerStatus() const { return mCookerStatus; }

///////////////////////////////////////////////////////////////////////////////
PxU32 ClothGeodesicTetherCooker::getNbTethersPerParticle() const { return mTetherAnchors.size() / mNumParticles; }

///////////////////////////////////////////////////////////////////////////////
void ClothGeodesicTetherCooker::getTetherData(PxU32* userTetherAnchors, PxReal* userTetherLengths) const {
...
PxU32 ClothGeodesicTetherCooker::findTriNeighbors() {
    ...
    for (int i = 0; i < numEdges;) {
        ...
        if (i - j > 2) return 1;  // non-manifold
        ...
        while (++j < i) {
            ...
            if (orientation0 == orientation1) return 2;  // bad winding
        }
    }
