...
cloth::SwCloth::SwCloth(SwFactory& factory, const SwCloth& cloth)
    ...
    // Both cloth and this have a reference to fabric. The factory that created fabric does not have to be the same as
    // mFactory. mFabric needs to outlive both cloth instances. (this is checked with refcount asserts).
    mFabric.incRefCount();
}

cloth::SwCloth::~SwCloth() { mFabric.decRefCount(); }
...
cloth::Range<const PxVec3> cloth::SwCloth::clampTriangleCount(Range<const PxVec3> range, uint32_t) { return range; }
...
uint32_t SwCloth::getNumParticles() const { return uint32_t(mCurParticles.size()); }

void SwCloth::lockParticles() const {}

void SwCloth::unlockParticles() const {}

MappedRange<physx::PxVec4> SwCloth::getCurrentParticles() { return getMappedParticles(&mCurParticles.front()); }

MappedRange<const physx::PxVec4> SwCloth::getCurrentParticles() const {
    return getMappedParticles(&mCurParticles.front());
}

MappedRange<physx::PxVec4> SwCloth::getPreviousParticles() { return getMappedParticles(&mPrevParticles.front()); }
...
void SwCloth::setPhaseConfig(Range<const PhaseConfig> configs) {
    ...
    for (; !configs.empty(); configs.popFront())
        if (configs.front().mStiffness > 0.0f) mPhaseConfigs.pushBack(transform(configs.front()));
...
uint32_t SwCloth::getNumVirtualParticles() const { return uint32_t(mNumVirtualParticles); }
