...
#include "vox.cloth/ClothClone.h"
#include "vox.cloth/foundation/PxIO.h"
...
void prefixSum(const T* first, const T* last, T* dest) {
    if (first != last) {
        *(dest++) = *(first++);
        for (; first != last; ++first, ++dest) *dest = *(dest - 1) + *first;
    }
}

template <typename T>
void gatherAdjacencies(nv::cloth::Vector<PxU32>::Type& valency,
                       nv::cloth::Vector<PxU32>::Type& adjacencies,
                       const BoundedData& triangles,
                       const BoundedData& quads) {
...
void gatherTriangles(nv::cloth::Vector<PxU32>::Type& indices, const BoundedData& triangles, const BoundedData& quads) {
    ...
    tIt = PxMakeIterator(reinterpret_cast<const T*>(triangles.data), triangles.stride);
    for (PxU32 i = 0; i < triangles.count; ++i, ++tIt, ++qIt) {
        for (PxU32 j = 0; j < 3; ++j) indices.pushBack(tIt.ptr()[j]);
    }
    qIt = PxMakeIterator(reinterpret_cast<const T*>(quads.data), quads.stride);
    for (PxU32 i = 0; i < quads.count; ++i, ++tIt, ++qIt) {
...
struct Edge {
    ...
    void classify() { mStretching += 0.1f; }
...
void pushHeap(typename nv::cloth::Vector<T>::Type& heap, const T& value) {
    ...
    while (current > 0) {
        ...
        if (!(begin[parent] < begin[current])) break;
...
T popHeap(typename nv::cloth::Vector<T>::Type& heap) {
    ...
    while (begin + (current * 2 + 1) < end) {
        ...
        if (begin + child + 1 < end && begin[child] < begin[child + 1]) ++child;

        if (!(begin[current] < begin[child])) break;
...
struct ConstraintGraphColorCount {
    ConstraintGraphColorCount(PxU32 cid, PxU32 count) : constraint(cid), colorCount(count) {}

    PxU32 constraint;
    PxU32 colorCount;

    bool operator<(const ConstraintGraphColorCount& c) const { return colorCount < c.colorCount; }
};
...
bool FabricCookerImpl::cook(const ClothMeshDesc& desc, PxVec3 gravity, bool useGeodesicTether) {
    ...
    for (PxU32 i = 0; i < mNumParticles; ++i) particles.pushBack(PxVec4(*pIt++, wIt.ptr() ? *wIt++ : 1.0f));
    ...
    nv::cloth::Vector<PxU32>::Type neighbors;
    neighbors.reserve(adjacencies.size());
    ...
    for (PxU32 i = 0; i < mNumParticles; ++i) {
        ...
        for (PxU32 j = valency[i]; j < jlast; ++j) {
            // add 1-ring edge
            PxU32 m = neighbors[j];
            if (wi + particles[m].w > 0.0f) edges[Pair(PxMin(i, m), PxMax(i, m))].classify();
            ...
            for (PxU32 k = valency[m]; k < klast; ++k) {
                ...
                if (n != i && wi + particles[n].w > 0.0f) {
                    // add 2-ring edge
                    edges[Pair(PxMin(i, n), PxMax(i, n))].classify(particles[i], particles[m], particles[n]);
                }
            }
        }
    }
    ...
    valency.resize(0);
    valency.resize(mNumParticles + 1, 0);
    ...
    adjacencies.resize(valency.back(), 0);
    ...
    nv::cloth::Vector<PxU32>::Type colors(numConstraints,
                                          numConstraints);  // constraint -> color, initialily not colored
    mark.resize(0);
    mark.resize(numConstraints + 1, PX_MAX_U32);                      // color -> constraint index
    ...
    for (;;) {
        ...
        if (constraint >= numConstraints) break;
        ...
        while (!constraintHeap.empty()) {
            ...
            for (PxU32 j = 0; j < 2; ++j) {
                ...
                if (particles[index].w == 0.0f) continue;  // don't mark adjacent particles if attached

                for (nv::cloth::Vector<PxU32>::Type::ConstIterator aIt = aFirst + valency[index],
                                                                   aEnd = aFirst + valency[index + 1];
                     aIt != aEnd; ++aIt) {
                    ...
                    pushHeap(constraintHeap,
                             ConstraintGraphColorCount(adjacentConstraint, adjColorCount[adjacentConstraint]));
                }
            }
    ...
    for (PxU32 i = 0; i < numConstraints; ++i) {
        ...
        mRestvalues[index] = reinterpret_cast<const PxVec3&>(diff).magnitude();
    }
    ...
    for (PxU32 i = 0; i < mSets.size() - 1; ++i) {
        ...
        for (PxU32 r = 0; r < reorder.size(); ++r) reorder[r] = r;
    ...
    if (desc.pointsStiffness.count) {
        ...
        PxStrideIterator<const PxReal> stIt(reinterpret_cast<const PxReal*>(desc.pointsStiffness.data),
                                            desc.pointsStiffness.stride);
...
NV_CLOTH_API(nv::cloth::Fabric*)
NvClothCookFabricFromMesh(nv::cloth::Factory* factory,
                          const nv::cloth::ClothMeshDesc& desc,
    ...
    return factory->createFabric(data.mNumParticles, data.mPhaseIndices, data.mSets, data.mRestvalues,
                                 data.mStiffnessValues, data.mIndices, data.mAnchors, data.mTetherLengths,
                                 data.mTriangles);
}
