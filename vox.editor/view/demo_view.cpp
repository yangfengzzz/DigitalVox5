...
DemoView::DemoView(const std::string &title,
                   bool opened,
    ...
    auto editor_root = scene->FindEntityByName("SceneRoot");
    if (!editor_root) {
        editor_root = scene->CreateRootEntity("SceneRoot");
    }
    auto grid = editor_root->AddComponent<MeshRenderer>();
    grid->SetMesh(CreatePlane());
    grid->SetMaterial(std::make_shared<GridMaterial>(render_context_.GetDevice()));
    main_camera_ = app->LoadScene(editor_root);
    camera_control_ = main_camera_->GetEntity()->GetComponent<control::OrbitControl>();
...
void DemoView::Update(float delta_time) {
...
void DemoView::Render(CommandBuffer &command_buffer) {
    if (IsFocused()) {
...
void DemoView::Pick(float offset_x, float offset_y) {
    need_pick_ = true;
    pick_pos_ = Vector2F(offset_x, offset_y);
}

void DemoView::CopyRenderTargetToBuffer(CommandBuffer &command_buffer) {
...
void DemoView::ReadColorFromRenderTarget() {
...
void DemoView::InputEvent(const vox::InputEvent &input_event) {
...
void DemoView::EditTransform(float *camera_view, float *camera_projection, float *matrix) {
...
control::OrbitControl *DemoView::CameraControl() { return camera_control_; }
