...
public:
    ...
    void OnUpdate(float delta_time) override {
        ...
        entity()->transform->SetPosition(10 * std::sin(speed_ * total_time_), 10, 10 * std::cos(speed_ * total_time_));
...
public:
    ...
    void OnUpdate(float delta_time) override {
        total_time_ += delta_time;
        total_time_ = fmod(total_time_, 100);
        entity()->transform->SetPosition(10 * std::cos(speed_ * total_time_), 10, 10 * std::sin(speed_ * total_time_));
        entity()->transform->LookAt(Point3F(0, 0, 0));
    }
};
}
