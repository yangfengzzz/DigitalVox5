...
SingleDownloadDataset::SingleDownloadDataset(const std::string& prefix,
    ...
    const std::string filename = utility::filesystem::GetFileNameWithoutDirectory(urls[0]);

    const bool is_extract_present = utility::filesystem::DirectoryExists(Dataset::GetExtractDir());

    if (!is_extract_present) {
        // `download_dir` is relative path from `${data_root}`.
        const std::string download_dir = "download/" + GetPrefix();
        const std::string download_file_path = utility::DownloadFromURL(urls, md5, download_dir, data_root_);
        ...
        if (!no_extract) {
            ...
            utility::filesystem::Copy(download_file_path, Dataset::GetExtractDir());
        }
    }
}
...
DemoFeatureMatchingPointClouds::DemoFeatureMatchingPointClouds(const std::string& data_root)
    : SingleDownloadDataset("DemoFeatureMatchingPointClouds",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    point_cloud_paths_ = {extract_dir + "/cloud_bin_0.pcd", extract_dir + "/cloud_bin_1.pcd"};
    fpfh_feature_paths_ = {extract_dir + "/cloud_bin_0.fpfh.bin", extract_dir + "/cloud_bin_1.fpfh.bin"};
    l32d_feature_paths_ = {extract_dir + "/cloud_bin_0.d32.bin", extract_dir + "/cloud_bin_1.d32.bin"};
}
...
DemoPoseGraphOptimization::DemoPoseGraphOptimization(const std::string& data_root)
    : SingleDownloadDataset("DemoPoseGraphOptimization",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    pose_graph_fragment_path_ = extract_dir + "/pose_graph_example_fragment.json";
...
SampleRedwoodRGBDImages::SampleRedwoodRGBDImages(const std::string& data_root)
    ...
    color_paths_ = {extract_dir + "/color/00000.jpg", extract_dir + "/color/00001.jpg",
                    extract_dir + "/color/00002.jpg", extract_dir + "/color/00003.jpg",
                    extract_dir + "/color/00004.jpg"};

    depth_paths_ = {extract_dir + "/depth/00000.png", extract_dir + "/depth/00001.png",
...
SampleFountainRGBDImages::SampleFountainRGBDImages(const std::string& data_root)
    : SingleDownloadDataset("SampleFountainRGBDImages",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    color_paths_ = {extract_dir + "/image/0000010-000001228920.jpg", extract_dir + "/image/0000031-000004096400.jpg",
    ...
    depth_paths_ = {extract_dir + "/depth/0000038-000001234662.png", extract_dir + "/depth/0000124-000004104418.png",
...
MonkeyModel::MonkeyModel(const std::string& data_root)
    : SingleDownloadDataset("MonkeyModel",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/albedo.png"},
...
SwordModel::SwordModel(const std::string& data_root)
    : SingleDownloadDataset("SwordModel",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"sword_material", extract_dir + "/UV.mtl"},
...
MetalTexture::MetalTexture(const std::string& data_root)
    : SingleDownloadDataset("MetalTexture",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/Metal008_Color.jpg"},
...
PaintedPlasterTexture::PaintedPlasterTexture(const std::string& data_root)
    : SingleDownloadDataset("PaintedPlasterTexture",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/PaintedPlaster017_Color.jpg"},
...
TilesTexture::TilesTexture(const std::string& data_root)
    : SingleDownloadDataset("TilesTexture",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/Tiles074_Color.jpg"},
...
TerrazzoTexture::TerrazzoTexture(const std::string& data_root)
    : SingleDownloadDataset("TerrazzoTexture",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/Terrazzo018_Color.jpg"},
...
WoodTexture::WoodTexture(const std::string& data_root)
    : SingleDownloadDataset("WoodTexture",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/Wood049_Color.jpg"},
...
WoodFloorTexture::WoodFloorTexture(const std::string& data_root)
    : SingleDownloadDataset("WoodFloorTexture",
                            ...
                            data_root) {
    const std::string extract_dir = Dataset::GetExtractDir();
    map_filename_to_path_ = {{"albedo", extract_dir + "/WoodFloor050_Color.jpg"},
