...
public:
    ...
    static std::pair<Tensor, std::vector<Tensor>> ShuffleIndexedDimsToFront(const Tensor& tensor,
    ...
    static std::pair<std::vector<Tensor>, SizeVector> ExpandToCommonShapeExceptZeroDim(
            const std::vector<Tensor>& index_tensors);
...
public:
    ...
    [[nodiscard]] inline OPEN3D_HOST_DEVICE int64_t GetIndexedOffset(int64_t workload_idx) const {
        ...
        for (int64_t i = 0; i < num_indices_; ++i) {
            ...
            OPEN3D_ASSERT(index >= -indexed_shape_[i] && index < indexed_shape_[i] && "Index out of bounds.");
