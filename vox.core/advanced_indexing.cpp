...
std::pair<Tensor, std::vector<Tensor>> AdvancedIndexPreprocessor::ShuffleIndexedDimsToFront(
    ...
    return std::make_pair(tensor.Permute(permutation), std::move(permuted_index_tensors));
}

std::pair<std::vector<Tensor>, SizeVector> AdvancedIndexPreprocessor::ExpandToCommonShapeExceptZeroDim(
        const std::vector<Tensor>& index_tensors) {
    SizeVector replacement_shape({});  // {} can be broadcasted to any shape.
    for (const Tensor& index_tensor : index_tensors) {
        if (index_tensor.NumDims() != 0) {
            replacement_shape = shape_util::BroadcastedShape(replacement_shape, index_tensor.GetShape());
        }
    }
...
Tensor AdvancedIndexPreprocessor::RestrideIndexTensor(const Tensor& index_tensor,
                                                      int64_t dims_before,
                                                      int64_t dims_after) {
    SizeVector old_shape = index_tensor.GetShape();
    SizeVector new_shape(dims_before + index_tensor.NumDims() + dims_after, 1);
    std::copy(old_shape.begin(), old_shape.end(), new_shape.begin() + dims_before);
...
void AdvancedIndexPreprocessor::RunPreprocess() {
    ...
    //      A[[1, 2], [3, 4], :, :].
    Tensor empty_index_tensor = Tensor(SizeVector(), core::Int64, tensor_.GetDevice());
    ...
    if (IsIndexSplittedBySlice(index_tensors_)) {
        std::tie(tensor_, index_tensors_) = ShuffleIndexedDimsToFront(tensor_, index_tensors_);
    }
    ...
    SizeVector replacement_shape;
    std::tie(index_tensors_, replacement_shape) = ExpandToCommonShapeExceptZeroDim(index_tensors_);
    ...
    for (size_t dim = 0; dim < index_tensors_.size(); dim++) {
        if (index_tensors_[dim].NumDims() == 0) {
            ...
            if (!replacement_shape_inserted) {
                output_shape_.insert(output_shape_.end(), replacement_shape.begin(), replacement_shape.end());
...
std::vector<Tensor> AdvancedIndexPreprocessor::ExpandBoolTensors(const std::vector<Tensor>& index_tensors) {
    ...
    for (const Tensor& index_tensor : index_tensors) {
        if (index_tensor.GetDtype() == core::Bool) {
            ...
            res_index_tensors.insert(res_index_tensors.end(), non_zero_indices.begin(), non_zero_indices.end());
        } else {
