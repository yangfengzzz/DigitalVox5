...
public:
    ...
    Tensor(const SizeVector& shape, Dtype dtype, const Device& device = Device("CPU:0"))
        : shape_(shape),
          ...
          dtype_(dtype),
          blob_(std::make_shared<Blob>(shape.NumElements() * dtype.ByteSize(), device)) {
    ...
    Tensor(const std::vector<T>& init_vals,
        ...
        // Copy data to blob
        MemoryManager::MemcpyFromHost(blob_->GetDataPtr(), GetDevice(), init_vals.data(),
    ...
    Tensor(const T* init_vals, const SizeVector& shape, Dtype dtype, const Device& device = Device("CPU:0"))
        ...
        // Copy data to blob
        MemoryManager::MemcpyFromHost(blob_->GetDataPtr(), GetDevice(), init_vals,
    ...
    Tensor(const SizeVector& shape,
        ...
        : shape_(shape), strides_(strides), data_ptr_(data_ptr), dtype_(dtype), blob_(blob) {}
    ...
    template <typename T>
    static Tensor Init(const std::initializer_list<std::initializer_list<T>>& in_list,
                       const Device& device = Device("CPU:0")) {
    ...
    template <typename T>
    static Tensor Init(const std::initializer_list<std::initializer_list<std::initializer_list<T>>>& in_list,
                       const Device& device = Device("CPU:0")) {
    ...
    std::vector<T> ToFlatVector() const {
        ...
        MemoryManager::MemcpyToHost(values.data(), Contiguous().GetDataPtr(), GetDevice(),
    ...
    inline bool IsContiguous() const { return shape_util::DefaultStrides(shape_) == strides_; }
    ...
    inline int64_t GetShape(int64_t dim) const { return shape_[shape_util::WrapDim(dim, NumDims())]; }

    inline SizeVector GetStrides() const { return strides_; }

    inline const SizeVector& GetStridesRef() const { return strides_; }

    inline int64_t GetStride(int64_t dim) const { return strides_[shape_util::WrapDim(dim, NumDims())]; }
...
private:
    ...
    static Tensor InitWithInitializerList(const tensor_init::NestedInitializerList<T, D>& nested_list,
                                          const Device& device = Device("CPU:0")) {
        SizeVector shape = tensor_init::InferShape(nested_list);
        std::vector<T> values = tensor_init::ToFlatVector<T, D>(shape, nested_list);
...
template <>
inline Tensor::Tensor(const std::vector<bool>& init_vals, const SizeVector& shape, Dtype dtype, const Device& device)
    : Tensor(shape, dtype, device) {
    // Check number of elements
    if (static_cast<int64_t>(init_vals.size()) != shape_.NumElements()) {
        LOGE("Tensor initialization values' size {} does not match the "
    ...
    MemoryManager::MemcpyFromHost(blob_->GetDataPtr(), GetDevice(), init_vals_uchar.data(),
...
inline std::vector<bool> Tensor::ToFlatVector() const {
    ...
    MemoryManager::MemcpyToHost(values_uchar.data(), Contiguous().GetDataPtr(), GetDevice(),
                                GetDtype().ByteSize() * NumElements());
...
inline bool Tensor::Item() const {
    ...
    AssertTemplateDtype<bool>();
    uint8_t value;
    MemoryManager::MemcpyToHost(&value, data_ptr_, GetDevice(), sizeof(uint8_t));
...
inline void Tensor::Fill(S v) {
    DISPATCH_DTYPE_TO_TEMPLATE_WITH_BOOL(GetDtype(), [&]() {
        ...
        Tensor tmp(std::vector<scalar_t>({casted_v}), SizeVector({}), GetDtype(), GetDevice());
        AsRvalue() = tmp;
    });
}
...
inline void Tensor::FillObject(const Object& v) {
    Tensor tmp(std::vector<Object>({v}), SizeVector({}), GetDtype(), GetDevice());
    AsRvalue() = tmp;
}
