...
Tensor NonZeroCPU(const Tensor& src) {
    ...
    DISPATCH_DTYPE_TO_TEMPLATE_WITH_BOOL(src.GetDtype(), [&]() {
        auto it = std::copy_if(indices.begin(), indices.end(), non_zero_indices.begin(), [&src_iter](int64_t index) {
            const void* src_ptr = src_iter.GetPtr(index);
            OPEN3D_ASSERT(src_ptr != nullptr && "Internal error.");
            return static_cast<float>(*static_cast<const scalar_t*>(src_ptr)) != 0;
        });
    ...
    std::vector<std::vector<int64_t>> non_zero_indices_by_dimensions(num_dims, std::vector<int64_t>(num_non_zeros, 0));
