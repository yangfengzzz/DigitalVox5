...
bool SelectionPolygonVolume::ConvertFromJsonValue(const Json::Value &value) {
    if (!value.isObject()) {
        LOGW("SelectionPolygonVolume read JSON failed: unsupported json "
             "format.")
        return false;
    }
    if (value.get("class_name", "").asString() != "SelectionPolygonVolume" ||
        value.get("version_major", 1).asInt() != 1 || value.get("version_minor", 0).asInt() != 0) {
        LOGW("SelectionPolygonVolume read JSON failed: unsupported json "
             "format.")
        return false;
    }
    ...
    if (polygon_array.empty()) {
        LOGW("SelectionPolygonVolume read JSON failed: empty trajectory.")
        return false;
    }
...
std::shared_ptr<geometry::PointCloud> SelectionPolygonVolume::CropPointCloud(const geometry::PointCloud &input) const {
    if (orthogonal_axis_.empty() || bounding_polygon_.empty()) return std::make_shared<geometry::PointCloud>();
    return CropPointCloudInPolygon(input);
}

std::shared_ptr<geometry::PointCloud> SelectionPolygonVolume::CropPointCloudInPolygon(
        const geometry::PointCloud &input) const {
    return input.SelectByIndex(CropInPolygon(input.points_));
}

std::shared_ptr<geometry::TriangleMesh> SelectionPolygonVolume::CropTriangleMesh(
        const geometry::TriangleMesh &input) const {
    if (orthogonal_axis_.empty() || bounding_polygon_.empty()) return std::make_shared<geometry::TriangleMesh>();
...
std::shared_ptr<geometry::TriangleMesh> SelectionPolygonVolume::CropTriangleMeshInPolygon(
        const geometry::TriangleMesh &input) const {
    return input.SelectByIndex(CropInPolygon(input.vertices_));
}

std::vector<size_t> SelectionPolygonVolume::CropInPolygon(const std::vector<Eigen::Vector3d> &input) const {
    ...
    for (size_t k = 0; k < input.size(); k++) {
        ...
        for (size_t i = 0; i < bounding_polygon_.size(); i++) {
            ...
            if ((bounding_polygon_[i](v) < point(v) && bounding_polygon_[j](v) >= point(v)) ||
                (bounding_polygon_[j](v) < point(v) && bounding_polygon_[i](v) >= point(v))) {
                nodes.push_back(bounding_polygon_[i](u) + (point(v) - bounding_polygon_[i](v)) /
