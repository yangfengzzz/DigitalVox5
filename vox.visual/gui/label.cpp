...
static const vox::Color DEFAULT_COLOR(0, 0, 0, 0);
...
vox::Color Label::GetTextColor() const { return impl_->color_; }

void Label::SetTextColor(const vox::Color& color) { impl_->color_ = color; }

FontId Label::GetFontId() const { return impl_->font_id_; }

void Label::SetFontId(const FontId font_id) { impl_->font_id_ = font_id; }

Size Label::CalcPreferredSize(const LayoutContext& context, const Constraints& constraints) const {
    ...
    if (impl_->is_single_line) {
        auto wrap_width = float(constraints.width);
        auto size = font->CalcTextSizeA(font->FontSize, float(constraints.width), wrap_width, impl_->text_.c_str());
        pref = Size(int(std::ceil(size.x + 2.0f * padding.x)), int(std::ceil(size.y + 2.0f * padding.y)));
    } else {
        ImVec2 size(0, 0);
        size_t line_start = 0;
        auto line_end = impl_->text_.find('\n');
        auto em = int(std::round(font->FontSize));
        float wrap_width = float(std::min(constraints.width, PREFERRED_WRAP_WIDTH_EM * em));
        float spacing = ImGui::GetTextLineHeightWithSpacing() - ImGui::GetTextLineHeight();
        do {
            ImVec2 sz;
            if (line_end == std::string::npos) {
                sz = font->CalcTextSizeA(font->FontSize, FLT_MAX, wrap_width, impl_->text_.c_str() + line_start);
                line_start = line_end;
            } else {
                sz = font->CalcTextSizeA(font->FontSize, FLT_MAX, wrap_width, impl_->text_.c_str() + line_start,
