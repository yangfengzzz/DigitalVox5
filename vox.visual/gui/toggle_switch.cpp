...
#include "vox.visual/gui/toggle_switch.h"
...
ToggleSwitch::ToggleSwitch(const char* name) : impl_(new ToggleSwitch::Impl()) {
    impl_->name_ = name;
    impl_->id_ = impl_->name_ + "##toggle_" + std::to_string(g_next_toggle_id++);
}
...
void ToggleSwitch::SetOnClicked(std::function<void(bool)> on_clicked) { impl_->on_clicked_ = std::move(on_clicked); }

Size ToggleSwitch::CalcPreferredSize(const LayoutContext& context, const Constraints& constraints) const {
    auto em = ImGui::GetTextLineHeight();
    auto padding = ImGui::GetStyle().FramePadding;
    auto text_size = ImGui::GetFont()->CalcTextSizeA(float(context.theme.font_size), constraints.width, 10000,
                                                     impl_->name_.c_str());
    int height = int(std::ceil(em + 2.0f * padding.y));
    auto switch_width = CalcSwitchWidth(height);
    return {int(switch_width + std::ceil(text_size.x + 2.0f * padding.x)), height};
}
...
Widget::DrawResult ToggleSwitch::Draw(const DrawContext& context) {
    ...
    if (impl_->is_on_) {
        if (ImGui::IsItemHovered()) {
            track_color = colorToImguiRGBA(theme.toggle_background_hover_on_color);
        } else {
            track_color = colorToImguiRGBA(theme.toggle_background_on_color);
        }
    } else {
        if (ImGui::IsItemHovered()) {
            track_color = colorToImguiRGBA(theme.toggle_background_hover_off_color);
        } else {
    ...
    draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), track_color, radius);
    draw_list->AddRect(p, ImVec2(p.x + width, p.y + height), colorToImguiRGBA(theme.border_color), radius);
    float thumb_offset = (impl_->is_on_ ? 1.0f : 0.0f) * (width - 2.0f * radius);
    draw_list->AddCircleFilled(ImVec2(p.x + radius + thumb_offset, p.y + radius), radius - 1.5f, thumb_color);
