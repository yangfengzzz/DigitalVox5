...
#include "vox.visual/gui/tree_view.h"
...
#include "vox.visual/gui/check_box.h"
#include "vox.visual/gui/color_edit.h"
#include "vox.visual/gui/label.h"
#include "vox.visual/gui/number_edit.h"
...
CheckableTextTreeCell::CheckableTextTreeCell(const char *text, bool is_checked, std::function<void(bool)> on_toggled)
    ...
    impl_->checkbox_->SetOnChecked(std::move(on_toggled));
    impl_->label_ = std::make_shared<Label>(text);
    AddChild(impl_->checkbox_);
    AddChild(impl_->label_);
}

CheckableTextTreeCell::~CheckableTextTreeCell() = default;

std::shared_ptr<Checkbox> CheckableTextTreeCell::GetCheckbox() { return impl_->checkbox_; }

std::shared_ptr<Label> CheckableTextTreeCell::GetLabel() { return impl_->label_; }

Size CheckableTextTreeCell::CalcPreferredSize(const LayoutContext &context, const Constraints &constraints) const {
    ...
    return {check_pref.width + label_pref.width, std::max(check_pref.height, label_pref.height)};
}

void CheckableTextTreeCell::Layout(const LayoutContext &context) {
    auto &frame = GetFrame();
    auto check_width = impl_->checkbox_->CalcPreferredSize(context, Constraints()).width;
    impl_->checkbox_->SetFrame(Rect(frame.x, frame.y, check_width, frame.height));
    auto x = impl_->checkbox_->GetFrame().GetRight();
    impl_->label_->SetFrame(Rect(x, frame.y, frame.GetRight() - x, frame.height));
}
...
LUTTreeCell::LUTTreeCell(const char *text,
    ...
    impl_->checkbox_ = std::make_shared<Checkbox>(" ");
    impl_->checkbox_->SetChecked(is_checked);
    impl_->checkbox_->SetOnChecked(std::move(on_enabled));
    impl_->label_ = std::make_shared<Label>(text);
    impl_->color_ = std::make_shared<ColorEdit>();
    impl_->color_->SetValue(color);
    impl_->color_->SetOnValueChanged(std::move(on_color_changed));
    AddChild(impl_->checkbox_);
    AddChild(impl_->label_);
    AddChild(impl_->color_);
}

LUTTreeCell::~LUTTreeCell() = default;

std::shared_ptr<Checkbox> LUTTreeCell::GetCheckbox() { return impl_->checkbox_; }
...
Size LUTTreeCell::CalcPreferredSize(const LayoutContext &context, const Constraints &constraints) const {
    auto check_pref = impl_->checkbox_->CalcPreferredSize(context, constraints);
    auto label_pref = impl_->label_->CalcPreferredSize(context, constraints);
    auto color_pref = impl_->color_->CalcPreferredSize(context, constraints);
    return {check_pref.width + label_pref.width + color_pref.width,
            std::max(check_pref.height, std::max(label_pref.height, color_pref.height))};
}
...
void LUTTreeCell::Layout(const LayoutContext &context) {
    ...
    impl_->checkbox_->SetFrame(Rect(frame.x, frame.y, check_width, frame.height));
    impl_->color_->SetFrame(Rect(frame.GetRight() - color_width, frame.y, color_width, frame.height));
    auto x = impl_->checkbox_->GetFrame().GetRight();
    impl_->label_->SetFrame(Rect(x, frame.y, impl_->color_->GetFrame().x - x, frame.height));
}
...
ColormapTreeCell::ColormapTreeCell(double value,
    ...
    impl_->value_->SetOnValueChanged(std::move(on_value_changed));
    impl_->color_ = std::make_shared<ColorEdit>();
    impl_->color_->SetValue(color);
    impl_->color_->SetOnValueChanged(std::move(on_color_changed));
...
ColormapTreeCell::~ColormapTreeCell() = default;

std::shared_ptr<NumberEdit> ColormapTreeCell::GetNumberEdit() { return impl_->value_; }

std::shared_ptr<ColorEdit> ColormapTreeCell::GetColorEdit() { return impl_->color_; }

Size ColormapTreeCell::CalcPreferredSize(const LayoutContext &context, const Constraints &constraints) const {
    auto number_pref = impl_->value_->CalcPreferredSize(context, constraints);
    auto color_pref = impl_->color_->CalcPreferredSize(context, constraints);
    return {number_pref.width + color_pref.width, std::max(number_pref.height, color_pref.height)};
}

void ColormapTreeCell::Layout(const LayoutContext &context) {
    auto &frame = GetFrame();
    auto number_pref = impl_->value_->CalcPreferredSize(context, Constraints());
    impl_->value_->SetFrame(Rect(frame.x, frame.y, number_pref.width, frame.height));
    auto x = impl_->value_->GetFrame().GetRight();
    impl_->color_->SetFrame(Rect(x, frame.y, frame.GetRight() - x, frame.height));
}
...
void TreeView::RemoveItem(ItemId item_id) {
    ...
    if (item_it != impl_->id2item_.end()) {
        ...
        if (item->parent) {
            for (auto sibling = item->parent->children.begin(); sibling != item->parent->children.end(); ++sibling) {
...
bool TreeView::GetCanSelectItemsWithChildren() const { return impl_->can_select_parents_; }

void TreeView::SetCanSelectItemsWithChildren(bool can_select) { impl_->can_select_parents_ = can_select; }
...
void TreeView::SetSelectedItemId(ItemId item_id) { impl_->selected_id_ = item_id; }

void TreeView::SetOnSelectionChanged(std::function<void(ItemId)> on_selection_changed) {
    impl_->on_selection_changed_ = std::move(on_selection_changed);
}

Size TreeView::CalcPreferredSize(const LayoutContext &context, const Constraints &constraints) const {
    return {constraints.width, Widget::DIM_GROW};
}

Size TreeView::CalcMinimumSize(const LayoutContext &context) const { return {0, 3 * context.theme.font_size}; }
...
Widget::DrawResult TreeView::Draw(const DrawContext &context) {
    ...
    ImGui::SetCursorScreenPos(ImVec2(float(frame.x), float(frame.y) - outer_scroll_y));
    ...
    ImGui::PushStyleColor(ImGuiCol_ChildBg, colorToImgui(context.theme.tree_background_color));

    // ImGUI's tree is basically a layout in the parent ImGUI window.
    // Make this a child so it's all in a nice frame.
    ImGui::BeginChild(impl_->id_, ImVec2(float(frame.width), float(frame.height)), true);

    Impl::Item *new_selection = nullptr;

    std::function<void(Impl::Item &)> DrawItem;
    DrawItem = [&DrawItem, this, &frame, &context, &new_selection, &result, outer_scroll_y](Impl::Item &item) {
        int height = item.cell->CalcPreferredSize({context.theme, context.fonts}, Constraints()).height;
        ...
        if (item.id == impl_->selected_id_) {
            ...
            // upper left)
            auto y = frame.y - outer_scroll_y + ImGui::GetCursorPosY() - ImGui::GetScrollY();
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(float(frame.x), y),
                                                      ImVec2(float(frame.GetRight()), y + height),
                                                      colorToImguiRGBA(context.theme.tree_selected_color));
        }

        int flags = ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_AllowItemOverlap;
        ...
        auto DrawThis = [this, &tree_frame = frame, &context, &new_selection, &result](TreeView::Impl::Item &item,
                                                                                       int height, bool is_selectable) {
            ImGui::SameLine(0, 0);
            auto x = int(std::round(ImGui::GetCursorScreenPos().x));
            auto y = int(std::round(ImGui::GetCursorScreenPos().y /*- ImGui::GetScrollY()*/));
            ...
            // work correctly.
            item.cell->SetFrame(Rect(x, y + scroll_y, tree_frame.width - indent - scroll_width, height));
