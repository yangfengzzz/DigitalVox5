...
#include "vox.visual/gui/menu_macos.h"
...
void MenuMacOS::AddSeparator() { [impl_->menu_ addItem:[NSMenuItem separatorItem]]; }

void MenuMacOS::InsertItem(int index, const char *name, ItemId item_id /*= NO_ITEM*/, vox::KeyCode key /*= KEY_NONE*/) {
    std::string shortcut;
    shortcut += char(key);
...
void MenuMacOS::InsertMenu(int index, const char *name, std::shared_ptr<MenuBase> submenu) {
    ...
    auto item = [[NSMenuItem alloc] initWithTitle:[NSString stringWithUTF8String:name] action:nil keyEquivalent:@""];
    if (index < impl_->menu_.numberOfItems) {
        [impl_->menu_ insertItem:item atIndex:index];
    } else {
        [impl_->menu_ addItem:item];
...
void MenuMacOS::InsertSeparator(int index) { [impl_->menu_ insertItem:[NSMenuItem separatorItem] atIndex:index]; }
...
MenuBase::ItemId MenuMacOS::DrawMenuBar(const DrawContext &context, bool is_enabled) { return NO_ITEM; }

MenuBase::ItemId MenuMacOS::Draw(const DrawContext &context, const char *name, bool is_enabled) { return NO_ITEM; }
