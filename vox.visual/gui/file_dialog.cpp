...
#include "vox.visual/gui/ListView.h"
...
public:
    ...
    bool operator==(const DirEntry &rhs) const { return (type_ == rhs.type_ && name_ == rhs.name_); }
...
struct FileDialog::Impl {
...
#ifndef __APPLE__
        if (path != "/") {
            entries_.insert(entries_.begin(), DirEntry("..", DirEntry::Type::DIR));
        }
#endif  // __APPLE__
    ...
    void UpdateOk() { ok_->SetEnabled(mode_ == Mode::OPEN_DIR || std::string(filename_->GetText()) != ""); }
};
...
FileDialog::FileDialog(Mode mode, const char *title, const Theme &theme)
    ...
    impl_->filename_->SetOnTextChanged([this](const char *) { this->impl_->UpdateOk(); });
    impl_->dirtree_->SetOnValueChanged([this](const char *, int) {
        auto newpath = this->impl_->UpdateDirectoryListing();
        SetPath(newpath.c_str());
    });
    impl_->filelist_->SetOnValueChanged([this](const char *value, bool is_double_click) {
...
void FileDialog::AddFilter(const char *filter, const char *description) {
    ...
    impl_->filter_idx_2_filter[int(impl_->filter_idx_2_filter.size())] = ext_filter;
...
void FileDialog::SetOnCancel(std::function<void()> on_cancel) { impl_->on_cancel_ = on_cancel; }

void FileDialog::SetOnDone(std::function<void(const char *)> on_done) { impl_->on_done_ = on_done; }
...
Size FileDialog::CalcPreferredSize(const LayoutContext &context, const Constraints &constraints) const {
    ...
    auto width = std::max(25 * em, Super::CalcPreferredSize(context, constraints).width);
