...
#include "vox.visual/gui/radio_button.h"
...
void RadioButton::SetOnSelectionChanged(std::function<void(int)> callback) {
    impl_->on_selection_changed_ = std::move(callback);
}

Size RadioButton::CalcPreferredSize(const LayoutContext& context, const Constraints& constraints) const {
    ...
    return Size{int(std::ceil(size.x + 2.0f * st.FramePadding.x)), int(std::ceil(size.y + 2.0f * st.FramePadding.y))};
}
...
Widget::DrawResult RadioButton::Draw(const DrawContext& context) {
    ...
    for (size_t i = 0; i < impl_->items_.size(); ++i) {
        ...
        if (is_selected) {
            ImGui::PushStyleColor(ImGuiCol_FrameBg, colorToImgui(context.theme.radiobtn_background_on_color));
            ImGui::PushStyleColor(ImGuiCol_FrameBgHovered,
                                  colorToImgui(context.theme.radiobtn_background_hover_on_color));
        } else {
            ImGui::PushStyleColor(ImGuiCol_FrameBg, colorToImgui(context.theme.radiobtn_background_off_color));
            ImGui::PushStyleColor(ImGuiCol_FrameBgHovered,
                                  colorToImgui(context.theme.radiobtn_background_hover_off_color));
        }
