...
void NumberEdit::SetDecimalPrecision(int num_digits) { impl_->num_decimal_digits_ = num_digits; }

void NumberEdit::SetPreferredWidth(int width) { impl_->preferred_width_ = width; }

void NumberEdit::SetOnValueChanged(std::function<void(double)> on_changed) {
    impl_->on_changed_ = std::move(on_changed);
}

Size NumberEdit::CalcPreferredSize(const LayoutContext& context, const Constraints& constraints) const {
    ...
    int width = (num_digits * context.theme.font_size) / 2 + padding + incdec_width;
...
Widget::DrawResult NumberEdit::Draw(const DrawContext& context) {
    ...
    if (impl_->type_ == INT) {
        ...
        if (ImGui::InputDouble(impl_->id_.c_str(), &dvalue, 0.0, 0.0, fmt.c_str())) {
            SetValue(dvalue);
            result = Widget::DrawResult::REDRAW;
        }
    }
