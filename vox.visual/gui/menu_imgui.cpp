...
#include "vox.visual/gui/menu_imgui.h"
...
std::string CalcShortcutText(vox::KeyCode key) {
    return "";
...
struct MenuImgui::Impl {
    struct MenuItem {
        MenuBase::ItemId id_;
        std::string name_;
        vox::KeyCode shortcut_key_;
        std::shared_ptr<MenuImgui> submenu_;
        MenuImgui::Impl *submenu_impl_ = nullptr;  // so FindMenuItem needn't be a friend
...
MenuImgui::~MenuImgui() = default;

void *MenuImgui::GetNativePointer() { return nullptr; }

void MenuImgui::AddItem(const char *name, ItemId itemId /*= NO_ITEM*/, vox::KeyCode key /*= KEY_NONE*/) {
...
void MenuImgui::InsertItem(int index, const char *name, ItemId item_id /*= NO_ITEM*/, vox::KeyCode key /*= KEY_NONE*/) {
    ...
    impl_->items_.insert(impl_->items_.begin() + index, {item_id, name, key, nullptr});
}

void MenuImgui::InsertMenu(int index, const char *name, std::shared_ptr<MenuBase> submenu) {
    for (auto &kv : impl_->id2idx_) {
        if (int(kv.second) >= index) {
            kv.second += 1;
        }
    ...
    impl_->items_.insert(impl_->items_.begin() + index,
                         {NO_ITEM, name, vox::KeyCode::UNKNOWN, imgui_submenu, imgui_submenu->impl_.get()});
}
...
void MenuImgui::InsertSeparator(int index) {
    ...
    impl_->items_.insert(impl_->items_.begin() + index,
                         {NO_ITEM, "", vox::KeyCode::UNKNOWN, nullptr, nullptr, false, false, true});
}
...
bool MenuImgui::CheckVisibilityChange() const { return impl_->submenu_visibility_changed_; }

MenuBase::ItemId MenuImgui::DrawMenuBar(const DrawContext &context, bool is_enabled) {
    ...
    auto padding = ImGui::GetStyle().FramePadding;
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(padding.x, padding.y + EXTRA_PADDING_Y));
    ...
    for (auto &item : impl_->items_) {
        if (item.submenu_) {
            ...
            auto id = item.submenu_->Draw(context, item.name_.c_str(), is_enabled);
            if (id >= 0) {
                activate_id = id;
            }
    ...
    draw_list->AddLine(ImVec2(0, y), ImVec2(size.x, y), context.theme.menubar_border_color.toABGR32(), 1.0f);
...
MenuBase::ItemId MenuImgui::Draw(const DrawContext &context, const char *name, bool is_enabled) {
    ItemId activate_id = NO_ITEM;
    ...
    for (auto &item : impl_->items_) {
        auto size1 = font->CalcTextSizeA(float(context.theme.font_size), 10000, 10000, item.name_.c_str());
        auto shortcut = CalcShortcutText(item.shortcut_key_);
        auto size2 = font->CalcTextSizeA(float(context.theme.font_size), 10000, 10000, shortcut.c_str());
        name_width = std::max(name_width, int(std::ceil(size1.x)));
        shortcut_width = std::max(shortcut_width, int(std::ceil(size2.x)));
    }
    int width =
            padding + name_width + 2 * em + shortcut_width + 2 * em + int(std::ceil(1.5 * em)) + padding;  // checkbox

    ImGui::SetNextWindowContentSize(ImVec2(float(width), 0));
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, float(context.theme.default_margin)));
    ImGui::PushStyleVar(ImGuiStyleVar_PopupRounding, float(context.theme.font_size) / 3.0f);
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing,
                        ImVec2(float(context.theme.default_margin), float(context.theme.default_margin)));
    ...
    if (ImGui::BeginMenu(name, is_enabled)) {
        ...
        for (auto &item : impl_->items_) {
            if (item.is_separator_) {
                ...
                auto possibility = item.submenu_->Draw(context, item.name_.c_str(), is_enabled);
                ...
                ImGui::MenuItem(item.name_.c_str(), shortcut_text.c_str(), item.is_checked_, item.is_enabled_);
            }
        }
        ImGui::EndMenu();
    } else {
