...
TEST_P(IndexerPermuteDevices, TensorRef) {
    ...
    EXPECT_EQ(core::SizeVector(tr.shape_, tr.shape_ + 3), core::SizeVector({2, 1, 3}));
    EXPECT_EQ(core::SizeVector(tr.byte_strides_, tr.byte_strides_ + 3), core::SizeVector({3 * 4, 3 * 4, 1 * 4}));
    ...
    EXPECT_EQ(core::SizeVector(tr_new.shape_, tr_new.shape_ + 3), core::SizeVector({2, 1, 3}));
...
TEST_P(IndexerPermuteDevices, IndexerCopyConstructor) {
    ...
    for (int64_t i = 0; i < indexer_a.NumDims(); i++) {
        ...
        EXPECT_EQ(indexer_a.GetMasterStrides()[i], indexer_b.GetMasterStrides()[i]);
...
TEST_P(IndexerPermuteDevices, BroadcastRestride) {
    ...
    EXPECT_EQ(core::SizeVector(indexer.GetMasterShape(), indexer.GetMasterShape() + indexer.NumDims()),
              core::SizeVector({2, 2, 2, 1, 3}));
    EXPECT_EQ(core::SizeVector(indexer.GetMasterStrides(), indexer.GetMasterStrides() + indexer.NumDims()),
              core::SizeVector({12, 6, 3, 3, 1}));

    // Check tensor shape
    EXPECT_EQ(core::SizeVector(input0_tr.shape_, input0_tr.shape_ + input0_tr.ndims_),
              core::SizeVector({1, 2, 1, 1, 3}));
    EXPECT_EQ(core::SizeVector(input1_tr.shape_, input1_tr.shape_ + input1_tr.ndims_),
              core::SizeVector({1, 1, 1, 1, 3}));
    EXPECT_EQ(core::SizeVector(output_tr.shape_, output_tr.shape_ + output_tr.ndims_),
              core::SizeVector({2, 2, 2, 1, 3}));

    // Check tensor strides
    EXPECT_EQ(core::SizeVector(input0_tr.byte_strides_, input0_tr.byte_strides_ + input0_tr.ndims_),
              core::SizeVector({0, 3 * 4, 0, 3 * 4, 1 * 4}));
    EXPECT_EQ(core::SizeVector(input1_tr.byte_strides_, input1_tr.byte_strides_ + input1_tr.ndims_),
              core::SizeVector({0, 0, 0, 3 * 4, 1 * 4}));
    EXPECT_EQ(core::SizeVector(output_tr.byte_strides_, output_tr.byte_strides_ + output_tr.ndims_),
...
TEST_P(IndexerPermuteDevices, GetPointersVectorized) {
    ...
    core::ParallelFor(
            core::Device("CPU:0"), size, [&](int64_t idx) { input0_ptrs[idx] = indexer.GetInputPtr(0, idx); },
            OPEN3D_VECTORIZED(GetInputPointersKernel, &ispc_indexer, 0, input0_ptrs));

    core::ParallelFor(
            core::Device("CPU:0"), size, [&](int64_t idx) { input1_ptrs[idx] = indexer.GetInputPtr(1, idx); },
            OPEN3D_VECTORIZED(GetInputPointersKernel, &ispc_indexer, 1, input1_ptrs));

    core::ParallelFor(
            core::Device("CPU:0"), size, [&](int64_t idx) { output_ptrs[idx] = indexer.GetOutputPtr(idx); },
            OPEN3D_VECTORIZED(GetOutputPointersKernel_Zero, &ispc_indexer, output_ptrs));

    void* expected_input0_ptrs[size] = {input0_base_ptr + 0 * dtype_byte_size, input0_base_ptr + 0 * dtype_byte_size,
                                        input0_base_ptr + 1 * dtype_byte_size, input0_base_ptr + 1 * dtype_byte_size,
                                        input0_base_ptr + 2 * dtype_byte_size, input0_base_ptr + 2 * dtype_byte_size};

    void* expected_input1_ptrs[size] = {input1_base_ptr + 0 * dtype_byte_size, input1_base_ptr + 1 * dtype_byte_size,
                                        input1_base_ptr + 0 * dtype_byte_size, input1_base_ptr + 1 * dtype_byte_size,
                                        input1_base_ptr + 0 * dtype_byte_size, input1_base_ptr + 1 * dtype_byte_size};

    void* expected_output_ptrs[size] = {output_base_ptr + 0 * dtype_byte_size, output_base_ptr + 1 * dtype_byte_size,
                                        output_base_ptr + 2 * dtype_byte_size, output_base_ptr + 3 * dtype_byte_size,
                                        output_base_ptr + 4 * dtype_byte_size, output_base_ptr + 5 * dtype_byte_size};
