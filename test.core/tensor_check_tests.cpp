...
TEST_P(TensorCheckPermuteDevices, AssertTensorDtype) {
    ...
    try {
        ...
        EXPECT_TRUE(utility::ContainsString(err.what(), "Tensor has dtype Float32, but is expected to have Int32."));
    ...
    core::AssertTensorDtype(t, core::Dtype(core::Dtype::DtypeCode::Float, 4, "Float32"));
    core::AssertTensorDtype(t, core::Dtype{core::Dtype::DtypeCode::Float, 4, "Float32"});
    try {
        core::AssertTensorDtype(t, core::Dtype(core::Dtype::DtypeCode::Float, 4, "Float64"));
        FAIL() << "Should not reach here.";
    } catch (std::runtime_error const& err) {
        EXPECT_TRUE(utility::ContainsString(err.what(), "Tensor has dtype Float32, but is expected to have Float64."));
        EXPECT_TRUE(utility::ContainsString(err.what(), "TensorCheck.cpp:"));
        EXPECT_TRUE(utility::ContainsString(err.what(), "AssertTensorDtype"));
    } catch (...) {
        FAIL() << "std::runtime_error not thrown.";
...
TEST_P(TensorCheckPermuteDevices, AssertTensorDtypes) {
    ...
    EXPECT_ANY_THROW(core::AssertTensorDtypes(t, std::vector<core::Dtype>{core::Int32, core::Int64}));
    EXPECT_ANY_THROW(core::AssertTensorDtypes(t, std::vector<core::Dtype>({core::Int32, core::Int64})));
}
...
TEST_P(TensorCheckPermuteDevices, AssertTensorDevice) {
    ...
    try {
        ...
        EXPECT_TRUE(utility::ContainsString(err.what(), "but is expected to have CUDA:1000"));
        EXPECT_TRUE(utility::ContainsString(err.what(), "TensorCheck.cpp:"));
...
TEST_P(TensorCheckPermuteDevices, AssertTensorShape) {
    ...
    try {
        ...
        EXPECT_TRUE(utility::ContainsString(err.what(), "Tensor has shape {10}, but is expected to have {}."));
        EXPECT_TRUE(utility::ContainsString(err.what(), "TensorCheck.cpp:"));
    ...
    try {
        ...
        EXPECT_TRUE(utility::ContainsString(err.what(), "Tensor has shape {10}, but is expected to have {1}."));
        EXPECT_TRUE(utility::ContainsString(err.what(), "TensorCheck.cpp:"));
        EXPECT_TRUE(utility::ContainsString(err.what(), "AssertTensorShape"));
    } catch (...) {
        FAIL() << "std::runtime_error not thrown.";
    ...
    try {
        ...
        EXPECT_TRUE(utility::ContainsString(err.what(), "Tensor has shape {10}, but is expected to have {1, 2}."));
        EXPECT_TRUE(utility::ContainsString(err.what(), "TensorCheck.cpp:"));
        EXPECT_TRUE(utility::ContainsString(err.what(), "AssertTensorShape"));
    } catch (...) {
        FAIL() << "std::runtime_error not thrown.";
    ...
    try {
        t = core::Tensor::Empty({10}, core::Float32, device);
        core::AssertTensorShape(t, core::DynamicSizeVector(core::SizeVector({1, 2})));
        FAIL() << "Should not reach here.";
    } catch (std::runtime_error const& err) {
        EXPECT_TRUE(utility::ContainsString(err.what(), "Tensor has shape {10}, but is expected to have {1, 2}."));
        EXPECT_TRUE(utility::ContainsString(err.what(), "TensorCheck.cpp:"));
