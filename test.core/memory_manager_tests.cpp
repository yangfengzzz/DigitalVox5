...
std::shared_ptr<core::CachedMemoryManager> MakeCachedDeviceMemoryManager(const core::Device& device) {
    if (device.GetType() == core::Device::DeviceType::CPU) {
        return std::make_shared<core::CachedMemoryManager>(std::make_shared<core::CPUMemoryManager>());
    }
#ifdef BUILD_CUDA_MODULE
    if (device.GetType() == core::Device::DeviceType::CUDA) {
        return std::make_shared<core::CachedMemoryManager>(std::make_shared<core::CUDAMemoryManager>());
    }
#endif
...
TEST_P(MemoryManagerPermuteDevicePairs, Memcpy) {
    ...
    core::MemoryManager::MemcpyFromHost(src_ptr, src_device, (void*)src_vals, num_bytes);

    core::MemoryManager::Memcpy(dst_ptr, dst_device, src_ptr, src_device, num_bytes);
    core::MemoryManager::MemcpyToHost((void*)dst_vals, dst_ptr, dst_device, num_bytes);
...
TEST(MemoryManagerPermuteDevices, NestedCachedMemoryManager) {
    ...
    EXPECT_THROW(std::make_shared<core::CachedMemoryManager>(cached_mm), std::runtime_error);
}
