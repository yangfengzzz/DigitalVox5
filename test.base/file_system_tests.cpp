...
TEST(FileSystem, ListFilesInDirectory) {
    std::string path = "test/test_dir";
    std::vector<std::string> fileNames = {"fileName00.ext", "fileName01.ext", "fileName02.ext", "fileName03.ext"};
    ...
    for (size_t i = 0; i < fileNames.size(); i++) {
        EXPECT_EQ(fileNames[i], utility::filesystem::GetFileNameWithoutDirectory(list[i]));
    }
...
TEST(FileSystem, ListFilesInDirectoryWithExtension) {
    std::string path = "test/test_dir";
    std::vector<std::string> fileNames = {"fileName00.ext0", "fileName01.ext0", "fileName02.ext0", "fileName03.ext0",
                                          "fileName04.ext1", "fileName05.ext1", "fileName06.ext1", "fileName07.ext1"};
...
TEST(FileSystem, GetPathComponents) {
    // setup
    std::string cwd = utility::filesystem::GetWorkingDirectory();
    std::vector<std::string> cwd_components = utility::filesystem::GetPathComponents(cwd);
    if (cwd_components.size() < 2) {
        LOGE("Please do not run unit test from root directory.")
    }
    std::vector<std::string> parent_components(cwd_components.begin(), cwd_components.end() - 1);
    ...
    expected.emplace_back("bogus");
    expected.emplace_back("test.abc");
    EXPECT_EQ(result, expected);
    ...
    result = utility::filesystem::GetPathComponents(R"(c:\windows\system\winnt.dll)");
