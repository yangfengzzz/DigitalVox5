...
inline Point2<T> Transform2<T>::toLocal(const Point2<T> &pointInWorld) const {
    ...
    return Point2<T>(_cosAngle * xmt.x + _sinAngle * xmt.y, -_sinAngle * xmt.x + _cosAngle * xmt.y);
}
...
inline Ray2<T> Transform2<T>::toLocal(const Ray2<T> &rayInWorld) const {
    return Ray2<T>(toLocal(rayInWorld.origin), toLocalDirection(rayInWorld.direction));
}
...
inline Point2<T> Transform2<T>::toWorld(const Point2<T> &pointInLocal) const {
    ...
    return Point2<T>(_cosAngle * pointInLocal.x - _sinAngle * pointInLocal.y + _translation.x,
...
inline Ray2<T> Transform2<T>::toWorld(const Ray2<T> &rayInLocal) const {
    return Ray2<T>(toWorld(rayInLocal.origin), toWorldDirection(rayInLocal.direction));
}
